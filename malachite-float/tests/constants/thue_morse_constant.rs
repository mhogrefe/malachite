...
fn thue_morse_constant_prec_properties() {
    unsigned_gen_var_11().test_properties(|prec| {
        ...
        assert_eq!(tmc.get_exponent(), Some(if prec == 1 { 0 } else { -1 }));
        assert_ne!(o, Equal);
        if o == Less {
...
fn thue_morse_constant_prec_round_properties() {
    unsigned_rounding_mode_pair_gen_var_4().test_properties(|(prec, rm)| {
        ...
        assert_eq!(tmc.get_exponent(), Some(expected_exponent));
        assert_ne!(o, Equal);
        if o == Less {
            let (tmc_alt, o_alt) = Float::thue_morse_constant_prec_round(prec, Ceiling);
            let mut next_upper = tmc.clone();
    ...
    test_constant(Float::thue_morse_constant_prec_round, 10000);
}
