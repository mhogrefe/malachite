...
#[test]
fn test_float_to_latex() {
    let test = |x: Float, out: &str| assert_eq!(x.to_latex_string(), out);
    // the specials, as the primitive floats write them
    test(Float::NAN, r"\text{NaN}");
    test(Float::INFINITY, r"\infty");
    test(Float::NEGATIVE_INFINITY, r"-\infty");
    ...
    test(Float::power_of_2(100u64), r"1.3 \times 10^{30}");
    test(Float::power_of_2(-100i64), r"7.9 \times 10^{-31}");
    // the digit count follows the precision, not the value
    test(Float::one_prec(100), "1.0000000000000000000000000000000");
}

#[test]
fn float_to_latex_properties() {
    float_gen().test_properties(|x| {
        ...
        assert_eq!(ComparableFloat(x.clone()).to_latex_string(), s);
        assert_eq!(ComparableFloatRef(&x).to_latex_string(), s);
    });
}
