...
#[test]
fn test_eq_abs_gaussian_rational() {
    let test = |s, t, out| {
    ...
    test("1.0", "i", true);
    test("-0.5", "i/2", true);
    test("1.0", "3/5+4i/5", true);
    test("-1.0", "3/5+4i/5", true);
    test("2.5", "3/2+2i", true);
    test("-2.5", "3/2+2i", true);
    test("2.0", "3/2+2i", false);
    test("0.5", "3/10+2i/5", true);
    test("5.0", "3+4i", true);
    test("3.0", "2+2i", false);
    test("NaN", "0", false);
    test("Infinity", "3/2+2i", false);
    test("-Infinity", "0", false);
}

#[test]
fn eq_abs_gaussian_rational_properties() {
    ...
    float_rational_pair_gen().test_properties(|(x, y)| {
        ...
        assert_eq!(GaussianRational::from(y.clone()).eq_abs(&x), y.eq_abs(&x));
    });
}
