...
use malachite_float::float::arithmetic::log_base_power_of_2_1_plus_x::*;
use malachite_float::test_util::common::{
    assert_rounding_ordering_consistent, rug_round_try_from_rounding_mode, to_hex_string,
};
use malachite_float::test_util::float::arithmetic::log_base_1_plus_x::{
...
fn log_base_1_plus_x_properties() {
    let f = |x: Float, base: u64, extreme: bool| {
        ...
        let expected = check(&x, base, prec, Nearest, extreme).0;
...
fn log_base_1_plus_x_prec_round_fail() {
    ...
    assert_panic!(Float::ONE.log_base_1_plus_x_prec_round(3, 10, Exact));
}
...
fn test_log_base_1_plus_x_round() {
    let test =
        |n: i64, d: u64, base: u64, rm: RoundingMode, out: &str, out_hex: &str, o_out: Ordering| {
            let x = Float::exact_from(Rational::from_signeds(n, i64::exact_from(d)));
