...
// bracket, where everything is `Rational` arithmetic, so no 2^30-bit `Float` is ever formed and the
// test is cheap.
#[test]
fn test_tan_rational_tiny_underflow() {
    ...
    // x = 2^(-2^30), the smallest positive Float: tan(x) is just above it
    let x = Rational::power_of_2(min_exp);
    test(&x, 1, Floor, min_positive(1), Less);
    test(&x, 10, Nearest, min_positive(10), Less);
    test(&x, 10, Ceiling, min_positive(10) << 1u32, Greater);
