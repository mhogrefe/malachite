...
fn log_base_2_1_plus_x_prec_round_properties_helper(x: Float, prec: u64, rm: RoundingMode) {
    let (log_base_2_1_plus_x, o) = x.clone().log_base_2_1_plus_x_prec_round(prec, rm);
    assert!(log_base_2_1_plus_x.is_valid());
    assert_rounding_ordering_consistent(&log_base_2_1_plus_x, rm, o);
...
fn test_primitive_float_log_base_2_1_plus_x() {
    ...
    test::<f32>(1.0, 1.0); // log2(2)
    test::<f32>(-0.5, -1.0); // log2(1/2)
    test::<f32>(3.0, 2.0); // log2(4)
    test::<f32>(7.0, 3.0); // log2(8)
    test::<f32>(100.0, 6.6582117); // log2(101)
    // A point where the naive `(1.0 + x).log2()` rounds differently (it gives -1.9262185e-5);
    // `primitive_float_log_base_2_1_plus_x` is correctly rounded.
    test::<f32>(-1.3376945e-5, -1.9298981e-5);
    ...
    test::<f64>(1.0, 1.0); // log2(2)
    test::<f64>(-0.5, -1.0); // log2(1/2)
    test::<f64>(3.0, 2.0); // log2(4)
    test::<f64>(7.0, 3.0); // log2(8)
    test::<f64>(100.0, 6.658211482751795); // log2(101)
    // A point where the naive `(1.0 + x).log2()` rounds differently (it gives
    // 3.0656791641505506e-13); `primitive_float_log_base_2_1_plus_x` is correctly rounded.
    test::<f64>(2.1255041535787165e-13, 3.0664543017565823e-13);
}
