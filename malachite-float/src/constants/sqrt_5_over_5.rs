...
impl Float {
    /// Returns an approximation of one fifth of the square root of 5, with the given precision and
    ...
    ///
    /// $$
    /// x = \sqrt{5}/5+\varepsilon=\sqrt{1/5}+\varepsilon=1/\sqrt{5}+\varepsilon.
    /// $$
    ...
    /// ```
    #[inline]
    pub fn sqrt_5_over_5_prec_round(prec: u64, rm: RoundingMode) -> (Self, Ordering) {
        Self::reciprocal_sqrt_prec_round(const { Self::const_from_unsigned(5) }, prec, rm)
    }
    ...
    /// ```
    #[inline]
    pub fn sqrt_5_over_5_prec(prec: u64) -> (Self, Ordering) {
        Self::sqrt_5_over_5_prec_round(prec, Nearest)
    }
}
