...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_primitive_float_demos!(runner, demo_primitive_float_ln_1_plus_x);
    ...
    register_primitive_float_benches!(runner, benchmark_primitive_float_ln_1_plus_x);
}
...
fn demo_primitive_float_ln_1_plus_x<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
    ...
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
    for x in primitive_float_gen::<T>().get(gm, config).take(limit) {
        println!(
            ...
            NiceFloat(x),
            NiceFloat(primitive_float_ln_1_plus_x(x))
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_ln_1_plus_x<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
{
    run_benchmark(
        &format!("primitive_float_ln_1_plus_x({})", T::NAME),
        BenchmarkType::Single,
        ...
        &mut [("malachite", &mut |x| {
            no_out!(primitive_float_ln_1_plus_x(x));
        })],
    );
}
