...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_demo!(runner, demo_float_exp_rational_prec);
    register_demo!(runner, demo_float_exp_rational_prec_debug);
    register_demo!(runner, demo_float_exp_rational_prec_ref);
    register_demo!(runner, demo_float_exp_rational_prec_ref_debug);
    register_demo!(runner, demo_float_exp_rational_prec_round);
    register_demo!(runner, demo_float_exp_rational_prec_round_debug);
    register_demo!(runner, demo_float_exp_rational_prec_round_ref);
    register_demo!(runner, demo_float_exp_rational_prec_round_ref_debug);
    register_primitive_float_demos!(runner, demo_primitive_float_exp);
    register_primitive_float_demos!(runner, demo_primitive_float_exp_rational);
    ...
    register_bench!(
        runner,
        benchmark_float_exp_rational_prec_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_exp_rational_prec_round_evaluation_strategy
    );
    register_primitive_float_benches!(runner, benchmark_primitive_float_exp);
    register_primitive_float_benches!(runner, benchmark_primitive_float_exp_rational);
}

fn demo_float_exp_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::exp_rational_prec({}, {}) = {:?}",
            n.clone(),
            p,
            Float::exp_rational_prec(n, p)
        );
    }
}

fn demo_float_exp_rational_prec_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        let (f, o) = Float::exp_rational_prec(n.clone(), p);
        println!(
            "Float::exp_rational_prec({}, {}) = ({:#x}, {:?})",
            n,
            p,
            ComparableFloat(f),
            o
...
fn demo_float_exp_rational_prec_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::exp_rational_prec_ref(&{}, {}) = {:?}",
            n,
            p,
            Float::exp_rational_prec_ref(&n, p)
        );
    }
}

fn demo_float_exp_rational_prec_ref_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_exp_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::exp_rational_prec_round({}, {}, {:?}) = {:?}",
            n.clone(),
            p,
            rm,
            Float::exp_rational_prec_round(n, p, rm)
        );
    }
}

fn demo_float_exp_rational_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_exp_rational_prec_round_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::exp_rational_prec_round_ref(&{}, {}, {:?}) = {:?}",
            n,
            p,
            rm,
            Float::exp_rational_prec_round_ref(&n, p, rm)
        );
    }
}

fn demo_float_exp_rational_prec_round_ref_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
...
fn benchmark_float_exp_rational_prec_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::exp_rational_prec(Rational, u64)",
        ...
        &mut [
            (
                "Float::exp_rational_prec(Rational, u64)",
                &mut |(n, prec)| no_out!(Float::exp_rational_prec(n, prec)),
            ),
            (
                "Float::exp_rational_prec_ref(&Rational, u64)",
                &mut |(n, prec)| no_out!(Float::exp_rational_prec_ref(&n, prec)),
            ),
        ],
    );
}

fn benchmark_float_exp_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::exp_rational_prec_round(Rational, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float::exp_rational_prec_round(Rational, u64, RoundingMode)",
                &mut |(n, prec, rm)| no_out!(Float::exp_rational_prec_round(n, prec, rm)),
            ),
            (
                "Float::exp_rational_prec_round_ref(&Rational, u64, RoundingMode)",
                &mut |(n, prec, rm)| no_out!(Float::exp_rational_prec_round_ref(&n, prec, rm)),
            ),
        ],
    );
}
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_exp_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
    ...
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
    for x in rational_gen().get(gm, config).take(limit) {
        println!(
            ...
            x,
            NiceFloat(primitive_float_exp_rational::<T>(&x))
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_exp_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
{
    run_benchmark(
        &format!("primitive_float_exp_rational::<{}>(Rational)", T::NAME),
        BenchmarkType::Single,
        ...
        &mut [("Malachite", &mut |x| {
            no_out!(primitive_float_exp_rational::<T>(&x));
        })],
    );
}
