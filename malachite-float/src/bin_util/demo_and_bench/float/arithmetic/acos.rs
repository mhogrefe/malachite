...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_demo!(runner, demo_float_acos_pi_prec_round);
    register_demo!(runner, demo_float_acos_pi_prec_round_debug);
    register_demo!(runner, demo_float_acos_pi_prec);
    register_demo!(runner, demo_float_acos_pi_round);
    register_demo!(runner, demo_float_acos_pi_prec_round_assign);
    register_demo!(runner, demo_float_acos_pi_rational_prec_round);
    register_demo!(runner, demo_float_acos_pi_rational_prec);
    register_primitive_float_demos!(runner, demo_primitive_float_acos_pi);
    register_primitive_float_demos!(runner, demo_primitive_float_acos_pi_rational);
    register_demo!(runner, demo_float_acos_pi);
    register_demo!(runner, demo_float_acos_pi_debug);
    register_demo!(runner, demo_float_acos_pi_ref);
    register_demo!(runner, demo_float_acos_pi_assign);
    register_bench!(runner, benchmark_float_acos_pi_evaluation_strategy);
}
...
fn demo_float_acos_pi_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_42()
        .get(gm, config)
        .take(limit)
    {
        println!(
            ...
            prec,
            rm,
            x.acos_pi_prec_round(prec, rm)
        );
    }
}

fn demo_float_acos_pi_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_42()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_acos_pi_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in float_unsigned_pair_gen_var_1().get(gm, config).take(limit) {
        println!(
            "({}).acos_pi_prec({}) = {:?}",
            x.clone(),
...
fn demo_float_acos_pi_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, rm) in float_rounding_mode_pair_gen_var_49()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_acos_pi_prec_round_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_42()
        .get(gm, config)
        .take(limit)
    {
        let x_old = x.clone();
        let o = x.acos_pi_prec_round_assign(prec, rm);
        println!("x := {x_old}; x.acos_pi_prec_round_assign({prec}, {rm}) = {o:?}; x = {x}");
    }
}

fn demo_float_acos_pi_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in rational_unsigned_rounding_mode_triple_gen_var_11()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::acos_pi_rational_prec_round({}, {}, {}) = {:?}",
            x.clone(),
            prec,
            rm,
            Float::acos_pi_rational_prec_round(x, prec, rm)
        );
    }
}

fn demo_float_acos_pi_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::acos_pi_rational_prec({}, {}) = {:?}",
            x.clone(),
            prec,
            Float::acos_pi_rational_prec(x, prec)
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_acos_pi<T: PrimitiveFloat>(gm: GenMode, config: &GenConfig, limit: usize)
where
    Float: From<T> + PartialOrd<T>,
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_acos_pi_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
fn demo_float_acos_pi(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        println!("({}).acos_pi() = {}", x_old, x.acos_pi());
    }
}

fn demo_float_acos_pi_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        println!(
            "({:#x}).acos_pi() = {:#x}",
            ComparableFloat(x_old),
            ComparableFloat(x.acos_pi())
        );
    }
}

fn demo_float_acos_pi_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        println!("(&{}).acos_pi_ref() = {}", x, x.acos_pi_ref());
    }
}

fn demo_float_acos_pi_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for mut x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        x.acos_pi_assign();
        println!("x := {x_old}; x.acos_pi_assign(); x = {x}");
    }
}

fn benchmark_float_acos_pi_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ("Float.acos_pi()", &mut |x| no_out!(x.acos_pi())),
            ("(&Float).acos_pi_ref()", &mut |x| no_out!(x.acos_pi_ref())),
        ],
    );
}
