...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_demo!(runner, demo_float_asec_with_period_rational_prec_round);
    register_demo!(
        runner,
        demo_float_asec_with_period_rational_prec_round_debug
    );
    register_demo!(runner, demo_float_asec_with_period_rational_prec);
    register_primitive_float_demos!(runner, demo_primitive_float_asec_with_period_rational);
    register_bench!(
        runner,
        benchmark_float_asec_with_period_rational_prec_round_evaluation_strategy
    );
    register_primitive_float_benches!(runner, benchmark_primitive_float_asec_with_period_rational);
    register_demo!(runner, demo_float_asec_pi_prec_round);
    register_demo!(runner, demo_float_asec_pi_prec_round_debug);
    register_demo!(runner, demo_float_asec_pi_prec);
    register_demo!(runner, demo_float_asec_pi_round);
    register_demo!(runner, demo_float_asec_pi_prec_round_assign);
    register_demo!(runner, demo_float_asec_pi_rational_prec_round);
    register_demo!(runner, demo_float_asec_pi_rational_prec);
    register_primitive_float_demos!(runner, demo_primitive_float_asec_pi);
    register_primitive_float_demos!(runner, demo_primitive_float_asec_pi_rational);
    register_demo!(runner, demo_float_asec_pi);
    register_demo!(runner, demo_float_asec_pi_debug);
    register_demo!(runner, demo_float_asec_pi_ref);
    register_demo!(runner, demo_float_asec_pi_assign);
    register_bench!(runner, benchmark_float_asec_pi_evaluation_strategy);
}
...
fn demo_float_asec_with_period_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, rm) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_9()
        .get(gm, config)
        .take(limit)
    {
        println!(
            ...
            prec,
            rm,
            Float::asec_with_period_rational_prec_round(x, u, prec, rm)
        );
    }
}

fn demo_float_asec_with_period_rational_prec_round_debug(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
        ...
        .take(limit)
    {
        ...
        println!(
            "Float::asec_with_period_rational_prec_round({}, {}, {}, {}) = ({:#x}, {:?})",
            x,
            u,
            prec,
            rm,
...
fn demo_float_asec_with_period_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_9()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::asec_with_period_rational_prec({}, {}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            Float::asec_with_period_rational_prec(x, u, prec)
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_asec_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
    ...
    for<'a> T: ExactFrom<&'a Float>,
{
        ...
        .take(limit)
    {
        println!(
            ...
            x,
            u,
            NiceFloat(primitive_float_asec_with_period_rational::<T>(&x, u))
        );
    }
}

fn benchmark_float_asec_with_period_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::asec_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_9().get(gm, config),
        gm.name(),
        limit,
        file_name,
        ...
        &mut [
            (
                "Float::asec_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::asec_with_period_rational_prec_round(x, u, prec, rm));
                },
            ),
            (
                "Float::asec_with_period_rational_prec_round_ref(&Rational, u64, u64, \
                 RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::asec_with_period_rational_prec_round_ref(
                        &x, u, prec, rm
                    ));
                },
            ),
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_asec_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    for<'a> T: ExactFrom<&'a Float>,
{
    run_benchmark(
        &format!(
            "primitive_float_asec_with_period_rational({}, u64)",
            T::NAME
        ),
        BenchmarkType::Single,
        ...
        &mut [("malachite", &mut |(x, u)| {
            no_out!(primitive_float_asec_with_period_rational::<T>(&x, u));
        })],
    );
}

fn demo_float_asec_pi_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_45()
        .get(gm, config)
        .take(limit)
    {
        println!(
            ...
            prec,
            rm,
            x.asec_pi_prec_round(prec, rm)
        );
    }
}

fn demo_float_asec_pi_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_45()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_asec_pi_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in float_unsigned_pair_gen_var_1().get(gm, config).take(limit) {
        println!(
            "({}).asec_pi_prec({}) = {:?}",
            x.clone(),
...
fn demo_float_asec_pi_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, rm) in float_rounding_mode_pair_gen_var_50()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_asec_pi_prec_round_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_45()
        .get(gm, config)
        .take(limit)
    {
        let x_old = x.clone();
        let o = x.asec_pi_prec_round_assign(prec, rm);
        println!("x := {x_old}; x.asec_pi_prec_round_assign({prec}, {rm}) = {o:?}; x = {x}");
    }
}

fn demo_float_asec_pi_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in rational_unsigned_rounding_mode_triple_gen_var_12()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::asec_pi_rational_prec_round({}, {}, {}) = {:?}",
            x.clone(),
            prec,
            rm,
            Float::asec_pi_rational_prec_round(x, prec, rm)
        );
    }
}

fn demo_float_asec_pi_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::asec_pi_rational_prec({}, {}) = {:?}",
            x.clone(),
            prec,
            Float::asec_pi_rational_prec(x, prec)
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_asec_pi<T: PrimitiveFloat>(gm: GenMode, config: &GenConfig, limit: usize)
where
    Float: From<T> + PartialOrd<T>,
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_asec_pi_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
fn demo_float_asec_pi(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        println!("({}).asec_pi() = {}", x_old, x.asec_pi());
    }
}

fn demo_float_asec_pi_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        println!(
            "({:#x}).asec_pi() = {:#x}",
            ComparableFloat(x_old),
            ComparableFloat(x.asec_pi())
        );
    }
}

fn demo_float_asec_pi_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        println!("(&{}).asec_pi_ref() = {}", x, x.asec_pi_ref());
    }
}

fn demo_float_asec_pi_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for mut x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        x.asec_pi_assign();
        println!("x := {x_old}; x.asec_pi_assign(); x = {x}");
    }
}

fn benchmark_float_asec_pi_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ("Float.asec_pi()", &mut |x| no_out!(x.asec_pi())),
            ("(&Float).asec_pi_ref()", &mut |x| no_out!(x.asec_pi_ref())),
        ],
    );
}
