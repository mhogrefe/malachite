...
use malachite_float::test_util::float::arithmetic::sin_cos::{
    rug_sin_cos, rug_sin_cos_prec_round, rug_sin_cos_with_period_prec_round,
};
...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_demo!(runner, demo_float_sin_cos_with_period_prec_round_ref);
    register_demo!(runner, demo_float_sin_cos_with_period_prec_round_assign);
    register_demo!(runner, demo_float_sin_cos_with_period_prec);
    register_demo!(runner, demo_float_sin_cos_with_period_prec_debug);
    register_demo!(runner, demo_float_sin_cos_with_period_prec_ref);
    register_demo!(runner, demo_float_sin_cos_with_period_prec_assign);
    register_demo!(runner, demo_float_sin_cos_with_period_round);
    register_demo!(runner, demo_float_sin_cos_with_period_round_debug);
    register_demo!(runner, demo_float_sin_cos_with_period_round_ref);
    register_demo!(runner, demo_float_sin_cos_with_period_round_assign);
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_prec_round_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_prec_round_library_comparison
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_prec_round_algorithms
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_prec_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_round_evaluation_strategy
    );
    register_demo!(runner, demo_float_sin_cos_with_period_rational_prec_round);
    register_demo!(
        runner,
        demo_float_sin_cos_with_period_rational_prec_round_debug
    );
    register_demo!(
        runner,
        demo_float_sin_cos_with_period_rational_prec_round_ref
    );
    register_demo!(runner, demo_float_sin_cos_with_period_rational_prec);
    register_demo!(runner, demo_float_sin_cos_with_period_rational_prec_debug);
    register_demo!(runner, demo_float_sin_cos_with_period_rational_prec_ref);
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_rational_prec_round_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_rational_prec_round_algorithms
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_with_period_rational_prec_evaluation_strategy
    );
    register_primitive_float_demos!(runner, demo_primitive_float_sin_cos_with_period);
    register_primitive_float_demos!(runner, demo_primitive_float_sin_cos_with_period_rational);
    register_primitive_float_benches!(runner, benchmark_primitive_float_sin_cos_with_period);
    register_primitive_float_benches!(
        runner,
        benchmark_primitive_float_sin_cos_with_period_rational
    );
    ...
    register_demo!(runner, demo_float_sin_cos_rational_prec);
    register_demo!(runner, demo_float_sin_cos_rational_prec_ref);
    register_bench!(
        runner,
        benchmark_float_sin_cos_rational_prec_round_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_rational_prec_round_algorithms
    );
    register_bench!(
        runner,
        benchmark_float_sin_cos_rational_prec_evaluation_strategy
    );
...
fn demo_float_sin_cos_with_period_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, rm) in float_unsigned_unsigned_rounding_mode_quadruple_gen_var_17()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "({}).sin_cos_with_period_prec_round({}, {}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            rm,
            x.sin_cos_with_period_prec_round(u, prec, rm)
        );
    }
}

fn demo_float_sin_cos_with_period_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, rm) in float_unsigned_unsigned_rounding_mode_quadruple_gen_var_17()
        .get(gm, config)
        .take(limit)
    {
        ...
        println!(
            "({:#x}).sin_cos_with_period_prec_round({}, {}, {}) = ({:#x}, {:#x}, {:?}, {:?})",
            ComparableFloat(x),
            u,
            prec,
            rm,
...
fn demo_float_sin_cos_with_period_prec_round_extreme(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
...
fn demo_float_sin_cos_with_period_prec_round_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, rm) in float_unsigned_unsigned_rounding_mode_quadruple_gen_var_17()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "(&{}).sin_cos_with_period_prec_round_ref({}, {}, {}) = {:?}",
            x,
            u,
            prec,
            rm,
...
fn demo_float_sin_cos_with_period_prec_round_assign(gm: GenMode, config: &GenConfig, limit: usize) {
        ...
        .take(limit)
    {
        let x_old = x.clone();
        let mut c = Float::NAN;
        let o = x.sin_cos_with_period_prec_round_assign(&mut c, u, prec, rm);
        println!(
            "x := {x_old}; x.sin_cos_with_period_prec_round_assign(&mut c, {u}, {prec}, {rm}) = \
             {o:?}; x = {x}; c = {c}"
        );
    }
}

fn demo_float_sin_cos_with_period_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec) in float_unsigned_unsigned_triple_gen_var_1::<u64, u64>()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "({}).sin_cos_with_period_prec({}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            x.sin_cos_with_period_prec(u, prec)
        );
    }
}

fn demo_float_sin_cos_with_period_prec_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec) in float_unsigned_unsigned_triple_gen_var_1::<u64, u64>()
        .get(gm, config)
        .take(limit)
    {
        ...
        println!(
            "({:#x}).sin_cos_with_period_prec({}, {}) = ({:#x}, {:#x}, {:?}, {:?})",
            ComparableFloat(x),
            u,
            prec,
            ComparableFloat(s),
...
fn demo_float_sin_cos_with_period_prec_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec) in float_unsigned_unsigned_triple_gen_var_1::<u64, u64>()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "(&{}).sin_cos_with_period_prec_ref({}, {}) = {:?}",
            x,
            u,
            prec,
            x.sin_cos_with_period_prec_ref(u, prec)
        );
    }
}

fn demo_float_sin_cos_with_period_prec_assign(gm: GenMode, config: &GenConfig, limit: usize) {
        ...
        .take(limit)
    {
        let x_old = x.clone();
        let mut c = Float::NAN;
        let o = x.sin_cos_with_period_prec_assign(&mut c, u, prec);
        println!(
            "x := {x_old}; x.sin_cos_with_period_prec_assign(&mut c, {u}, {prec}) = {o:?}; \
             x = {x}; c = {c}"
        );
    }
}

fn demo_float_sin_cos_with_period_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, rm) in float_unsigned_rounding_mode_triple_gen_var_39()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "({}).sin_cos_with_period_round({}, {}) = {:?}",
            x.clone(),
            u,
            rm,
            x.sin_cos_with_period_round(u, rm)
        );
    }
}

fn demo_float_sin_cos_with_period_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, rm) in float_unsigned_rounding_mode_triple_gen_var_39()
        .get(gm, config)
        .take(limit)
    {
        ...
        println!(
            "({:#x}).sin_cos_with_period_round({}, {}) = ({:#x}, {:#x}, {:?}, {:?})",
            ComparableFloat(x),
            u,
            rm,
            ComparableFloat(s),
...
fn demo_float_sin_cos_with_period_round_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, rm) in float_unsigned_rounding_mode_triple_gen_var_39()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "(&{}).sin_cos_with_period_round_ref({}, {}) = {:?}",
            x,
            u,
            rm,
            x.sin_cos_with_period_round_ref(u, rm)
        );
    }
}

fn demo_float_sin_cos_with_period_round_assign(gm: GenMode, config: &GenConfig, limit: usize) {
        ...
        .take(limit)
    {
        let x_old = x.clone();
        let mut c = Float::NAN;
        let o = x.sin_cos_with_period_round_assign(&mut c, u, rm);
        println!(
            "x := {x_old}; x.sin_cos_with_period_round_assign(&mut c, {u}, {rm}) = {o:?}; \
             x = {x}; c = {c}"
        );
    }
}

fn benchmark_float_sin_cos_with_period_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float.sin_cos_with_period_prec_round(u64, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float.sin_cos_with_period_prec_round(u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| no_out!(x.sin_cos_with_period_prec_round(u, prec, rm)),
            ),
            (
                "(&Float).sin_cos_with_period_prec_round_ref(u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(x.sin_cos_with_period_prec_round_ref(u, prec, rm));
                },
            ),
        ],
    );
...
fn benchmark_float_sin_cos_with_period_prec_round_library_comparison(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ("Malachite", &mut |(x, u, prec, rm)| {
                no_out!(x.sin_cos_with_period_prec_round(u, prec, rm));
            }),
            ("rug", &mut |(x, u, prec, rm)| {
                no_out!(rug_sin_cos_with_period_prec_round(
...
fn benchmark_float_sin_cos_with_period_prec_round_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &quadruple_1_float_complexity_bucketer("x"),
        &mut [
            ("together", &mut |(x, u, prec, rm)| {
                no_out!(x.sin_cos_with_period_prec_round(u, prec, rm));
            }),
            ("separately", &mut |(x, u, prec, rm)| {
                no_out!((
...
fn benchmark_float_sin_cos_with_period_prec_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float.sin_cos_with_period_prec(u64, u64)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float.sin_cos_with_period_prec(u64, u64)",
                &mut |(x, u, prec)| no_out!(x.sin_cos_with_period_prec(u, prec)),
            ),
            (
                "(&Float).sin_cos_with_period_prec_ref(u64, u64)",
                &mut |(x, u, prec)| no_out!(x.sin_cos_with_period_prec_ref(u, prec)),
            ),
        ],
    );
}

fn benchmark_float_sin_cos_with_period_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float.sin_cos_with_period_round(u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float.sin_cos_with_period_round(u64, RoundingMode)",
                &mut |(x, u, rm)| no_out!(x.sin_cos_with_period_round(u, rm)),
            ),
            (
                "(&Float).sin_cos_with_period_round_ref(u64, RoundingMode)",
                &mut |(x, u, rm)| no_out!(x.sin_cos_with_period_round_ref(u, rm)),
            ),
        ],
    );
}

fn demo_float_sin_cos_with_period_rational_prec_round(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
    ...
    {
        println!(
            "Float::sin_cos_with_period_rational_prec_round({}, {}, {}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            rm,
...
fn demo_float_sin_cos_with_period_rational_prec_round_debug(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
        ...
        .take(limit)
    {
        ...
        println!(
            "Float::sin_cos_with_period_rational_prec_round({}, {}, {}, {}) = ({:#x}, {:#x}, {:?}, \
             {:?})",
            x,
            u,
            prec,
...
fn demo_float_sin_cos_with_period_rational_prec_round_ref(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
    ...
    {
        println!(
            "Float::sin_cos_with_period_rational_prec_round_ref(&{}, {}, {}, {}) = {:?}",
            x,
            u,
            prec,
            rm,
...
fn demo_float_sin_cos_with_period_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_5()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_with_period_rational_prec({}, {}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            Float::sin_cos_with_period_rational_prec(x, u, prec)
        );
    }
}

fn demo_float_sin_cos_with_period_rational_prec_debug(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
        ...
        .take(limit)
    {
        ...
        println!(
            "Float::sin_cos_with_period_rational_prec({}, {}, {}) = ({:#x}, {:#x}, {:?}, {:?})",
            x,
            u,
            prec,
            ComparableFloat(s),
...
fn demo_float_sin_cos_with_period_rational_prec_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_5()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_with_period_rational_prec_ref(&{}, {}, {}) = {:?}",
            x,
            u,
            prec,
...
fn benchmark_float_sin_cos_with_period_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::sin_cos_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float::sin_cos_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::sin_cos_with_period_rational_prec_round(
                        x, u, prec, rm
                    ));
                },
            ),
            (
                "Float::sin_cos_with_period_rational_prec_round_ref(&Rational, u64, u64, \
                 RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::sin_cos_with_period_rational_prec_round_ref(
                        &x, u, prec, rm
                    ));
                },
            ),
...
fn benchmark_float_sin_cos_with_period_rational_prec_round_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
        ...
        &mut [
            ...
            ("separately", &mut |(x, u, prec, rm)| {
                no_out!((
                    ...
                    Float::cos_with_period_rational_prec_round(x, u, prec, rm)
                ));
            }),
        ],
    );
...
fn benchmark_float_sin_cos_with_period_rational_prec_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::sin_cos_with_period_rational_prec(Rational, u64, u64)",
        BenchmarkType::EvaluationStrategy,
        rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_5().get(gm, config),
        gm.name(),
        limit,
        ...
        &mut [
            (
                "Float::sin_cos_with_period_rational_prec(Rational, u64, u64)",
                &mut |(x, u, prec, _)| {
                    no_out!(Float::sin_cos_with_period_rational_prec(x, u, prec));
                },
            ),
            (
                "Float::sin_cos_with_period_rational_prec_ref(&Rational, u64, u64)",
                &mut |(x, u, prec, _)| {
                    no_out!(Float::sin_cos_with_period_rational_prec_ref(&x, u, prec));
                },
            ),
        ],
    );
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_sin_cos_with_period<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
    ...
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
        ...
        .take(limit)
    {
        ...
        println!(
            "primitive_float_sin_cos_with_period({}, {}) = ({}, {})",
            NiceFloat(x),
            u,
            NiceFloat(s),
            NiceFloat(c)
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_sin_cos_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_sin_cos_with_period<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
{
    run_benchmark(
        &format!("primitive_float_sin_cos_with_period({}, u64)", T::NAME),
        BenchmarkType::Algorithms,
        ...
        &mut [
            ("together", &mut |(x, u)| {
                no_out!(primitive_float_sin_cos_with_period(x, u));
            }),
            ("separately", &mut |(x, u)| {
                no_out!((
                    sin::primitive_float_sin_with_period(x, u),
                    cos::primitive_float_cos_with_period(x, u)
                ));
            }),
        ],
    );
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_sin_cos_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
    run_benchmark(
        &format!(
            "primitive_float_sin_cos_with_period_rational::<{}>(&Rational, u64)",
            T::NAME
        ),
        BenchmarkType::Algorithms,
        ...
        &mut [
            ("together", &mut |(x, u)| {
                no_out!(primitive_float_sin_cos_with_period_rational::<T>(&x, u));
            }),
            ("separately", &mut |(x, u)| {
                no_out!((
                    sin::primitive_float_sin_with_period_rational::<T>(&x, u),
                    cos::primitive_float_cos_with_period_rational::<T>(&x, u)
                ));
            }),
        ],
    );
...
fn demo_float_sin_cos_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_rational_prec_round({}, {}, {}) = {:?}",
            n.clone(),
            p,
            rm,
            Float::sin_cos_rational_prec_round(n, p, rm)
        );
    }
}

fn demo_float_sin_cos_rational_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        let (s, c, o_s, o_c) = Float::sin_cos_rational_prec_round(n.clone(), p, rm);
        println!(
            "Float::sin_cos_rational_prec_round({}, {}, {}) = ({:#x}, {:#x}, {:?}, {:?})",
            n,
            p,
            rm,
            ComparableFloat(s),
...
fn demo_float_sin_cos_rational_prec_round_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_rational_prec_round_ref(&{}, {}, {}) = {:?}",
            n,
            p,
            rm,
            Float::sin_cos_rational_prec_round_ref(&n, p, rm)
        );
    }
}

fn demo_float_sin_cos_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_rational_prec({}, {}) = {:?}",
            n.clone(),
            p,
            Float::sin_cos_rational_prec(n, p)
        );
    }
}

fn demo_float_sin_cos_rational_prec_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (n, p) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::sin_cos_rational_prec_ref(&{}, {}) = {:?}",
            n,
            p,
            Float::sin_cos_rational_prec_ref(&n, p)
        );
    }
}

#[allow(unused_must_use)]
fn benchmark_float_sin_cos_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::sin_cos_rational_prec_round(Rational, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float::sin_cos_rational_prec_round(Rational, u64, RoundingMode)",
                &mut |(n, p, rm)| no_out!(Float::sin_cos_rational_prec_round(n, p, rm)),
            ),
            (
                "Float::sin_cos_rational_prec_round_ref(&Rational, u64, RoundingMode)",
                &mut |(n, p, rm)| no_out!(Float::sin_cos_rational_prec_round_ref(&n, p, rm)),
            ),
        ],
    );
}
...
#[allow(unused_must_use)]
fn benchmark_float_sin_cos_rational_prec_round_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ("together", &mut |(n, p, rm)| {
                no_out!(Float::sin_cos_rational_prec_round(n, p, rm));
            }),
...
#[allow(unused_must_use)]
fn benchmark_float_sin_cos_rational_prec_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::sin_cos_rational_prec(Rational, u64)",
        BenchmarkType::EvaluationStrategy,
        ...
        &mut [
            (
                "Float::sin_cos_rational_prec(Rational, u64)",
                &mut |(n, p)| no_out!(Float::sin_cos_rational_prec(n, p)),
            ),
            (
                "Float::sin_cos_rational_prec_ref(&Rational, u64)",
