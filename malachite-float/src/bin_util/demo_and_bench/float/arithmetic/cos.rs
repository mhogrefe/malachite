...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_primitive_float_demos!(runner, demo_primitive_float_cos_with_period);
    register_primitive_float_demos!(runner, demo_primitive_float_cos_with_period_rational);
    ...
    register_demo!(runner, demo_float_cos_with_period_rational_prec_round);
    register_demo!(runner, demo_float_cos_with_period_rational_prec_round_debug);
    register_demo!(runner, demo_float_cos_with_period_rational_prec_round_ref);
    register_demo!(runner, demo_float_cos_with_period_rational_prec);
    register_demo!(runner, demo_float_cos_with_period_rational_prec_debug);
    register_demo!(runner, demo_float_cos_with_period_rational_prec_ref);
    register_demo!(runner, demo_float_cos_pi_prec_round);
    register_demo!(runner, demo_float_cos_pi_prec_round_debug);
    register_demo!(runner, demo_float_cos_pi_prec);
    register_demo!(runner, demo_float_cos_pi_round);
    register_demo!(runner, demo_float_cos_pi_prec_round_assign);
    register_demo!(runner, demo_float_cos_pi_rational_prec_round);
    register_demo!(runner, demo_float_cos_pi_rational_prec);
    register_primitive_float_demos!(runner, demo_primitive_float_cos_pi);
    register_primitive_float_demos!(runner, demo_primitive_float_cos_pi_rational);
    ...
    register_primitive_float_benches!(runner, benchmark_primitive_float_cos_with_period);
    register_primitive_float_benches!(runner, benchmark_primitive_float_cos_with_period_rational);
    register_bench!(
        runner,
        benchmark_float_cos_pi_prec_round_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_cos_pi_rational_prec_round_evaluation_strategy
    );
    register_primitive_float_benches!(runner, benchmark_primitive_float_cos_pi);
    register_bench!(
        runner,
    ...
    register_bench!(
        runner,
        benchmark_float_cos_with_period_rational_prec_round_evaluation_strategy
    );
    register_bench!(
        runner,
        benchmark_float_cos_with_period_rational_prec_evaluation_strategy
    );
    register_demo!(runner, demo_float_cos_with_period);
    register_demo!(runner, demo_float_cos_with_period_debug);
    register_demo!(runner, demo_float_cos_with_period_ref);
    register_demo!(runner, demo_float_cos_with_period_assign);
    register_bench!(runner, benchmark_float_cos_with_period_evaluation_strategy);
    register_demo!(runner, demo_float_cos_pi);
    register_demo!(runner, demo_float_cos_pi_debug);
    register_demo!(runner, demo_float_cos_pi_ref);
    register_demo!(runner, demo_float_cos_pi_assign);
    register_bench!(runner, benchmark_float_cos_pi_evaluation_strategy);
}
...
fn demo_float_cos_with_period_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, rm) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4()
        .get(gm, config)
        .take(limit)
    {
        println!(
...
fn demo_float_cos_with_period_rational_prec_round_debug(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
        ...
        .take(limit)
    {
        ...
        println!(
            "Float::cos_with_period_rational_prec_round({}, {}, {}, {}) = ({:#x}, {:?})",
            x,
            u,
            prec,
            rm,
...
fn demo_float_cos_with_period_rational_prec_round_ref(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
    ...
    {
        println!(
            "Float::cos_with_period_rational_prec_round_ref(&{}, {}, {}, {}) = {:?}",
            x,
            u,
            prec,
            rm,
            Float::cos_with_period_rational_prec_round_ref(&x, u, prec, rm)
        );
    }
}

fn demo_float_cos_with_period_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::cos_with_period_rational_prec({}, {}, {}) = {:?}",
            x.clone(),
            u,
            prec,
            Float::cos_with_period_rational_prec(x, u, prec)
        );
    }
}

fn demo_float_cos_with_period_rational_prec_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4()
        .get(gm, config)
        .take(limit)
    {
        ...
        println!(
            "Float::cos_with_period_rational_prec({}, {}, {}) = ({:#x}, {:?})",
            x,
            u,
            prec,
            ComparableFloat(c),
...
fn demo_float_cos_with_period_rational_prec_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u, prec, _) in rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::cos_with_period_rational_prec_ref(&{}, {}, {}) = {:?}",
            x,
            u,
            prec,
            Float::cos_with_period_rational_prec_ref(&x, u, prec)
        );
    }
}

fn benchmark_float_cos_with_period_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::cos_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4().get(gm, config),
        gm.name(),
        limit,
        file_name,
        ...
        &mut [
            (
                "Float::cos_with_period_rational_prec_round(Rational, u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::cos_with_period_rational_prec_round(x, u, prec, rm));
                },
            ),
            (
                "Float::cos_with_period_rational_prec_round_ref(&Rational, u64, u64, RoundingMode)",
                &mut |(x, u, prec, rm)| {
                    no_out!(Float::cos_with_period_rational_prec_round_ref(
                        &x, u, prec, rm
                    ));
                },
            ),
...
fn benchmark_float_cos_with_period_rational_prec_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::cos_with_period_rational_prec(Rational, u64, u64)",
        BenchmarkType::EvaluationStrategy,
        rational_unsigned_unsigned_rounding_mode_quadruple_gen_var_4().get(gm, config),
        gm.name(),
        limit,
        ...
        &mut [
            (
                "Float::cos_with_period_rational_prec(Rational, u64, u64)",
                &mut |(x, u, prec, _)| no_out!(Float::cos_with_period_rational_prec(x, u, prec)),
            ),
            (
                "Float::cos_with_period_rational_prec_ref(&Rational, u64, u64)",
                &mut |(x, u, prec, _)| {
                    no_out!(Float::cos_with_period_rational_prec_ref(&x, u, prec));
                },
            ),
        ],
    );
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_cos_with_period<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_cos_with_period<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
{
    run_benchmark(
        &format!("primitive_float_cos_with_period({}, u64)", T::NAME),
        BenchmarkType::Single,
        ...
        &mut [("malachite", &mut |(x, u)| {
            no_out!(primitive_float_cos_with_period(x, u));
        })],
    );
}

#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_cos_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_cos_with_period_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
    run_benchmark(
        &format!(
            "primitive_float_cos_with_period_rational::<{}>(&Rational, u64)",
            T::NAME
        ),
        BenchmarkType::Single,
        ...
        &mut [("malachite", &mut |(x, u)| {
            no_out!(primitive_float_cos_with_period_rational::<T>(&x, u));
        })],
    );
}

fn demo_float_cos_pi_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_36()
        .get(gm, config)
        .take(limit)
    {
        println!(
            ...
            prec,
            rm,
            x.cos_pi_prec_round(prec, rm)
        );
    }
}

fn demo_float_cos_pi_prec_round_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_36()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_cos_pi_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in float_unsigned_pair_gen_var_1().get(gm, config).take(limit) {
        println!(
            "({}).cos_pi_prec({}) = {:?}",
            x.clone(),
...
fn demo_float_cos_pi_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, rm) in float_rounding_mode_pair_gen_var_47()
        .get(gm, config)
        .take(limit)
    {
...
fn demo_float_cos_pi_prec_round_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, prec, rm) in float_unsigned_rounding_mode_triple_gen_var_36()
        .get(gm, config)
        .take(limit)
    {
        let x_old = x.clone();
        let o = x.cos_pi_prec_round_assign(prec, rm);
        println!("x := {x_old}; x.cos_pi_prec_round_assign({prec}, {rm}) = {o:?}; x = {x}");
    }
}

fn demo_float_cos_pi_rational_prec_round(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec, rm) in rational_unsigned_rounding_mode_triple_gen_var_10()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::cos_pi_rational_prec_round({}, {}, {}) = {:?}",
            x.clone(),
            prec,
            rm,
            Float::cos_pi_rational_prec_round(x, prec, rm)
        );
    }
}

fn demo_float_cos_pi_rational_prec(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, prec) in rational_unsigned_pair_gen_var_3()
        .get(gm, config)
        .take(limit)
    {
        println!(
            "Float::cos_pi_rational_prec({}, {}) = {:?}",
            x.clone(),
            prec,
            Float::cos_pi_rational_prec(x, prec)
        );
    }
}

#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_cos_pi<T: PrimitiveFloat>(gm: GenMode, config: &GenConfig, limit: usize)
where
    Float: From<T> + PartialOrd<T>,
    for<'a> T: ExactFrom<&'a Float> + RoundingFrom<&'a Float>,
{
...
#[allow(clippy::type_repetition_in_bounds)]
fn demo_primitive_float_cos_pi_rational<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) where
...
fn benchmark_float_cos_pi_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float.cos_pi_prec_round(u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        float_unsigned_rounding_mode_triple_gen_var_36().get(gm, config),
        gm.name(),
        limit,
        ...
        &mut [
            (
                "Float.cos_pi_prec_round(u64, RoundingMode)",
                &mut |(x, prec, rm)| no_out!(x.cos_pi_prec_round(prec, rm)),
            ),
            (
                "(&Float).cos_pi_prec_round_ref(u64, RoundingMode)",
                &mut |(x, prec, rm)| no_out!(x.cos_pi_prec_round_ref(prec, rm)),
            ),
        ],
    );
}

fn benchmark_float_cos_pi_rational_prec_round_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Float::cos_pi_rational_prec_round(Rational, u64, RoundingMode)",
        BenchmarkType::EvaluationStrategy,
        rational_unsigned_rounding_mode_triple_gen_var_10().get(gm, config),
        gm.name(),
        limit,
        ...
        &mut [
            (
                "Float::cos_pi_rational_prec_round(Rational, u64, RoundingMode)",
                &mut |(x, prec, rm)| no_out!(Float::cos_pi_rational_prec_round(x, prec, rm)),
            ),
            (
                "Float::cos_pi_rational_prec_round_ref(&Rational, u64, RoundingMode)",
                &mut |(x, prec, rm)| no_out!(Float::cos_pi_rational_prec_round_ref(&x, prec, rm)),
            ),
        ],
    );
}
...
#[allow(clippy::type_repetition_in_bounds)]
fn benchmark_primitive_float_cos_pi<T: PrimitiveFloat>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
{
    run_benchmark(
        &format!("primitive_float_cos_pi({})", T::NAME),
        BenchmarkType::Single,
        primitive_float_gen::<T>().get(gm, config),
        gm.name(),
        limit,
        ...
        &primitive_float_bucketer("x"),
        &mut [("malachite", &mut |x| {
            no_out!(primitive_float_cos_pi(x));
        })],
    );
}

fn demo_float_cos_with_period(gm: GenMode, config: &GenConfig, limit: usize) {
...
fn demo_float_cos_with_period_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u) in float_unsigned_pair_gen_var_2::<u64>()
        .get(gm, config)
        .take(limit)
    {
        let x_old = x.clone();
        let s = x.cos_with_period(u);
        println!(
            "({:#x}).cos_with_period({}) = {:#x}",
            ComparableFloat(x_old),
            u,
...
fn demo_float_cos_with_period_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, u) in float_unsigned_pair_gen_var_2::<u64>()
        .get(gm, config)
        .take(limit)
    {
        println!(
            ...
            x,
            u,
            x.cos_with_period_ref(u)
        );
    }
}

fn demo_float_cos_with_period_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, u) in float_unsigned_pair_gen_var_2::<u64>()
        .get(gm, config)
        .take(limit)
    {
        let x_old = x.clone();
        x.cos_with_period_assign(u);
        println!("x := {x_old}; x.cos_with_period_assign({u}); x = {x}");
    }
}

fn benchmark_float_cos_with_period_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ("Float.cos_with_period(u64)", &mut |(x, u)| {
                no_out!(x.cos_with_period(u));
            }),
            ("(&Float).cos_with_period_ref(u64)", &mut |(x, u)| {
                no_out!(x.cos_with_period_ref(u));
            }),
        ],
    );
}

fn demo_float_cos_pi(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        println!("({}).cos_pi() = {}", x_old, x.cos_pi());
    }
}

fn demo_float_cos_pi_debug(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        let s = x.cos_pi();
        println!(
...
fn demo_float_cos_pi_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in float_gen().get(gm, config).take(limit) {
        println!("(&{}).cos_pi_ref() = {}", x, x.cos_pi_ref());
    }
}

fn demo_float_cos_pi_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for mut x in float_gen().get(gm, config).take(limit) {
        let x_old = x.clone();
        x.cos_pi_assign();
        println!("x := {x_old}; x.cos_pi_assign(); x = {x}");
    }
}

fn benchmark_float_cos_pi_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &float_complexity_bucketer("x"),
        &mut [
            ("Float.cos_pi()", &mut |x| no_out!(x.cos_pi())),
            ("(&Float).cos_pi_ref()", &mut |x| no_out!(x.cos_pi_ref())),
        ],
    );
}
