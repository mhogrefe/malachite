...
fn extract(f: &Float) -> (u128, i16) {
    ...
    let mantissa_rational = Rational::exact_from(f) * Rational::power_of_2(shift);
...
pub(crate) fn generate_l2b_data() {
    ...
    for b in 2u64..=62 {
        let beta = u32::exact_from(b);
        // Column 0: a 23-bit upper bound on log2(b) = log(b)/log(2), via log2 rounded up.
        let col_0 = Float::from(beta).log_base_2_prec_round(23, Ceiling).0;
        ...
        // MPFR's.
        let log2_b_down = Float::from(beta).log_base_2_prec_round(77, Floor).0;
        let col_1 = Float::ONE.div_prec_round(log2_b_down, 77, Ceiling).0;
