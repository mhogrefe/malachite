...
// Returns `Some(log_base(x))` if the `Rational` `x` and the rug `Float` `base` (both positive, not
// 1) are commensurable. Reuses `rational_log_base_rational_rational_base` (itself oracle-checked)
// with the `log_b = -log_{1/b}` reduction for a base below 1. Balloon-safe via the same size bound.
fn rug_log_base_rational_float_base_exact(
    ...
    if br > 1u32 {
        rational_log_base_rational_rational_base(x, &br)
    } else {
        rational_log_base_rational_rational_base(x, &(Rational::ONE / br)).map(|q| -q)
    }
}

// log_base(x) = ln(x) / ln(base) for a `Rational` `x` and a rug `Float` `base`. For inputs outside
// the normal domain (x not positive, or base not finite-positive, or base 1) the result is the IEEE
// quotient of the natural logs -- always exact (0, +-infinity, or NaN) -- computed with rug. For
// the normal domain rug brackets ln(x) (from `x` rounded down and up to a rug `Float`) and ln(base)
// (native), divides the intervals (min/max over the four corners, since a base below 1 makes
...
pub fn rug_log_base_rational_float_base_prec_round(
    ...
    loop {
        ...
        // q_lo <= ln(x) / ln(base) <= q_hi, via the min/max of the four corner quotients.
