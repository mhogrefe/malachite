...
// The secant of a `Rational` fraction of a turn, bracketed from the cosine exactly as
// `sec_with_period_naive` does for a `Float`. `None` means the bracket did not settle it.
pub fn sec_with_period_rational_naive(
    x: &Rational,
    u: u64,
    prec: u64,
    rm: RoundingMode,
