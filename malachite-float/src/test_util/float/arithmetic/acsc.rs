...
// MPFR has no arccosecant, so the oracle is its arcsine of the reciprocal. The reciprocal is taken
// with the input's precision to spare, which absorbs the arcsine's amplification near |x| = 1: an
// error there is magnified by about the square root of the input's precision in bits.
pub fn rug_acsc_prec_round(x: &rug::Float, prec: u64, rm: Round) -> (rug::Float, Ordering) {
    ...
    let o = a.assign_round(t.asin_ref(), rm);
    (a, o)
}

pub fn rug_acsc_prec(x: &rug::Float, prec: u64) -> (rug::Float, Ordering) {
    rug_acsc_prec_round(x, prec, Round::Nearest)
}

pub fn rug_acsc(x: &rug::Float) -> rug::Float {
    rug_acsc_prec_round(x, rug_float_significant_bits(x), Round::Nearest).0
}

// As for the other `Rational` oracles, the input carries its denominator's worth of extra bits, and
// twice its exponent's; `rug_acsc_prec_round` then widens again for the reciprocal.
pub fn rug_acsc_rational_prec_round(x: &Rational, prec: u64, rm: Round) -> (rug::Float, Ordering) {
    ...
    rug_acsc_prec_round(&rx, prec, rm)
}
...
// The `Rational` widening of `rug_acsc_rational_prec_round`, with a period.
pub fn rug_acsc_with_period_rational_prec_round(
    x: &Rational,
    u: u64,
    prec: u64,
    rm: Round,
    ...
    rug_acsc_with_period_prec_round(&rx, u, prec, rm)
}

pub fn rug_acsc_with_period_rational_prec(
    x: &Rational,
    u: u64,
    prec: u64,
) -> (rug::Float, Ordering) {
    rug_acsc_with_period_rational_prec_round(x, u, prec, Round::Nearest)
}
