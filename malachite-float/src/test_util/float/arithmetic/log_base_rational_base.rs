...
// Returns `Some(log_base(x))` if the rug `Float` `x` (finite, positive, not 1) equals `base^m` for
// a rational `m`. This short-circuits the exact case, which the bracketing Ziv loop could never
// resolve when the result is exactly representable (for example `log_3(9) = 2`). It is
// balloon-safe, using the same exponent/size bound as the implementation under test.
