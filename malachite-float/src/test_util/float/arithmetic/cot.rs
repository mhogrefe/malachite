...
// The cotangent of a `Rational` fraction of a turn, bracketed from the sine and cosine exactly as
// `cot_with_period_naive` does for a `Float`. `None` means the bracket did not settle it.
pub fn cot_with_period_rational_naive(
    x: &Rational,
    u: u64,
    prec: u64,
    rm: RoundingMode,
    ...
    let c = Float::cos_with_period_rational_prec_round_ref(x, u, w, Down).0;
    if s == 0u32 || c == 0u32 || !s.is_finite() || !c.is_finite() {
        return None;
    }
    let negative = s.is_sign_negative() != c.is_sign_negative();
