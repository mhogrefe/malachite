...
// MPFR has no arccotangent, so the oracle is its arctangent of the reciprocal, taken with the
// input's precision to spare; the arctangent amplifies nothing, so the spare bits are only caution.
pub fn rug_acot_prec_round(x: &rug::Float, prec: u64, rm: Round) -> (rug::Float, Ordering) {
    ...
    let o = a.assign_round(t.atan_ref(), rm);
    (a, o)
}
