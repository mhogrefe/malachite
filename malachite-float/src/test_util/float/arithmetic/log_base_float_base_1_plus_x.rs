...
use crate::test_util::float::arithmetic::log_base::simplest_dyadic_in;
...
// Returns `Some(log_base(1 + x))` if the rug `Float`s `x` (in (-1, 0) or positive, not 0) and
// `base` (finite, positive, not 1) make `1 + x` and `base` commensurable. Reuses
...
fn rug_log_base_float_base_1_plus_x_exact(
    x: &rug::Float,
    base: &rug::Float,
    prec: u64,
) -> Option<Rational> {
...
// log_base(1 + x) = ln(1 + x) / ln(base) for rug `Float`s `x` and `base`. For inputs outside the
// normal domain (x not finite, x <= -1, x = +-0, or base not finite-positive, or base 1) the result
// is the IEEE quotient of the natural logs -- always exact (0, +-infinity, or NaN) -- computed via
// rug's `ln_1p` and `ln`. For the normal domain rug brackets ln_1p(x) and ln(base) (both native)
// down and up, divides the intervals (min/max over the four corners, since a base below 1 makes
// ln(base) negative), short-circuits the commensurable case, and recovers a straddled dyadic result
// past `exact_threshold`.
pub fn rug_log_base_float_base_1_plus_x_prec_round(
    x: &rug::Float,
    base: &rug::Float,
    prec: u64,
    rm: Round,
    ...
    let normal = x.is_finite() && *x > -1 && *x != 0 && base.is_finite() && *base > 0 && *base != 1;
    if !normal {
        // The result is ln_1p(x)/ln(base) under IEEE: 0, +-infinity, or NaN -- exact.
        let wp = max(target_prec, 64);
        let a = rug::Float::with_val(wp, x.ln_1p_ref());
        let b = rug::Float::with_val(wp, base.ln_ref());
        let mut r = rug::Float::with_val(target_prec, 0);
        r.assign_round(&a / &b, rm);
        return (r, Equal);
    }
    if let Some(q) = rug_log_base_float_base_1_plus_x_exact(x, base, prec) {
    ...
    loop {
        ...
        // b_lo <= ln(base) <= b_hi (same nonzero sign: positive for base > 1, negative for base <
        // 1)
        ...
        // q_lo <= ln(1 + x) / ln(base) <= q_hi, via the min/max of the four corner quotients.
...
pub fn rug_log_base_float_base_1_plus_x_prec(
    x: &rug::Float,
    base: &rug::Float,
    prec: u64,
) -> (rug::Float, Ordering) {
    rug_log_base_float_base_1_plus_x_prec_round(x, base, prec, Round::Nearest)
}

pub fn rug_log_base_float_base_1_plus_x_round(
    x: &rug::Float,
    base: &rug::Float,
    rm: Round,
) -> (rug::Float, Ordering) {
    rug_log_base_float_base_1_plus_x_prec_round(x, base, rug_float_significant_bits(x), rm)
}

pub fn rug_log_base_float_base_1_plus_x(x: &rug::Float, base: &rug::Float) -> rug::Float {
    rug_log_base_float_base_1_plus_x_prec_round(
        x,
        base,
