...
// Returns a partial sum S_k = t - t^3/3! + ... of the sine series for a `Rational` t with |t| < 1,
// which is an upper bound on sin(t) if `upper` is true and a lower bound otherwise, and is within
// |t| 2^-(w + 2) of sin(t). The bound thus tightens with the working precision w, unlike a fixed
// bound such as t - t^3/6 <= sin(t) <= t, which for |t| near 2^-64 leaves a relative gap of about
// 2^-128 that no amount of pi precision can close.
//
...
impl Float {
    ...
    ///
    /// If the cosine is equidistant from two [`Float`]s with the precision of the input, the
    ...
    ///
    /// If you want to use a rounding mode other than `Nearest`, consider using
    /// [`Float::cos_with_period_round`] instead. If you want to specify an output precision,
    /// consider using [`Float::cos_with_period_prec`]. If you want both of these things, consider
    /// using [`Float::cos_with_period_prec_round`].
    ///
    /// # Examples
    /// ```
    /// use malachite_float::Float;
    ///
    /// let c = Float::from_unsigned_prec(1u32, 10).0.cos_with_period(7);
    /// assert_eq!(c.to_string(), "0.62305");
    ///
    /// // a half turn is exactly -1
    /// assert_eq!(
    ///     Float::from(180u32).cos_with_period(360).to_string(),
    ///     "-1.00"
    /// );
    /// ```
    #[inline]
    ...
    ///
    /// If you want to use a rounding mode other than `Nearest`, consider using
    /// [`Float::cos_with_period_round_assign`] instead. If you want to specify an output precision,
    /// consider using [`Float::cos_with_period_prec_assign`]. If you want both of these things,
    /// consider using [`Float::cos_with_period_prec_round_assign`].
    ///
    /// # Examples
    /// ```
    /// use malachite_float::Float;
...
// Computes cos(2 pi q) for a nonzero fraction of a turn q with |q| < 1, rounded to precision `prec`
// with rounding mode `rm`. This is the `Rational` counterpart of
// `cos_with_period_prec_round_normal_ref`, with the same structure: the small-input shortcut, the
...
pub(crate) fn cos_turns_helper(q: &Rational, prec: u64, rm: RoundingMode) -> (Float, Ordering) {
    ...
    loop {
        ...
        if t == 0u32 {
            // Unreachable in practice: such a q is caught by the small-input shortcut above unless
            // `prec` exceeds 2^31 bits.
        ...
        // since w >= 2, |(1 + theta)^3 - 1| <= 4*theta <= 2^(2-w), and |cos(t) - cos(2 pi q)| <=
        // |s| <= 2^(EXP(t) + 2 - w)
        let exp_t = i64::from(t.get_exponent().unwrap());
        let w_i = i64::exact_from(w);
        ...
        // the total error is at most 2^err + ulp(t)/2, bounded by 2^(EXP(t)-w) if err <= EXP(t)-w-1
...
impl Float {
    /// Computes $\cos(2\pi x/u)$, the cosine of a [`Rational`] measured in $u$ths of a turn,
    ...
    ///
    /// Underflow requires $x/u$ within $2^{-2^{30}}$ of an odd multiple of $1/4$ without being one,
    /// which takes a denominator of more than $2^{30}$ bits.
    ///
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round(Rational::ONE, 7, 10, Floor);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round(Rational::ONE, 7, 10, Ceiling);
    /// assert_eq!(c.to_string(), "0.62402");
    /// assert_eq!(o, Greater);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round(Rational::ONE, 7, 10, Nearest);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// // a third of a turn is exact
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round_ref(&Rational::ONE, 7, 10, Floor);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round_ref(&Rational::ONE, 7, 10, Ceiling);
    /// assert_eq!(c.to_string(), "0.62402");
    /// assert_eq!(o, Greater);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_round_ref(&Rational::ONE, 7, 10, Nearest);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// // a third of a turn is exact
    /// let (c, o) = Float::cos_with_period_rational_prec_round_ref(
    ///     &Rational::from_unsigneds(1u8, 3),
    ///     1,
    ///     10,
    ///     Exact,
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec(Rational::ONE, 7, 10);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec(Rational::ONE, 7, 53);
    /// assert_eq!(c.to_string(), "0.62348980185873348");
    /// assert_eq!(o, Less);
    ///
    /// // an eighth of a turn: sqrt(2)/2
    /// let (c, o) = Float::cos_with_period_rational_prec(Rational::from_unsigneds(1u8, 8), 1, 53);
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_ref(&Rational::ONE, 7, 10);
    /// assert_eq!(c.to_string(), "0.62305");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::cos_with_period_rational_prec_ref(&Rational::ONE, 7, 53);
    /// assert_eq!(c.to_string(), "0.62348980185873348");
    /// assert_eq!(o, Less);
    ///
    /// // an eighth of a turn: sqrt(2)/2
    /// let (c, o) =
    ///     Float::cos_with_period_rational_prec_ref(&Rational::from_unsigneds(1u8, 8), 1, 53);
    /// assert_eq!(c.to_string(), "0.70710678118654757");
    /// assert_eq!(o, Greater);
    /// ```
    #[inline]
...
impl Float {
    ...
    /// any [`Float`], whenever this function returns a `NaN` it also returns `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see [`Float::cos_with_period_prec_round`] for
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_prec_round(10, Floor);
    /// assert_eq!(c.to_string(), "0.95020");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_prec_round(10, Ceiling);
    /// assert_eq!(c.to_string(), "0.95117");
    /// assert_eq!(o, Greater);
    ///
    /// // a half-turn is exactly -1
    /// let (c, o) = Float::ONE.cos_pi_prec_round(10, Exact);
    ...
    /// whenever this function returns a `NaN` it also returns `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see [`Float::cos_with_period_prec`] for the
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_prec(10);
    /// assert_eq!(c.to_string(), "0.95117");
    /// assert_eq!(o, Greater);
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_prec(53);
    ...
    /// the rounded cosine is less than, equal to, or greater than the exact cosine. Although `NaN`s
    /// are not comparable to any [`Float`], whenever this function returns a `NaN` it also returns
    /// `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see [`Float::cos_with_period_round`] for the
    /// error bounds, the special and closed-form cases (integers give $\pm1$, half-integers give
    /// $+0.0$, and multiples of $1/3$, $1/4$, $1/5$, $1/6$, and $1/10$ have closed forms), overflow
    /// and underflow, and the complexity, with $u = 2$.
    ///
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_round(Floor);
    /// assert_eq!(c.to_string(), "0.95105651629515342");
    /// assert_eq!(o, Less);
    ///
    /// let (c, o) = Float::from(0.1f64).cos_pi_round(Nearest);
    ...
    ///
    /// If you want to use a rounding mode other than `Nearest`, consider using
    /// [`Float::cos_pi_round`] instead. If you want to specify an output precision, consider using
    /// [`Float::cos_pi_prec`]. If you want both of these things, consider using
    /// [`Float::cos_pi_prec_round`].
    ///
    /// # Examples
    /// ```
    /// use malachite_float::Float;
    ///
    /// let c = Float::from(0.1f64).cos_pi();
    /// assert_eq!(c.to_string(), "0.95105651629515364");
    ///
    /// // an integer is exactly 1 or -1
    /// assert_eq!(Float::from(3u32).cos_pi().to_string(), "-1.0");
    /// ```
    #[inline]
    ...
    /// comparable to any [`Float`], whenever this function sets a `NaN` it also returns `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see
    /// [`Float::cos_with_period_prec_round_assign`] for the error bounds, the special and
    ...
    /// assert_eq!(x.cos_pi_prec_round_assign(10, Floor), Less);
    /// assert_eq!(x.to_string(), "0.95020");
    ///
    /// let mut x = Float::from(0.1f64);
    /// assert_eq!(x.cos_pi_prec_round_assign(10, Ceiling), Greater);
    /// assert_eq!(x.to_string(), "0.95117");
    /// ```
    #[inline]
    ...
    /// [`Float`], whenever this function sets a `NaN` it also returns `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see [`Float::cos_with_period_prec_assign`] for
    /// the error bounds, the special and closed-form cases (integers give $\pm1$, half-integers
    /// give $+0.0$, and multiples of $1/3$, $1/4$, $1/5$, $1/6$, and $1/10$ have closed forms),
    /// overflow and underflow, and the complexity, with $u = 2$.
    ///
    ...
    ///
    /// let mut x = Float::from(0.1f64);
    /// assert_eq!(x.cos_pi_prec_assign(10), Greater);
    /// assert_eq!(x.to_string(), "0.95117");
    /// ```
    #[inline]
    ...
    /// also returns `Equal`.
    ///
    /// This is `cos_with_period` with a period of 2: see [`Float::cos_with_period_round_assign`]
    /// for the error bounds, the special and closed-form cases (integers give $\pm1$, half-integers
    /// give $+0.0$, and multiples of $1/3$, $1/4$, $1/5$, $1/6$, and $1/10$ have closed forms),
    /// overflow and underflow, and the complexity, with $u = 2$.
    ///
    ...
    ///
    /// let mut x = Float::from(0.1f64);
    /// assert_eq!(x.cos_pi_round_assign(Floor), Less);
    /// assert_eq!(x.to_string(), "0.95105651629515342");
    /// ```
    #[inline]
    ...
    ///
    /// If you want to use a rounding mode other than `Nearest`, consider using
    /// [`Float::cos_pi_round_assign`] instead. If you want to specify an output precision, consider
    /// using [`Float::cos_pi_prec_assign`]. If you want both of these things, consider using
    /// [`Float::cos_pi_prec_round_assign`].
    ///
    /// # Examples
    /// ```
    /// use malachite_float::Float;
    ...
    /// let mut x = Float::from(0.1f64);
    /// x.cos_pi_assign();
    /// assert_eq!(x.to_string(), "0.95105651629515364");
    /// ```
    #[inline]
    ...
    /// Computes $\cos(\pi x)$, the cosine of a [`Rational`] measured in half-turns, rounding the
    /// result to the specified precision and with the specified rounding mode and returning the
    /// result as a [`Float`]. The [`Rational`] is taken by value. An [`Ordering`] is also returned,
    /// indicating whether the rounded cosine is less than, equal to, or greater than the exact
    /// cosine.
    ///
    /// This is `cos_with_period_rational` with a period of 2: see
    /// [`Float::cos_with_period_rational_prec_round`] for the error bounds, the special and
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_pi_rational_prec_round(Rational::from_unsigneds(1u8, 7), 10, Floor);
    /// assert_eq!(c.to_string(), "0.90039");
    /// assert_eq!(o, Less);
    ///
    /// // a third of a half-turn is exactly 1/2
    /// let (c, o) = Float::cos_pi_rational_prec_round(Rational::from_unsigneds(1u8, 3), 10, Exact);
    /// assert_eq!(c.to_string(), "0.50000");
    /// assert_eq!(o, Equal);
    /// ```
    #[inline]
    ...
    ///
    /// This is `cos_with_period_rational` with a period of 2: see
    /// [`Float::cos_with_period_rational_prec_round_ref`] for the error bounds, the special and
    /// closed-form cases, overflow and underflow, and the complexity, with $u = 2$.
    ///
    /// # Panics
    /// Panics if `prec` is zero, or if `rm` is `Exact` but the result cannot be represented exactly
    ...
    ///
    /// let (c, o) =
    ///     Float::cos_pi_rational_prec_round_ref(&Rational::from_unsigneds(1u8, 7), 10, Ceiling);
    ...
    ///
    /// This is `cos_with_period_rational` with a period of 2: see
    /// [`Float::cos_with_period_rational_prec`] for the error bounds, the special and closed-form
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_pi_rational_prec(Rational::from_unsigneds(1u8, 7), 53);
    /// assert_eq!(c.to_string(), "0.90096886790241915");
    /// assert_eq!(o, Greater);
    /// ```
    #[inline]
    #[allow(clippy::needless_pass_by_value)]
    ...
    ///
    /// This is `cos_with_period_rational` with a period of 2: see
    /// [`Float::cos_with_period_rational_prec_ref`] for the error bounds, the special and
    /// closed-form cases, overflow and underflow, and the complexity, with $u = 2$.
    ///
    /// # Panics
    /// Panics if `prec` is zero.
    ...
    /// use std::cmp::Ordering::*;
    ///
    /// let (c, o) = Float::cos_pi_rational_prec_ref(&Rational::from_unsigneds(1u8, 7), 53);
    /// assert_eq!(c.to_string(), "0.90096886790241915");
    /// assert_eq!(o, Greater);
    /// ```
    #[inline]
...
///   of $1/4$, the result is exactly $0.0$ (always positive, following IEEE 754-2019's `cosPi`);
///   and if it is an odd multiple of $1/6$ or $1/3$, the result is exactly $1/2$ or $-1/2$.
///
/// Overflow and underflow are not possible: the result lies in $[-1, 1]$, and no [`f32`] or [`f64`]
...
/// Computes $\cos(2\pi x/u)$, the cosine of a [`Rational`] measured in $u$ths of a turn (so that `u
...
///
/// Overflow and underflow are not possible: the result lies in $[-1, 1]$, and a [`Rational`] close
/// enough to an odd quarter turn, without being one, for its cosine to be subnormal would need a
...
/// use malachite_base::num::basic::traits::Zero;
/// use malachite_base::num::float::NiceFloat;
/// use malachite_float::float::arithmetic::cos::primitive_float_cos_with_period_rational;
/// use malachite_q::Rational;
///
/// assert!(primitive_float_cos_with_period_rational::<f64>(&Rational::ZERO, 0).is_nan());
/// assert_eq!(
///     NiceFloat(primitive_float_cos_with_period_rational::<f64>(
///         &Rational::ZERO,
///         360
///     )),
///     NiceFloat(1.0)
/// );
/// // a third of a turn is exactly -1/2
/// assert_eq!(
///     NiceFloat(primitive_float_cos_with_period_rational::<f64>(
///         &Rational::from_unsigneds(1u8, 3),
///         1
...
/// );
/// assert_eq!(
///     NiceFloat(primitive_float_cos_with_period_rational::<f32>(
...
///
/// This is `primitive_float_cos_with_period` with a period of 2: see
/// [`primitive_float_cos_with_period`] for the error bound and the special cases, with $u = 2$.
/// Half-integers give exactly $+0.0$ and integers exactly $\pm1$.
///
/// # Worst-case complexity
/// Constant time and additional memory.
///
...
/// Computes $\cos(\pi x)$, the cosine of a [`Rational`] measured in half-turns, returning the
/// result as a primitive float.
///
/// This is `primitive_float_cos_with_period_rational` with a period of 2: see
/// [`primitive_float_cos_with_period_rational`] for the error bound, the special cases, and the
/// complexity, with $u = 2$.
///
/// # Worst-case complexity
/// $T(m) = O(m (\log m)^2 \log\log m)$
...
/// ```
/// use malachite_base::num::float::NiceFloat;
/// use malachite_float::float::arithmetic::cos::primitive_float_cos_pi_rational;
/// use malachite_q::Rational;
///
/// // a third of a half-turn is exactly 1/2
/// assert_eq!(
///     NiceFloat(primitive_float_cos_pi_rational::<f64>(
///         &Rational::from_unsigneds(1u8, 3)
///     )),
///     NiceFloat(0.5)
/// );
