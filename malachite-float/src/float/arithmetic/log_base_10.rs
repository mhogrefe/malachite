...
fn log_base_10_prec_round_normal(x: &Float, prec: u64, rm: RoundingMode) -> (Float, Ordering) {
    ...
    match sliver_of_one(x) {
        SliverOfOne::Representable(d) => return d.log_base_10_1_plus_x_prec_round(prec, rm),
        SliverOfOne::Underflow => {
            return Float::log_base_10_rational_prec_round(Rational::exact_from(x), prec, rm);
        }
        SliverOfOne::No => {}
    }
...
impl Float {
    ...
    /// assert_eq!(log.to_string(), "-2.0000"); // log_10(1/100) = -2
    /// assert_eq!(o, Equal);
    /// ```
...
impl LogBase10 for Float {
    ...
    /// use malachite_float::Float;
    ///
    /// assert_eq!(Float::from(1000).log_base_10().to_string(), "3.000");
    /// assert_eq!(Float::from(100).log_base_10().to_string(), "2.00");
    /// ```
    #[inline]
...
/// use malachite_float::float::arithmetic::log_base_10::primitive_float_log_base_10;
///
...
/// use malachite_base::num::float::NiceFloat;
/// use malachite_float::float::arithmetic::log_base_10::primitive_float_log_base_10_rational;
/// use malachite_q::Rational;
///
/// assert_eq!(
