...
impl ToTypst for Float {
    ...
    ///
    /// assert_eq!(Float::NAN.to_typst_string(), r#""NaN""#);
    /// assert_eq!(Float::INFINITY.to_typst_string(), "infinity");
    /// assert_eq!(Float::NEGATIVE_INFINITY.to_typst_string(), "-infinity");
    /// assert_eq!(Float::ZERO.to_typst_string(), "0.0");
    /// assert_eq!(Float::NEGATIVE_ZERO.to_typst_string(), "-0.0");
    /// assert_eq!(Float::ONE.to_typst_string(), "1.0");
    /// assert_eq!(Float::from(1.5).to_typst_string(), "1.5");
    /// assert_eq!(
    ///     Float::power_of_2(100u64).to_typst_string(),
    ///     "1.3 times 10^(30)"
    /// );
    /// assert_eq!(
    ///     Float::power_of_2(-100i64).to_typst_string(),
    ///     "7.9 times 10^(-31)"
    /// );
    /// ```
    ///
    ...
    /// | `Float::ONE`                | `1.0`               |
    /// | `Float::power_of_2(100u64)` | `1.3 times 10^(30)` |
...
impl ToTypst for ComparableFloat {
    /// Writes a [`ComparableFloat`] as a Typst math-mode fragment.
    ///
    ...
    ///
    /// # Worst-case complexity
    /// Same as the time and additional memory complexity of `fmt_typst` for [`Float`].
    ///
    /// # Examples
    /// ```
    ...
    ///
    /// assert_eq!(ComparableFloat(Float::ONE).to_typst_string(), "1.0");
    /// ```
    #[inline]
    fn fmt_typst(&self, f: &mut Formatter) -> Result {
        self.0.fmt_typst(f)
...
impl ToTypst for ComparableFloatRef<'_> {
    /// Writes a [`ComparableFloatRef`] as a Typst math-mode fragment.
    ///
    /// The fragment is the wrapped [`Float`]'s own: the wrapper exists to give an equality and an
    /// ordering that tell more [`Float`]s apart than the usual ones do, and does not change what
    /// the value is.
    ...
    ///
    /// let x = Float::ONE;
    /// assert_eq!(ComparableFloatRef(&x).to_typst_string(), "1.0");
    /// ```
    #[inline]
    fn fmt_typst(&self, f: &mut Formatter) -> Result {
        self.0.fmt_typst(f)
