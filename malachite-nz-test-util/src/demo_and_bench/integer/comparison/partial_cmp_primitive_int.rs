...
use malachite_nz_test_util::integer::comparison::partial_cmp_primitive_int::*;
use num::BigInt;
...
{
    for (n, u) in integer_unsigned_pair_gen::<T>()
        .get(gm, &config)
        .take(limit)
    {
...
fn demo_integer_partial_cmp_signed<T: PrimitiveSigned>(gm: GenMode, config: GenConfig, limit: usize)
where
    Integer: PartialOrd<T>,
{
