...
fn demo_limbs_xor_limb_in_place(gm: GenMode, config: GenConfig, limit: usize) {
        ...
        .take(limit)
    {
        ...
        println!(
            "xs := {:?}; limbs_xor_limb_in_place(&mut xs, {}); xs = {:?}",
            xs_old, y, xs
        );
    }
}
...
fn demo_limbs_xor_same_length_to_out(gm: GenMode, config: GenConfig, limit: usize) {
        ...
        .take(limit)
    {
        ...
        println!(
            "out := {:?}; limbs_xor_same_length_to_out(&mut out, {:?}, {:?}); out = {:?}",
            out_old, xs, ys, out
        );
    }
}
...
fn benchmark_natural_xor_algorithms(gm: GenMode, config: GenConfig, limit: usize, file_name: &str) {
    run_benchmark(
        "Natural ^ Natural",
