...
pub fn demo_natural_special_random_natural_up_to_bits(gm: NoSpecialGenerationMode, limit: usize) {
...
pub fn benchmark_natural_special_random_natural_up_to_bits(
    gm: NoSpecialGenerationMode,
    limit: usize,
    file_name: &str,
) {
    println!(
        "benchmarking {} Natural::special_random_natural_up_to_bits(&mut Rng, u64)",
        gm.name()
    );
    let mut rng = IsaacRng::from_seed(&EXAMPLE_SEED);
    benchmark_1(BenchmarkOptions1 {
        xs: small_u64s(gm),
        function_f: &mut (|bits| special_random_natural_up_to_bits(&mut rng, bits)),
        x_cons: &(|&bits| bits),
        x_param: &(|&bits| bits.significant_bits() as usize),
        limit,
        f_name: "malachite",
        title: "Natural::special\\\\_random\\\\_natural\\\\_up\\\\_to\\\\_bits(\\\\&mut Rng, u64)",
