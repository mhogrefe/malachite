...
pub fn benchmark_natural_random_natural_with_bits(
    ...
    println!(
        "benchmarking {} Natural::random_natural_with_bits(&mut Rng, u64)",
        gm.name()
    );
    let mut rng = IsaacRng::from_seed(&EXAMPLE_SEED);
    benchmark_1(BenchmarkOptions1 {
        xs: small_u64s(gm),
        function_f: &mut (|bits| random_natural_with_bits(&mut rng, bits)),
        x_cons: &(|&bits| bits),
        x_param: &(|&bits| bits.significant_bits() as usize),
        limit,
        f_name: "malachite",
        title: "Natural::random\\\\_natural\\\\_with\\\\_bits(\\\\&mut Rng, u64)",
