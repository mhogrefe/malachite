...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    register_demo!(registry, demo_limbs_round_to_multiple_of_power_of_2_down);
    register_demo!(registry, demo_limbs_round_to_multiple_of_power_of_2_up);
    register_demo!(registry, demo_limbs_round_to_multiple_of_power_of_2_nearest);
    register_demo!(registry, demo_limbs_round_to_multiple_of_power_of_2);
    register_demo!(
        registry,
    ...
    register_demo!(
        registry,
        demo_limbs_round_to_multiple_of_power_of_2_in_place
    );
    register_demo!(
        registry,
        demo_natural_round_to_multiple_of_power_of_2_assign
    );
    register_demo!(registry, demo_natural_round_to_multiple_of_power_of_2);
    register_demo!(registry, demo_natural_round_to_multiple_of_power_of_2_ref);
    ...
    register_bench!(
        registry,
        Small,
        benchmark_limbs_round_to_multiple_of_power_of_2_up
    );
    register_bench!(
        registry,
        Small,
    ...
    register_bench!(
        registry,
        Small,
        benchmark_limbs_round_to_multiple_of_power_of_2
    );
    register_bench!(
        registry,
        Small,
...
fn demo_limbs_round_to_multiple_of_power_of_2_down(gm: GenerationMode, limit: usize) {
    for (limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned(gm).take(limit) {
        println!(
            "limbs_round_to_multiple_of_power_of_2_down({:?}, {}) = {:?}",
            limbs,
            pow,
            limbs_round_to_multiple_of_power_of_2_down(&limbs, pow)
        );
    }
}

fn demo_limbs_round_to_multiple_of_power_of_2_up(gm: GenerationMode, limit: usize) {
    for (limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned_var_1(gm).take(limit) {
        println!(
            "limbs_round_to_multiple_of_power_of_2_up({:?}, {}) = {:?}",
            limbs,
            pow,
            limbs_round_to_multiple_of_power_of_2_up(&limbs, pow)
        );
    }
}

fn demo_limbs_round_to_multiple_of_power_of_2_nearest(gm: GenerationMode, limit: usize) {
    for (limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned(gm).take(limit) {
        println!(
            "limbs_round_to_multiple_of_power_of_2_nearest({:?}, {}) = {:?}",
            limbs,
            pow,
            limbs_round_to_multiple_of_power_of_2_nearest(&limbs, pow)
        );
    }
}

fn demo_limbs_round_to_multiple_of_power_of_2(gm: GenerationMode, limit: usize) {
    for (limbs, pow, rm) in
...
fn demo_limbs_round_to_multiple_of_power_of_2_down_in_place(gm: GenerationMode, limit: usize) {
    for (mut limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned(gm).take(limit) {
        ...
        println!(
            "limbs := {:?}; limbs_round_to_multiple_of_power_of_2_down_in_place(&mut limbs, {}); \
            limbs = {:?}",
            limbs_old, pow, limbs
        );
    }
...
fn demo_limbs_round_to_multiple_of_power_of_2_up_in_place(gm: GenerationMode, limit: usize) {
    for (mut limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned_var_1(gm).take(limit) {
        ...
        println!(
            "limbs := {:?}; limbs_round_to_multiple_of_power_of_2_up_in_place(&mut limbs, {}); \
            limbs = {:?}",
            limbs_old, pow, limbs
        );
    }
...
fn demo_limbs_round_to_multiple_of_power_of_2_nearest_in_place(gm: GenerationMode, limit: usize) {
    for (mut limbs, pow) in pairs_of_unsigned_vec_and_small_unsigned(gm).take(limit) {
        let limbs_old = limbs.clone();
        limbs_round_to_multiple_of_power_of_2_nearest_in_place(&mut limbs, pow);
        println!(
            "limbs := {:?}; \
            limbs_round_to_multiple_of_power_of_2_nearest_in_place(&mut limbs, {}); limbs = {:?}",
            limbs_old, pow, limbs
        );
    }
}

fn demo_limbs_round_to_multiple_of_power_of_2_in_place(gm: GenerationMode, limit: usize) {
    for (mut limbs, pow, rm) in
        triples_of_unsigned_vec_small_unsigned_and_rounding_mode_var_1(gm).take(limit)
    {
        let limbs_old = limbs.clone();
        let success = limbs_round_to_multiple_of_power_of_2_in_place(&mut limbs, pow, rm);
        println!(
            "limbs := {:?}; \
            limbs_round_to_multiple_of_power_of_2_in_place(&mut limbs, {}, {}) = {}; \
            limbs = {:?}",
...
fn demo_natural_round_to_multiple_of_power_of_2_assign(gm: GenerationMode, limit: usize) {
    for (mut n, pow, rm) in
...
fn demo_natural_round_to_multiple_of_power_of_2(gm: GenerationMode, limit: usize) {
...
fn demo_natural_round_to_multiple_of_power_of_2_ref(gm: GenerationMode, limit: usize) {
    for (n, pow, rm) in triples_of_natural_small_unsigned_and_rounding_mode_var_1(gm).take(limit) {
        println!(
            "(&{}).round_to_multiple_of_power_of_2({}, {}) = {}",
            n,
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_down(
    ...
    run_benchmark_old(
        ...
        &mut [(
            "Malachite",
            &mut (|(limbs, pow)| no_out!(limbs_round_to_multiple_of_power_of_2_down(&limbs, pow))),
        )],
    );
}
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_up(
    ...
    run_benchmark_old(
        ...
        &mut [(
            "Malachite",
            &mut (|(limbs, pow)| no_out!(limbs_round_to_multiple_of_power_of_2_up(&limbs, pow))),
        )],
    );
}
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_nearest(
    ...
    run_benchmark_old(
        ...
        &mut [(
            "Malachite",
            &mut (|(limbs, pow)| {
                no_out!(limbs_round_to_multiple_of_power_of_2_nearest(&limbs, pow))
            }),
        )],
    );
}
...
fn benchmark_limbs_round_to_multiple_of_power_of_2(
    ...
    run_benchmark_old(
        ...
        &mut [(
            "Malachite",
            &mut (|(limbs, pow, rm)| {
                no_out!(limbs_round_to_multiple_of_power_of_2(&limbs, pow, rm))
            }),
        )],
    );
}
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_down_in_place(
    ...
    run_benchmark_old(
        "limbs_round_to_multiple_of_power_of_2_down_in_place(&mut Vec<Limb>, u64)",
        BenchmarkType::Single,
        pairs_of_unsigned_vec_and_small_unsigned(gm),
        gm.name(),
        limit,
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_up_in_place(
    ...
    run_benchmark_old(
        "limbs_round_to_multiple_of_power_of_2_up_in_place(&mut Vec<Limb>, u64)",
        BenchmarkType::Single,
        pairs_of_unsigned_vec_and_small_unsigned_var_1(gm),
        gm.name(),
        limit,
...
fn benchmark_limbs_round_to_multiple_of_power_of_2_nearest_in_place(
    ...
    run_benchmark_old(
        "limbs_round_to_multiple_of_power_of_2_nearest_in_place(&mut Vec<Limb>, u64)",
        BenchmarkType::Single,
        pairs_of_unsigned_vec_and_small_unsigned(gm),
        gm.name(),
        limit,
    ...
    run_benchmark_old(
        ...
        &mut [
            ...
            (
                ...
                &mut (|(x, y, rm)| no_out!(x.round_to_multiple(Natural::power_of_2(y), rm))),
            ),
        ],
    );
}
