...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    ...
    register_demo!(registry, demo_limbs_mod_power_of_two_pow);
    ...
    register_bench!(registry, Small, benchmark_limbs_mod_power_of_two_pow);
    register_bench!(
        registry,
        Large,
...
fn demo_limbs_mod_power_of_two_pow(gm: GenerationMode, limit: usize) {
    for (xs, es, pow) in
        triples_of_unsigned_vec_unsigned_vec_and_unsigned_var_17(gm.with_scale(32)).take(limit)
    {
        ...
        println!(
            "xs := {:?}; limbs_mod_power_of_two_pow(&mut xs, {:?}, {}); xs = {:?}",
            xs_old, es, pow, xs
        );
    }
}
...
fn benchmark_limbs_mod_power_of_two_pow(gm: GenerationMode, limit: usize, file_name: &str) {
    run_benchmark_old(
        "limbs_mod_power_of_two_pow(&mut [Limb], &[Limb], Limb)",
        BenchmarkType::Single,
        ...
        &mut [(
            "Malachite",
            &mut (|(mut xs, es, pow)| limbs_mod_power_of_two_pow(&mut xs, &es, pow)),
        )],
    );
}
