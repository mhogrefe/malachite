...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    ...
    register_demo!(registry, demo_natural_and_u32_ref);
    register_demo!(registry, demo_u32_and_natural);
    register_demo!(registry, demo_u32_and_natural_ref);
    register_demo!(registry, demo_u32_and_assign_natural);
    register_demo!(registry, demo_u32_and_assign_natural_ref);
    ...
    register_bench!(
        registry,
        Large,
        benchmark_natural_and_u32_evaluation_strategy
    );
    ...
    register_bench!(
        registry,
        Large,
        benchmark_u32_and_natural_evaluation_strategy
    );
    register_bench!(
        registry,
        Large,
        benchmark_u32_and_assign_natural_evaluation_strategy
    );
}
...
fn demo_natural_and_u32_ref(gm: GenerationMode, limit: usize) {
    for (n, u) in pairs_of_natural_and_unsigned::<u32>(gm).take(limit) {
        println!("&{} & {} = {}", n, u, &n & u);
    }
}
...
fn demo_u32_and_natural_ref(gm: GenerationMode, limit: usize) {
    for (u, n) in pairs_of_unsigned_and_natural::<u32>(gm).take(limit) {
        println!("{} + &{} = {}", u, n, u & &n);
    }
}

fn demo_u32_and_assign_natural(gm: GenerationMode, limit: usize) {
...
fn demo_u32_and_assign_natural_ref(gm: GenerationMode, limit: usize) {
    for (mut u, n) in pairs_of_unsigned_and_natural::<u32>(gm).take(limit) {
        let u_old = u;
        u &= &n;
...
fn benchmark_natural_and_u32_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_u32_and_natural_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_u32_and_assign_natural_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
    m_run_benchmark(
        ...
        &mut [
            ...
            ("u32 &= &Natural", &mut (|(mut x, y)| no_out!(x &= &y))),
        ],
    );
}
