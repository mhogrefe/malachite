...
use natural::logic::xor::{natural_xor_alt_1, natural_xor_alt_2};
use num::BigUint;

pub fn natural_xor_limb_alt_1(n: &Natural, i: Limb) -> Natural {
    natural_xor_alt_1(n, &Natural::from(i))
}

pub fn natural_xor_limb_alt_2(n: &Natural, i: Limb) -> Natural {
    natural_xor_alt_2(n, &Natural::from(i))
}
...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    ...
    register_demo!(registry, demo_natural_xor_assign_limb);
    register_demo!(registry, demo_natural_xor_limb);
    register_demo!(registry, demo_natural_xor_limb_ref);
    register_demo!(registry, demo_limb_xor_natural);
    register_demo!(registry, demo_limb_xor_natural_ref);
    ...
    register_bench!(
        registry,
        Large,
        benchmark_natural_xor_limb_library_comparison
    );
    register_bench!(
        registry,
        Large,
        benchmark_natural_xor_limb_evaluation_strategy
    );
    register_bench!(registry, Large, benchmark_natural_xor_limb_algorithms);
    #[cfg(feature = "32_bit_limbs")]
    register_bench!(
        registry,
        Large,
        benchmark_limb_xor_natural_library_comparison
    );
    register_bench!(
        registry,
        Large,
        benchmark_limb_xor_natural_evaluation_strategy
    );
}
...
fn demo_limbs_xor_limb_to_out(gm: GenerationMode, limit: usize) {
        ...
        triples_of_unsigned_vec_unsigned_vec_and_unsigned_var_2(gm).take(limit)
    {
        ...
        println!(
            "out_limbs := {:?}; limbs_xor_limb_to_out(&mut out_limbs, {:?}, {}); out_limbs = {:?}",
...
fn demo_natural_xor_assign_limb(gm: GenerationMode, limit: usize) {
...
fn demo_natural_xor_limb(gm: GenerationMode, limit: usize) {
...
fn demo_natural_xor_limb_ref(gm: GenerationMode, limit: usize) {
    for (n, u) in pairs_of_natural_and_unsigned::<Limb>(gm).take(limit) {
        println!("&{} ^ {} = {}", n, u, &n ^ u);
    }
}

fn demo_limb_xor_natural(gm: GenerationMode, limit: usize) {
...
fn demo_limb_xor_natural_ref(gm: GenerationMode, limit: usize) {
    for (u, n) in pairs_of_unsigned_and_natural::<Limb>(gm).take(limit) {
        println!("{} ^ &{} = {}", u, n, u ^ &n);
    }
}
    ...
    m_run_benchmark(
        ...
        &mut [(
            ...
            &mut (|(mut out_limbs, in_limbs, limb)| {
                no_out!(limbs_xor_limb_to_out(&mut out_limbs, &in_limbs, limb))
            }),
        )],
    );
}
...
#[cfg(feature = "32_bit_limbs")]
fn benchmark_natural_xor_assign_limb_library_comparison(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_natural_xor_limb_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_natural_xor_limb_algorithms(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "Natural ^ Limb",
        ...
        &mut [
            ...
            (
                ...
                &mut (|(x, y)| no_out!(natural_xor_limb_alt_1(&x, y))),
            ),
            (
                "using limbs explicitly",
                &mut (|(x, y)| no_out!(natural_xor_limb_alt_2(&x, y))),
            ),
        ],
    );
}
...
#[cfg(feature = "32_bit_limbs")]
fn benchmark_limb_xor_natural_library_comparison(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_limb_xor_natural_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
