...
pub fn natural_xor_u32_alt_1(n: &Natural, i: u32) -> Natural {
    natural_xor_alt_1(n, &Natural::from(i))
}

pub fn natural_xor_u32_alt_2(n: &Natural, i: u32) -> Natural {
...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    ...
    register_bench!(
        registry,
        Large,
        benchmark_natural_xor_assign_u32_library_comparison
    );
    register_bench!(
        registry,
        Large,
        benchmark_natural_xor_u32_library_comparison
    );
    register_bench!(
        registry,
        Large,
...
fn demo_natural_xor_assign_u32(gm: GenerationMode, limit: usize) {
...
fn demo_natural_xor_u32(gm: GenerationMode, limit: usize) {
...
fn demo_natural_xor_u32_ref(gm: GenerationMode, limit: usize) {
    for (n, u) in pairs_of_natural_and_unsigned::<u32>(gm).take(limit) {
        println!("&{} ^ {} = {}", n, u, &n ^ u);
    }
}
...
fn benchmark_limbs_xor_limb_in_place(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "limbs_xor_limb_in_place(&mut [u32], u32)",
        BenchmarkType::Single,
        pairs_of_nonempty_unsigned_vec_and_unsigned(gm),
        gm.name(),
        limit,
...
fn benchmark_natural_xor_assign_u32_library_comparison(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_natural_xor_u32_library_comparison(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "Natural ^ u32",
        BenchmarkType::LibraryComparison,
...
fn benchmark_natural_xor_u32_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
    ...
    m_run_benchmark(
        ...
        &mut [
            ...
            (
                ...
                &mut (|(x, y)| no_out!(natural_xor_u32_alt_1(&x, y))),
            ),
            (
                "using limbs explicitly",
                &mut (|(x, y)| no_out!(natural_xor_u32_alt_2(&x, y))),
            ),
        ],
    );
}
