...
pub fn benchmark_integer_shl_round_assign_i32(gm: GenerationMode, limit: usize, file_name: &str) {
    ...
    benchmark_1(BenchmarkOptions1 {
        xs: triples_of_integer_small_i32_and_rounding_mode_var_1(gm),
        function_f: &mut (|(mut n, i, rm): (Integer, i32, RoundingMode)| n.shl_round_assign(i, rm)),
        x_cons: &(|p| p.clone()),
        x_param: &(|&(_, index, _)| index as usize),
        limit,
        f_name: "malachite",
        title: "Integer.shl_round_assign(i32, RoundingMode)",
        x_axis_label: "other",
        y_axis_label: "time (ns)",
        file_name: &format!("benchmarks/{}", file_name),
    });
...
pub fn benchmark_integer_shl_round_i32(gm: GenerationMode, limit: usize, file_name: &str) {
    ...
    benchmark_1(BenchmarkOptions1 {
        xs: triples_of_integer_small_i32_and_rounding_mode_var_1(gm),
        function_f: &mut (|(n, i, rm): (Integer, i32, RoundingMode)| n.shl_round(i, rm)),
        x_cons: &(|p| p.clone()),
        x_param: &(|&(_, index, _)| index as usize),
        limit,
        f_name: "malachite",
        title: "Integer.shl_round(i32, RoundingMode)",
        x_axis_label: "other",
        y_axis_label: "time (ns)",
        file_name: &format!("benchmarks/{}", file_name),
    });
