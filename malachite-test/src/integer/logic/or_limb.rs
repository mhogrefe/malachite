...
pub fn integer_or_limb_alt_2(n: &Integer, i: Limb) -> Integer {
...
pub(crate) fn register(registry: &mut DemoBenchRegistry) {
    ...
    register_demo!(registry, demo_integer_or_assign_limb);
    register_demo!(registry, demo_integer_or_limb);
    register_demo!(registry, demo_integer_or_limb_ref);
    register_demo!(registry, demo_limb_or_integer);
    register_demo!(registry, demo_limb_or_integer_ref);
    ...
    register_bench!(
        registry,
        Large,
        benchmark_integer_or_limb_library_comparison
    );
    register_bench!(
        registry,
        Large,
        benchmark_integer_or_limb_evaluation_strategy
    );
    register_bench!(registry, Large, benchmark_integer_or_limb_algorithms);
    #[cfg(feature = "32_bit_limbs")]
    register_bench!(
        registry,
        Large,
        benchmark_limb_or_integer_library_comparison
    );
    register_bench!(
        registry,
        Large,
        benchmark_limb_or_integer_evaluation_strategy
    );
}
...
fn demo_integer_or_assign_limb(gm: GenerationMode, limit: usize) {
...
fn demo_integer_or_limb(gm: GenerationMode, limit: usize) {
...
fn demo_integer_or_limb_ref(gm: GenerationMode, limit: usize) {
    for (n, u) in pairs_of_integer_and_unsigned::<Limb>(gm).take(limit) {
        println!("&{} | {} = {}", n, u, &n | u);
    }
}

fn demo_limb_or_integer(gm: GenerationMode, limit: usize) {
...
fn demo_limb_or_integer_ref(gm: GenerationMode, limit: usize) {
    for (u, n) in pairs_of_unsigned_and_integer::<Limb>(gm).take(limit) {
        println!("{} | &{} = {}", u, n, u | &n);
    }
}
...
#[cfg(feature = "32_bit_limbs")]
fn benchmark_integer_or_assign_limb_library_comparison(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
#[cfg(feature = "32_bit_limbs")]
fn benchmark_integer_or_limb_library_comparison(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "Integer | Limb",
        BenchmarkType::LibraryComparison,
        rm_pairs_of_integer_and_unsigned::<Limb>(gm),
...
fn benchmark_integer_or_limb_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
...
fn benchmark_integer_or_limb_algorithms(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "Integer | Limb",
        ...
        &mut [
            ...
            (
                ...
                &mut (|(x, y)| no_out!(integer_or_limb_alt_1(&x, y))),
            ),
            (
                "using limbs explicitly",
                &mut (|(x, y)| no_out!(integer_or_limb_alt_2(&x, y))),
            ),
        ],
    );
}
...
#[cfg(feature = "32_bit_limbs")]
fn benchmark_limb_or_integer_library_comparison(gm: GenerationMode, limit: usize, file_name: &str) {
    m_run_benchmark(
        "Limb | Integer",
        BenchmarkType::LibraryComparison,
...
fn benchmark_limb_or_integer_evaluation_strategy(
    gm: GenerationMode,
    limit: usize,
    file_name: &str,
) {
