...
use malachite_test::inputs::base::triples_of_unsigned_vec_var_32;
...
pub(crate) fn tune() -> Vec<String> {
    let result = compare_two(
        ...
        triples_of_unsigned_vec_var_32(GenerationMode::Random(1024)),
        10000,
        &(|&(_, ref xs, _)| xs.len()),
    );
    ...
    if let ComparisonResult::SecondBetterAbove(threshold) = result {
        lines.push(format!(
            "pub const MUL_TOOM6H_THRESHOLD: usize = {};",
            threshold
        ));
    } else {
        panic!("Unexpected Toom6h tuning result: {:?}", result);
    }
    lines
}
