...
use malachite_nz::natural::arithmetic::mod_op::{
    _limbs_mod_limb_any_leading_zeros, _limbs_mod_limb_at_least_1_leading_zero,
};
use malachite_nz::platform::Limb;

use malachite_test::common::GenerationMode;
use malachite_test::inputs::base::pairs_of_nonempty_unsigned_vec_and_positive_unsigned_var_1;
...
pub(crate) fn tune() -> Vec<String> {
    ...
    if let ComparisonResult::SecondBetterAbove(threshold) = result {
        lines.push(format!(
            "pub const MOD_1_1_TO_MOD_1_2_THRESHOLD: usize = {};",
            threshold
        ));
    } else {
        panic!(
            "Unexpected mod limb at least 1 leading zero tuning result: {:?}",
            result
        );
    }
    lines
