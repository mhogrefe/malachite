...
fn mod_power_of_two_square_properties_helper<T: PrimitiveUnsigned + Rand + SampleRange>() {
    test_properties_no_special(pairs_of_unsigned_and_small_u64_var_2::<T>, |&(x, pow)| {
        assert!(x.mod_power_of_two_is_reduced(pow));
        let square = x.mod_power_of_two_square(pow);
        assert!(square.mod_power_of_two_is_reduced(pow));

        let mut x_alt = x;
        x_alt.mod_power_of_two_square_assign(pow);
        ...
        assert_eq!(
            x.mod_power_of_two_neg(pow).mod_power_of_two_square(pow),
            square
        );
    });

    test_properties_no_special(unsigneds_var_3::<T>, |&pow| {
        assert_eq!(T::ZERO.mod_power_of_two_square(pow), T::ZERO);
        if pow != 0 {
            assert_eq!(T::ONE.mod_power_of_two_square(pow), T::ONE);
        }
    });
...
fn mod_power_of_two_square_properties() {
    mod_power_of_two_square_properties_helper::<u8>();
    mod_power_of_two_square_properties_helper::<u16>();
    mod_power_of_two_square_properties_helper::<u32>();
    mod_power_of_two_square_properties_helper::<u64>();
    mod_power_of_two_square_properties_helper::<usize>();
}
