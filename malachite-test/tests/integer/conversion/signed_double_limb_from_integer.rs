...
fn signed_double_limb_wrapping_from_integer_properties() {
    test_properties(integers, |x| {
        ...
        assert_eq!(
            result,
            SignedDoubleLimb::wrapping_from(DoubleLimb::exact_from(
                &x.mod_power_of_two(DoubleLimb::WIDTH.into())
            ))
        );
...
fn signed_double_limb_saturating_from_integer_properties() {
    test_properties(integers, |x| {
        ...
        assert_eq!(result == *x, SignedDoubleLimb::convertible_from(x));
    });
}
    ...
    test_properties(integers, |x| {
        ...
        assert_eq!(
            ...
            (
                ...
                !SignedDoubleLimb::convertible_from(x)
            )
        );
    });
}
