...
use malachite_test::inputs::base::{nonzero_signeds, pairs_of_i32_and_nonzero_i32_var_1};
use malachite_test::inputs::integer::{
    integers, pairs_of_i32_and_nonzero_integer_var_2, pairs_of_integer_and_nonzero_i32_var_1,
};
use malachite_test::integer::arithmetic::div_exact_i32::rug_div_exact_i32;
use rug;
use std::str::FromStr;

#[test]
fn test_div_exact_i32() {
    let test = |i, j: i32, quotient| {
        ...
        let q = rug_div_exact_i32(rug::Integer::from_str(i).unwrap(), j);
        assert_eq!(q.to_string(), quotient);
    };
    test("0", 1, "0");
    test("0", 123, "0");
...
fn div_exact_assign_i32_fail() {
    let mut n = Integer::from(10i32);
    n.div_exact_assign(0i32);
}

#[test]
#[should_panic(expected = "division by zero")]
fn div_exact_i32_fail() {
    Integer::from(10i32).div_exact(0i32);
}

#[test]
#[should_panic(expected = "division by zero")]
fn div_exact_i32_ref_fail() {
    (&Integer::from(10i32)).div_exact(0i32);
}

#[test]
fn test_i32_div_exact_integer() {
    let test = |i: i32, j, quotient| {
...
#[test]
#[should_panic(expected = "division by zero")]
fn i32_div_exact_integer_fail() {
    10i32.div_exact(Integer::ZERO);
}

#[test]
#[should_panic(expected = "division by zero")]
fn i32_div_exact_integer_ref_fail() {
    10i32.div_exact(&Integer::ZERO);
}

fn div_exact_i32_properties_helper(n: &Integer, i: i32) {
    ...
    assert_eq!(
        rug_integer_to_integer(&rug_div_exact_i32(integer_to_rug_integer(n), i)),
        quotient
    );
...
#[test]
fn div_exact_i32_properties() {
    test_properties(
        pairs_of_integer_and_nonzero_i32_var_1,
        |&(ref n, i): &(Integer, i32)| {
            div_exact_i32_properties_helper(n, i);
        },
    );

    test_properties(
        pairs_of_i32_and_nonzero_integer_var_2,
        |&(i, ref n): &(i32, Integer)| {
    ...
    test_properties(integers, |n| {
        assert_eq!(n.div_exact(1i32), *n);
        assert_eq!(n.div_exact(-1i32), -n);
    });

    test_properties(nonzero_signeds, |&i: &i32| {
        assert_eq!(Integer::ZERO.div_exact(i), 0);
        assert_eq!(i.div_exact(Integer::ONE), i);
        assert_eq!(i.div_exact(Integer::NEGATIVE_ONE), -Integer::from(i));
        assert_eq!(i.div_exact(Integer::from(i)), 1);
        assert_eq!(Integer::from(i).div_exact(i), 1);
        assert_eq!(i.div_exact(-Integer::from(i)), -1);
        assert_eq!((-Integer::from(i)).div_exact(i), -1);
    });

    test_properties(pairs_of_i32_and_nonzero_i32_var_1, |&(x, y)| {
