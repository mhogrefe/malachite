...
use malachite_test::inputs::base::{
    pairs_of_limb_vec_and_positive_limb_var_1, pairs_of_signeds, signeds,
};
...
fn test_or_signed_limb() {
    let test = |u, v: SignedLimb, out| {
        ...
        assert_eq!(
            integer_or_signed_limb_alt_1(&Integer::from_str(u).unwrap(), v).to_string(),
            out
        );
        assert_eq!(
            integer_or_signed_limb_alt_2(&Integer::from_str(u).unwrap(), v).to_string(),
            out
        );
...
fn or_signed_limb_properties() {
    test_properties(
        ...
        |&(ref n, i): &(Integer, SignedLimb)| {
            ...
            assert_eq!(integer_or_signed_limb_alt_1(&n, i), result);
            assert_eq!(integer_or_signed_limb_alt_2(&n, i), result);
    ...
    test_properties(integers, |n| {
        ...
        assert_eq!(n | -1 as SignedLimb, -1 as SignedLimb);
        assert_eq!(-1 as SignedLimb | n, -1 as SignedLimb);
    });
    ...
    test_properties(signeds, |&i: &SignedLimb| {
        ...
        assert_eq!(i | &Integer::NEGATIVE_ONE, -1 as SignedLimb);
    });
