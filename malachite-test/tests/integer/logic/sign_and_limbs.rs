...
fn test_sign_and_limbs_le() {
    let test = |n, out| {
        assert_eq!(native::Integer::from_str(n).unwrap().sign_and_limbs_le(),
                   out);
    ...
    test("18446744073709551615",
         (Ordering::Greater, vec![4294967295, 4294967295]));
    test("-18446744073709551615",
         (Ordering::Less, vec![4294967295, 4294967295]));
...
fn test_sign_and_limbs_be() {
    let test = |n, out| {
        assert_eq!(native::Integer::from_str(n).unwrap().sign_and_limbs_be(),
                   out);
