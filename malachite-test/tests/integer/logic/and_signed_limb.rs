...
use malachite_test::integer::logic::and_signed_limb::{
    integer_and_signed_limb_alt_1, integer_and_signed_limb_alt_2,
};
#[cfg(feature = "32_bit_limbs")]
use rug::{self, Assign};
use std::str::FromStr;
...
fn test_and_signed_limb() {
    let test = |u, v: SignedLimb, out| {
        ...
        assert_eq!(
            integer_and_signed_limb_alt_1(&Integer::from_str(u).unwrap(), v).to_string(),
            out
        );
        assert_eq!(
            integer_and_signed_limb_alt_2(&Integer::from_str(u).unwrap(), v).to_string(),
            out
        );
...
fn and_signed_limb_properties() {
    test_properties(
        ...
        |&(ref n, i): &(Integer, SignedLimb)| {
            ...
            assert_eq!(integer_and_signed_limb_alt_1(&n, i), result);
            assert_eq!(integer_and_signed_limb_alt_2(&n, i), result);
