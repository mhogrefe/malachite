...
use malachite_test::integer::comparison::partial_ord_i32::{num_partial_cmp_i32, select_inputs_1};
use num;
use rugint;
...
fn test_partial_ord_i32() {
    let test = |u, v: i32, out| {
        ...
        assert_eq!(
            num_partial_cmp_i32(&num::BigInt::from_str(u).unwrap(), v),
            out
        );
...
fn partial_cmp_i32_properties() {
    ...
    let integer_and_i32 = |gmp_n: gmp::Integer, i: i32| {
        ...
        assert_eq!(
            num_partial_cmp_i32(&native_integer_to_num_bigint(&n), i),
            cmp_1
        );
