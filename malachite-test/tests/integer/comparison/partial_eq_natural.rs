...
fn test_integer_partial_eq_natural() {
    let test = |u, v, out| {
        assert_eq!(native::integer::Integer::from_str(v).unwrap() ==
                   native::natural::Natural::from_str(u).unwrap(),
                   out);
        assert_eq!(gmp::integer::Integer::from_str(v).unwrap() ==
...
fn integer_partial_eq_natural_properties() {
    ...
    let integer_and_natural = |gmp_x: gmp::integer::Integer, gmp_y: gmp::natural::Natural| {
        ...
        assert_eq!(native_integer_to_rugint(&x) == native_natural_to_rugint_integer(&y),
                   eq);
