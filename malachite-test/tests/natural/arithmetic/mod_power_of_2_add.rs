    ...
    test_properties(
        ...
        |&(ref xs, y, pow)| {
            assert_eq!(
                ...
                Natural::from_limbs_asc(xs).mod_power_of_2_add(Natural::from(y), pow),
            );
        },
    );
}
...
fn limbs_mod_power_of_2_add_helper(
    ...
    assert_eq!(
        ...
        Natural::from_limbs_asc(xs).mod_power_of_2_add(Natural::from_limbs_asc(ys), pow)
    );
}
    ...
    test_properties(
        ...
        |&(ref xs, ref ys, pow)| {
            ...
            let n = Natural::from_owned_limbs_asc(xs_old)
                .mod_power_of_2_add(Natural::from_limbs_asc(ys), pow);
            let len = xs.len();
...
fn mod_power_of_2_add_properties() {
    ...
    test_properties(pairs_of_natural_and_u64_var_1, |&(ref x, pow)| {
        ...
        assert_eq!(x.mod_power_of_2_add(x, pow), x.mod_power_of_2_shl(1, pow));
    });
