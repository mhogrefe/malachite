...
fn limbs_next_power_of_2_properties() {
    test_properties(vecs_of_unsigned_var_1, |ref limbs| {
        assert_eq!(
            Natural::from_owned_limbs_asc(limbs_next_power_of_2(limbs)),
            Natural::from_limbs_asc(limbs).next_power_of_2(),
        );
    });
}
...
fn limbs_slice_next_power_of_2_in_place_properties() {
    test_properties(vecs_of_unsigned_var_1, |&ref limbs| {
        ...
        let n = Natural::from_limbs_asc(&old_limbs).next_power_of_2();
