...
fn test_limbs_special_random_up_to_bits() {
    let test = |bits, out: &[u32]| {
        ...
        assert_eq!(limbs_special_random_up_to_bits(&mut rng, bits), out);
    };
    test(1, &[1]);
    test(2, &[1]);
    test(3, &[5]);
...
fn limbs_special_random_up_to_bits_fail() {
    ...
    limbs_special_random_up_to_bits(&mut rng, 0);
}
...
fn limbs_special_random_up_to_bits_properties() {
    ...
    test_properties_no_special(small_positive_unsigneds, |&bits| {
        ...
        assert_eq!(
            Natural::from_owned_limbs_asc(limbs_special_random_up_to_bits(&mut rng, bits)),
