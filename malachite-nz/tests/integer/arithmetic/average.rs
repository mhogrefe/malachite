...
fn test_average_round() {
    ...
    test("-4", "-7", Floor, "-6", Less);
    test("-4", "-7", Ceiling, "-5", Greater);
    test("-4", "-7", Down, "-5", Greater);
    test("-4", "-7", Up, "-6", Less);
    test("-4", "-7", Nearest, "-6", Less);
    test(
        "1000000000000",
        "1000000000001",
        Floor,
...
fn average_round_exact_fail() {
    Integer::from_str("4")
        .unwrap()
        .average_round(Integer::from_str("7").unwrap(), Exact);
}

#[test]
fn average_properties() {
    integer_integer_rounding_mode_triple_gen_var_3().test_properties(|(x, y, rm)| {
        ...
        assert_eq!((&x + &y).div_round(Integer::TWO, rm), (avg.clone(), o));
        ...
        if (&x + &y).even() {
            ...
            assert_eq!(ceiling, &floor + Integer::ONE);
        ...
        if let (Ok(sx), Ok(sy)) = (i64::try_from(&x), i64::try_from(&y)) {
            let (savg, so) = sx.average_round(sy, rm);
            assert_eq!(avg, Integer::exact_from(savg));
            assert_eq!(o, so);
        }
    });
