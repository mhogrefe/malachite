...
fn mul_sub_mul_properties() {
    integer_quadruple_gen().test_properties(|(x, y, z, w)| {
        ...
        assert_eq!(x.clone().mul_sub_mul(y.clone(), &z, &w), result);
        assert_eq!(x.clone().mul_sub_mul(&y, z.clone(), &w), result);
        assert_eq!(x.clone().mul_sub_mul(&y, &z, w.clone()), result);
