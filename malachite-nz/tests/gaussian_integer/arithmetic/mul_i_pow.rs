...
use malachite_nz::test_util::generators::{
    gaussian_integer_gen, gaussian_integer_unsigned_pair_gen,
};
use std::str::FromStr;
...
fn expected(x: &GaussianInteger, k: u64) -> GaussianInteger {
    match k {
        0 => x.clone(),
        1 => x.mul_i(),
        2 => -x,
...
fn mul_i_pow_properties() {
    gaussian_integer_unsigned_pair_gen::<u64>().test_properties(|(x, k)| {
        let mut mut_x = x.clone();
        mut_x.mul_i_pow_assign(k);
        assert!(mut_x.real.is_valid());
        assert!(mut_x.imaginary.is_valid());
