...
fn test_partial_cmp_abs_u32() {
    ...
    test("1000000000000+i", 123, Some(Greater));
}
...
>()
where
    GaussianInteger: EqAbs<T> + From<T> + PartialOrdAbs<T>,
    Integer: From<T>,
{
    gaussian_integer_unsigned_pair_gen::<T>().test_properties(|(x, u)| {
        ...
        assert_eq!(
            (&x).abs_squared()
                .partial_cmp(&Integer::from(u).abs_squared()),
            cmp
        );
        assert_eq!((&x).conjugate().partial_cmp_abs(&u), cmp);
        assert_eq!((-&x).partial_cmp_abs(&u), cmp);
        assert_eq!(Some(x.cmp_abs(&GaussianInteger::from(u))), cmp);
    });
}
    ...
    Integer: From<T>,
{
    gaussian_integer_signed_pair_gen::<T>().test_properties(|(x, i)| {
        ...
        assert_eq!(
            (&x).abs_squared()
                .partial_cmp(&Integer::from(i).abs_squared()),
            cmp
        );
        assert_eq!((&x).conjugate().partial_cmp_abs(&i), cmp);
        assert_eq!((-&x).partial_cmp_abs(&i), cmp);
        assert_eq!(Some(x.cmp_abs(&GaussianInteger::from(i))), cmp);
    });
}
