...
fn partial_eq_primitive_int_properties_helper_unsigned<
    T: PartialEq<GaussianInteger> + PrimitiveUnsigned,
>()
where
    GaussianInteger: From<T> + PartialEq<T>,
{
    gaussian_integer_unsigned_pair_gen::<T>().test_properties(|(n, u)| {
        let eq = n == u;
        assert_eq!(u == n, eq);
        assert_eq!(&n == &GaussianInteger::from(u), eq);
    });
...
fn partial_eq_primitive_int_properties_helper_signed<
    T: PartialEq<GaussianInteger> + PrimitiveSigned,
>()
where
    GaussianInteger: From<T> + PartialEq<T>,
{
    gaussian_integer_signed_pair_gen::<T>().test_properties(|(n, i)| {
        let eq = n == i;
        assert_eq!(i == n, eq);
        assert_eq!(&n == &GaussianInteger::from(i), eq);
    });
