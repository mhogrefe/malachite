...
fn test_degree() {
    let test = |s, out| {
        assert_eq!(NaturalPolynomial::from_str(s).unwrap().degree(), out);
    };
    test("0", None);
    test("1", Some(0));
    test("5", Some(0));
...
fn test_zero_coefficients() {
    let test = |s, start, end, out| {
        let mut p = NaturalPolynomial::from_str(s).unwrap();
...
fn zero_coefficients_fail() {
    NaturalPolynomial::from_str("x^2+x+1")
        .unwrap()
...
fn coefficients_properties() {
    natural_polynomial_gen().test_properties(|p| {
        ...
        let q = NaturalPolynomial::from_coefficients_asc(cs.clone());
        assert!(q.is_valid());
        assert_eq!(q, p);
...
fn zero_coefficients_properties() {
    natural_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, i)| {
            ...
            [(i, i), (i >> 1, i), (0, i), (i, i + 1), (i, len.max(i)), (i, u64::MAX)]
        {
            ...
            assert_eq!(q, NaturalPolynomial::from_coefficients_asc(cs));
...
fn reverse_properties() {
    natural_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, i)| {
        ...
        for n in [i, len, len + i] {
            ...
            let mut cs = p.clone().into_coefficients_asc();
            cs.truncate(n_usize);
            cs.resize(n_usize, Natural::ZERO);
            cs.reverse();
            ...
            assert_eq!(
                q.reverse(n),
                NaturalPolynomial::from_coefficients_asc(truncated)
            );
        }
