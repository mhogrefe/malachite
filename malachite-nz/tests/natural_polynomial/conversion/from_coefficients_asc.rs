...
fn naturals(xs: &[u32]) -> Vec<Natural> {
    xs.iter().copied().map(Natural::from).collect()
}

#[test]
fn test_from_coefficients_asc() {
    let test = |xs: &[u32], out| {
        let p = NaturalPolynomial::from_coefficients_asc(naturals(xs));
        assert!(p.is_valid());
...
fn from_coefficients_asc_properties() {
    natural_vec_gen().test_properties(|xs| {
        ...
        padded.push(Natural::ZERO);
        assert_eq!(NaturalPolynomial::from_coefficients_asc(padded), p);
        assert_eq!(
            NaturalPolynomial::from_coefficients_asc(p.coefficients_asc().to_vec()),
            p
        );
    });
}
