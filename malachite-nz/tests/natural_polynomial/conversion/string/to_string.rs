...
fn test_to_string() {
    let test = |s| {
        assert_eq!(NaturalPolynomial::from_str(s).unwrap().to_string(), s);
    };
    test("0");
    test("1");
    test("5");
...
fn test_to_string_normalizes() {
    ...
    let test = |s, out| {
        assert_eq!(NaturalPolynomial::from_str(s).unwrap().to_string(), out);
    };
...
fn to_string_properties() {
    natural_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(NaturalPolynomial::from_str(&s).unwrap(), p);
        assert!(!s.is_empty());
        // Nothing in the output can be mistaken for a variable's name.
        assert!(!s.contains(' '));
        assert!(!s.contains('-'));
        assert_eq!(s == "0", p == NaturalPolynomial::ZERO);
        ...
        assert_eq!(
            NaturalPolynomial::from_string_with(vars.var(0), &named).unwrap(),
            p
        );
