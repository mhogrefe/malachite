...
use malachite_nz::test_util::generators::{
    ...
    natural_polynomial_unsigned_polynomial_pair_gen,
};
...
fn eq_truncated_agreement_properties() {
    natural_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, i)| {
        ...
        let mut q = p.clone();
        q.mutate_coefficient(i, |c| *c += Natural::ONE);
...
fn eq_truncated_unsigned_polynomial_properties() {
    natural_polynomial_unsigned_polynomial_pair_gen::<u64>().test_properties(|(p, q)| {
        let (p_len, q_len) = (p.len(), q.len());
        for len in [0, 1, 2, p_len, q_len, p_len.max(q_len), u64::MAX] {
            let eq = p.eq_truncated(&q, len);
            assert_eq!(q.eq_truncated(&p, len), eq);
        ...
        let mut r = NaturalPolynomial::from(q.clone());
        assert!(r.eq_truncated(&q, u64::MAX));
        let k = q.len() >> 1;
        r.mutate_coefficient(k, |c| *c += Natural::ONE);
