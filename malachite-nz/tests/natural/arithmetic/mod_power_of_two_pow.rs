...
#[cfg(feature = "32_bit_limbs")]
#[test]
fn test_limbs_mod_power_of_two_pow() {
    let test = |xs: &[Limb], es: &[Limb], pow: u64, out: &[Limb]| {
        let xs_old = xs;
        let mut xs = xs_old.to_vec();
        limbs_mod_power_of_two_pow(&mut xs, es, pow);
        assert_eq!(xs, out);
        verify_limbs_mod_power_of_two_pow(xs_old, es, pow, out);
    };
    test(&[1], &[2], 2, &[1]);
    test(&[3], &[2], 2, &[1]);
    test(&[3], &[3], 4, &[11]);
...
#[test]
#[should_panic]
fn limbs_mod_power_of_two_pow_fail_1() {
    limbs_mod_power_of_two_pow(&mut vec![2], &[2, 0], 5);
}

#[cfg(feature = "32_bit_limbs")]
#[test]
#[should_panic]
fn limbs_mod_power_of_two_pow_fail_2() {
    limbs_mod_power_of_two_pow(&mut vec![2], &[1], 5);
}
