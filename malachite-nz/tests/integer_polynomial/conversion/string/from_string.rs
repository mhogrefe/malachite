...
fn test_from_str_fail() {
    let test = |s| {
        assert!(
            IntegerPolynomial::from_str(s).is_err(),
            "{s:?} should not parse"
        );
    };
    test("");
...
fn test_from_string_with() {
    ...
    assert_eq!(
        IntegerPolynomial::from_string_with(GreekVars.var(0), "α^2+3*α+2").unwrap(),
        p
    );
    assert_eq!(
        IntegerPolynomial::from_string_with(XyzVars.var(1), "y^2+3*y+2").unwrap(),
        p
    );
    ...
    assert!(IntegerPolynomial::from_string_with(GreekVars.var(0), "x^2").is_none());
    assert!(IntegerPolynomial::from_string_with(XyzVars.var(1), "x^2").is_none());
}
...
fn from_str_properties() {
    integer_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(IntegerPolynomial::from_str(&p.to_string()).unwrap(), p);
        ...
        assert_eq!(
            IntegerPolynomial::from_string_with(XyzVars.var(0), &p.to_string()).unwrap(),
            p
        );
    });
    ...
    string_gen().test_properties(|s| {
        ...
        if let Ok(p) = IntegerPolynomial::from_str(&s) {
            assert!(p.is_valid());
            assert_eq!(IntegerPolynomial::from_str(&p.to_string()).unwrap(), p);
        }
    });
