...
use malachite_nz::test_util::generators::{
    integer_polynomial_pair_gen, integer_polynomial_triple_gen, natural_polynomial_pair_gen,
};
...
fn test_add() {
    ...
    test("0", "0", "0");
    test("0", "x-1", "x-1");
    test("x^2-3*x+2", "2*x+5", "x^2-x+7");
    test("-x^2+3*x+1", "x^2-3*x+2", "3");
    test("x^3+1", "-x^3+x", "x+1");
    test("x+1", "-x-1", "0");
    test(
...
fn add_properties() {
    integer_polynomial_pair_gen().test_properties(|(p, q)| {
        ...
        // Addition is commutative, zero is its identity, and p + (-p) = 0.
        assert_eq!(&q + &p, r);
    ...
    natural_polynomial_pair_gen().test_properties(|(p, q)| {
        // Adding Natural polynomials as Integer polynomials is adding them as Natural polynomials.
        assert_eq!(
