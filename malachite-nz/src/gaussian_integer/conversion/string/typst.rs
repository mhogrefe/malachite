...
impl ToTypst for ComparableGaussianIntegerRef<'_> {
    /// Writes a [`ComparableGaussianIntegerRef`] as a Typst math-mode fragment.
    ///
    /// The fragment is the wrapped [`GaussianInteger`]'s own: the wrapper exists to give an
    /// ordering, and does not change what the value is.
    ///
    ...
    /// let x = GaussianInteger::from(2);
    /// assert_eq!(
    ///     ComparableGaussianIntegerRef(&x).to_typst_string(),
    ///     x.to_typst_string()
    /// );
    /// ```
    #[inline]
