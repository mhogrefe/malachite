...
impl CheckedFrom<Integer> for i32 {
    /// Converts an `Integer` to an `i32`, taking the `Integer` by value and returning `None` if
    /// the `Integer` is outside the range of an `i32`.
    ///
    /// Time: worst case O(1)
    ///
...
impl<'a> CheckedFrom<&'a Integer> for i32 {
    /// Converts an `Integer` to an `i32`, taking the `Integer` by reference and returning `None` if
    /// the `Integer` is outside the range of an `i32`.
    ///
    /// Time: worst case O(1)
    ///
...
impl WrappingFrom<Integer> for i32 {
    /// Converts an `Integer` to a `i32`, taking the `Integer` by reference and wrapping mod
    /// 2<sup>32</sup>.
    ///
    /// Time: worst case O(1)
    ///
