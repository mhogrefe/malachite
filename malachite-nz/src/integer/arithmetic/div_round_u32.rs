...
impl DivRound<u32> for Integer {
    ...
    /// Divides an `Integer` by a `u32` and rounds according to a specified rounding mode, taking
...
impl DivRound<Integer> for u32 {
    ...
    /// Divides a `u32` by an `Integer` and rounds according to a specified rounding mode, taking
    /// the `Integer` by value. See the `RoundingMode` documentation for details.
    ///
    /// Time: worst case O(n)
    ///
...
impl DivRoundAssign<u32> for Integer {
    /// Divides an `Integer` by a `u32` in place and rounds according to a specified rounding mode.
