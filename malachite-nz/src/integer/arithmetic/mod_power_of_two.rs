...
impl NegModPowerOfTwo for Integer {
    type Output = Natural;

    /// Takes the negative of an `Integer` mod a power of 2, taking the `Integer` by value and
    /// returning a `Natural`. In other words, returns r, where
    ...
    /// fn main() {
    ///     // 2 * 2^8 - 252 = 260
    ///     assert_eq!(Integer::from(260).neg_mod_power_of_two(8).to_string(), "252");
    ///
    ///     // -100 * 2^4 - 11 = -1611
    ///     assert_eq!(Integer::from(-1611).neg_mod_power_of_two(4).to_string(), "11");
    /// }
    /// ```
...
impl<'a> NegModPowerOfTwo for &'a Integer {
    type Output = Natural;

    /// Takes the negative of an `Integer` mod a power of 2, taking the `Integer` by reference and
    /// returning a `Natural`. In other words, returns r, where
    /// `self` = q * 2<sup>`other`</sup> - r and 0 <= r < 2<sup>`other`</sup>.
    ///
    /// Time: worst case O(`other`)
...
impl NegModPowerOfTwoAssign for Integer {
    /// Takes the negative of an `Integer` mod a power of 2 in place. In other words, replaces
    /// `self` with r, where `self` = q * 2<sup>`other`</sup> - r and 0 <= r < 2<sup>`other`</sup>.
    ///
    /// Time: worst case O(`other`)
    ///
    /// Additional memory: worst case O(`other`)
