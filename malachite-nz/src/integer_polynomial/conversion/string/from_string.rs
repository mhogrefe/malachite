...
fn parse_term<S: VarScheme + ?Sized>(var: Var<'_, S>, term: &str) -> Option<(Integer, usize)> {
    if term.starts_with(|c: char| !char_is_reserved(c)) {
        return Some((Integer::ONE, parse_monic(var, term)?));
    }
...
impl FromStr for IntegerPolynomial {
    ...
    ///
    /// The variable is called `x`. [`from_string_with`](IntegerPolynomial::from_string_with) is the
