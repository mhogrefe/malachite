...
// (`limbs_mul_high_same_length_scratch_len`, fixed 2026-08-02) lived exactly in a band between
// two algorithms. The sentinel fill measures the other direction: how much of the advertised
// length the function actually touches.
...
// Runs `f` on a scratch slice of exactly `scratch_len` limbs pre-filled with `SCRATCH_SENTINEL`
// and returns the high-water mark: the number of limbs up to and including the highest one that
// `f` overwrote. If `scratch_len` is smaller than `f` actually needs, `f` is expected to panic. A
// limb that `f` overwrites with the sentinel value itself goes undetected, which only shifts the
...
pub fn scratch_high_water(scratch_len: usize, f: impl FnOnce(&mut [Limb])) -> usize {
    ...
    f(&mut scratch);
    scratch.iter().rposition(|&x| x != SCRATCH_SENTINEL).map_or(0, |i| i + 1)
}

// Lengths that exercise every algorithm band: for each threshold, all lengths within `radius` of
// it, plus geometric filler (about 8 lengths per octave) so the interiors of the bands are
// sampled too. All returned lengths are within `min..=max`.
...
pub fn canary_limbs(name: &str, len: usize) -> Vec<Limb> {
    random_primitive_ints(EXAMPLE_SEED.fork(name)).take(len).collect()
}
