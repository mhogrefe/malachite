...
impl Polynomial for NaturalPolynomial {
    ...
    /// Sets the coefficients of a [`NaturalPolynomial`] of $x^i$ for $i$ in `start..end` to zero.
    ///
    ...
    ///
    /// let mut p;
    /// p = NaturalPolynomial::from_str("5*x^4+4*x^3+3*x^2+2*x+1").unwrap();
    ...
    /// ```
    ///
    /// This is equivalent to `fmpz_mod_poly_zero_coeffs` from `fmpz_mod_poly/zero_coeffs.c`, FLINT
    /// 3.6.0, and to `fmpz_poly_zero_coeffs` from `fmpz_poly/zero_coeffs.c` for a polynomial whose
    ...
    /// Truncates a [`NaturalPolynomial`] to its first `len` coefficients, taking the polynomial by
    ...
    ///
    /// assert_eq!(
    ///     NaturalPolynomial::from_str("x^3+2*x^2+3*x+4")
    ///         .unwrap()
    ///         .truncate(2)
    ///         .to_string(),
    ///     "3*x+4"
    ...
    /// assert_eq!(
    ///     NaturalPolynomial::from_str("x^3+3*x+4")
    ///         .unwrap()
    ///         .truncate(3)
    ///         .to_string(),
    ///     "3*x+4"
    ...
    /// ```
    ///
    /// This is equivalent to `fmpz_mod_poly_set_trunc` from `fmpz_mod_poly/set_trunc.c`, FLINT
    /// 3.6.0, and to `fmpz_poly_set_trunc` from `fmpz_poly/set_trunc.c` for a polynomial whose
    /// coefficients are all nonnegative.
    ...
    /// Truncates a [`NaturalPolynomial`] to its first `len` coefficients, in place.
    ///
    ...
    ///
    /// let mut p;
    /// p = NaturalPolynomial::from_str("x^3+2*x^2+3*x+4").unwrap();
    /// p.truncate_assign(2);
    ...
    /// // Truncating can uncover zeros, which are dropped too.
    /// p = NaturalPolynomial::from_str("x^3+3*x+4").unwrap();
    /// p.truncate_assign(3);
    /// assert_eq!(p.to_string(), "3*x+4");
    /// p = NaturalPolynomial::from_str("x^3+2*x^2+3*x+4").unwrap();
    /// p.truncate_assign(0);
    ...
    /// ```
    ///
    /// This is equivalent to `fmpz_mod_poly_truncate` from `fmpz_mod_poly/truncate.c`, FLINT 3.6.0,
    /// and to `fmpz_poly_truncate` from `fmpz_poly/truncate.c` for a polynomial whose coefficients
    /// are all nonnegative.
    ...
    /// Reverses the coefficients of a [`NaturalPolynomial`], considered as having length `len`,
    ...
    ///
    /// assert_eq!(
    ///     NaturalPolynomial::from_str("x^2+2*x+3")
    ///         .unwrap()
    ///         .reverse(3)
    ///         .to_string(),
    ///     "3*x^2+2*x+1"
    ...
    /// assert_eq!(
    ///     NaturalPolynomial::from_str("x^2+2*x")
    ///         .unwrap()
    ///         .reverse(3)
    ///         .to_string(),
    ///     "2*x+1"
    ...
    /// ```
    ///
    /// This is equivalent to `fmpz_mod_poly_reverse` from `fmpz_mod_poly/reverse.c`, FLINT 3.6.0,
    /// and to `fmpz_poly_reverse` from `fmpz_poly/reverse.c` for a polynomial whose coefficients
    /// are all nonnegative.
    ...
    /// Reverses the coefficients of a [`NaturalPolynomial`], considered as having length `len`, in
    /// place.
    ///
    ...
    ///
    /// let mut p;
    /// p = NaturalPolynomial::from_str("x^2+2*x+3").unwrap();
    /// p.reverse_assign(3);
    ...
    /// // A zero constant term becomes a trailing zero, and is dropped.
    /// p = NaturalPolynomial::from_str("x^2+2*x").unwrap();
    /// p.reverse_assign(3);
    /// assert_eq!(p.to_string(), "2*x+1");
    /// ```
    ///
