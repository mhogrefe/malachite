...
use crate::natural::exhaustive::{
    ExhaustiveNaturalRange, ExhaustiveNaturalRangeToInfinity, exhaustive_natural_range,
...
/// 2.
pub type ExhaustiveNaturalPolynomialsReducedModPowerOf2 =
    ExhaustiveNaturalPolynomials<ExhaustiveNaturalRange, ExhaustiveNaturalRange>;

/// Generates all [`NaturalPolynomial`]s that are reduced modulo $2^k$.
///
...
///
/// # Panics
/// Panics if `pow` is zero. The only polynomial reduced modulo $2^0$ is the zero polynomial, which
/// leaves no leading coefficient to choose.
///
/// # Examples
/// ```
/// use malachite_base::iterators::prefix_to_string;
...
/// assert_eq!(
///     prefix_to_string(exhaustive_natural_polynomials_reduced_mod_power_of_2(1), 10),
///     "[0, 1, x, x^2, x+1, x^2+x, x^2+1, x^3, x^4, x^5, ...]"
/// );
/// ```
#[inline]
pub fn exhaustive_natural_polynomials_reduced_mod_power_of_2(
    pow: u64,
) -> ExhaustiveNaturalPolynomialsReducedModPowerOf2 {
    assert_ne!(
        pow, 0,
        "the only polynomial reduced modulo 2^0 is the zero polynomial"
    );
    let limit = Natural::power_of_2(pow);
    exhaustive_natural_polynomials_from_iterators(
        exhaustive_natural_range(Natural::ZERO, limit.clone()),
        exhaustive_natural_range(Natural::ONE, limit),
    )
}

/// The type of the [`NaturalPolynomial`] generator whose coefficients are reduced modulo a number.
pub type ExhaustiveNaturalPolynomialsReducedMod =
    ExhaustiveNaturalPolynomials<ExhaustiveNaturalRange, ExhaustiveNaturalRange>;

/// Generates all [`NaturalPolynomial`]s that are reduced modulo $m$.
///
...
///
/// Where $m$ is a power of 2, [`exhaustive_natural_polynomials_reduced_mod_power_of_2`] generates
/// the same polynomials in the same order.
///
/// # Worst-case complexity per iteration
/// $T(i) = O(\ell n)$
///
...
/// ```
#[inline]
pub fn exhaustive_natural_polynomials_reduced_mod(
    m: Natural,
) -> ExhaustiveNaturalPolynomialsReducedMod {
    assert!(
        m >= 2u32,
        "nothing is reduced modulo 0, and only the zero polynomial is reduced modulo 1"
    );
    exhaustive_natural_polynomials_from_iterators(
        exhaustive_natural_range(Natural::ZERO, m.clone()),
        exhaustive_natural_range(Natural::ONE, m),
    )
}
