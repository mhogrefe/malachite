...
impl From<U64Polynomial> for NaturalPolynomial {
    /// Converts a [`U64Polynomial`] to a [`NaturalPolynomial`].
    ///
    /// Every [`u64`] is a [`Natural`], so nothing is lost and nothing can fail. The coefficients
    /// are converted one by one, and the leading one stays nonzero, so the degree is unchanged.
    ///
    /// $f(p) = p$, read on the left over the [`u64`]s and on the right over $\N$.
    ///
    ...
    ///
    /// assert_eq!(
    ///     NaturalPolynomial::from(U64Polynomial::default()).to_string(),
    ///     "0"
    /// );
    /// ```
    #[inline]
