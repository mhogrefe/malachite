...
// and x are reduced.
fn mod_power_of_2_evaluate_ref(p: &NaturalPolynomial, x: &Natural, pow: u64) -> Natural {
...
// The same as `mod_power_of_2_evaluate_ref`, but taking the polynomial by value, so that the
// coefficient that is returned outright, or that starts Horner's rule, is moved rather than cloned.
fn mod_power_of_2_evaluate_val(p: NaturalPolynomial, x: &Natural, pow: u64) -> Natural {
    assert_reduced_mod_power_of_2(&p, x, pow);
...
impl ModPowerOf2Evaluate<&Natural> for &NaturalPolynomial {
    type Output = Natural;
    ...
    /// assert_eq!((&p).mod_power_of_2_evaluate(&Natural::from(6u32), 4), 13);
    /// assert_eq!((&p).mod_power_of_2_evaluate(&Natural::ZERO, 4), 7);
    /// // 5 + 3 + 7 = 15.
    /// assert_eq!((&p).mod_power_of_2_evaluate(&Natural::ONE, 4), 15);
    /// ```
    #[inline]
    fn mod_power_of_2_evaluate(self, x: &Natural, pow: u64) -> Natural {
        mod_power_of_2_evaluate_ref(self, x, pow)
    }
}
...
impl ModPowerOf2Evaluate<Natural> for NaturalPolynomial {
    ...
    /// // 5 * 36 + 3 * 6 + 7 = 205, which is 13 mod 16.
    /// assert_eq!(
    ///     p.clone().mod_power_of_2_evaluate(Natural::from(6u32), 4),
    ///     13
    /// );
    /// assert_eq!(p.clone().mod_power_of_2_evaluate(Natural::ZERO, 4), 7);
    /// // 5 + 3 + 7 = 15.
    /// assert_eq!(p.mod_power_of_2_evaluate(Natural::ONE, 4), 15);
    /// ```
    #[inline]
    fn mod_power_of_2_evaluate(self, x: Natural, pow: u64) -> Natural {
        mod_power_of_2_evaluate_val(self, &x, pow)
    }
}
...
fn mod_evaluate_ref(p: &NaturalPolynomial, x: &Natural, m: &Natural) -> Natural {
...
// The same as `mod_evaluate_ref`, but taking the polynomial by value, so that the coefficient that
// is returned outright, or that starts Horner's rule, is moved rather than cloned.
fn mod_evaluate_val(p: NaturalPolynomial, x: &Natural, m: &Natural) -> Natural {
    assert_reduced_mod(&p, x, m);
    let mut coefficients = p.coefficients;
    if coefficients.len() <= 1 || *x == 0u32 {
        return coefficients.into_iter().next().unwrap_or(Natural::ZERO);
...
impl ModEvaluate<&Natural, &Natural> for &NaturalPolynomial {
    type Output = Natural;
    ...
    /// assert_eq!((&p).mod_evaluate(&Natural::from(6u32), &m.clone()), 7);
    /// assert_eq!((&p).mod_evaluate(&Natural::ZERO, &m.clone()), 7);
    /// // 5 + 3 + 7 = 15, which is 4 mod 11.
    /// assert_eq!((&p).mod_evaluate(&Natural::ONE, &m), 4);
    /// ```
    ///
    ...
    #[inline]
    fn mod_evaluate(self, x: &Natural, m: &Natural) -> Natural {
        mod_evaluate_ref(self, x, m)
    }
}
...
impl ModEvaluate<&Natural, &Natural> for NaturalPolynomial {
    ...
    /// let m = Natural::from(11u32);
    /// // 5 * 36 + 3 * 6 + 7 = 205, which is 7 mod 11.
    /// assert_eq!(p.clone().mod_evaluate(&Natural::from(6u32), &m.clone()), 7);
    /// assert_eq!(p.clone().mod_evaluate(&Natural::ZERO, &m.clone()), 7);
    /// // 5 + 3 + 7 = 15, which is 4 mod 11.
    /// assert_eq!(p.mod_evaluate(&Natural::ONE, &m), 4);
    /// ```
    ///
...
fn mod_evaluate_many(p: &NaturalPolynomial, xs: &[Natural], m: &Natural) -> Vec<Natural> {
    assert!(
        p.mod_is_reduced(m),
        "self must be reduced mod m, but {p} has a coefficient >= {m}"
    );
...
impl ModEvaluateMany<Natural, &Natural> for &NaturalPolynomial {
    type Output = Natural;
    ...
    /// [`mod_evaluate`](malachite_base::polynomial::ModEvaluate::mod_evaluate) at each value, but
    ...
    /// assert_eq!(
    ///     (&p).mod_evaluate_many(&xs, &Natural::from(13u32)),
    ...
    #[inline]
    fn mod_evaluate_many(self, xs: &[Natural], m: &Natural) -> Vec<Natural> {
        mod_evaluate_many(self, xs, m)
    }
}
