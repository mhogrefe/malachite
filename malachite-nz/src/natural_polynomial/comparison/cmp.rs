...
impl PartialOrd for NaturalPolynomial {
    /// Compares two [`NaturalPolynomial`]s.
    ///
...
impl Ord for NaturalPolynomial {
    /// Compares two [`NaturalPolynomial`]s by how they behave for large arguments.
    ///
    ...
    /// constant polynomials it is the order on the [`Natural`](crate::natural::Natural)s, so the
    /// embedding of a number as a polynomial preserves comparisons. It is not a well-order, and no
    /// order compatible with addition can be: $x > x - 1 > x - 2 > \ldots$ descends forever. A
    /// well-order on polynomials needs to weigh a polynomial's size against its degree, which this
    /// order does not do.
    ///
    ...
    ///         > NaturalPolynomial::from_str("1000000*x").unwrap()
    /// );
    ///
    /// // At equal degrees the leading coefficient decides.
    /// assert!(
    ///     NaturalPolynomial::from_str("2*x^2").unwrap()
    ///         > NaturalPolynomial::from_str("x^2+1000000").unwrap()
    /// );
    ///
    /// // When that ties, the next coefficient down does.
    /// assert!(
    ///     NaturalPolynomial::from_str("x^2+3*x").unwrap()
    ///         > NaturalPolynomial::from_str("x^2+2*x+1000000").unwrap()
    /// );
    ///
    /// // The zero polynomial is below everything else.
    /// assert!(
    ///     NaturalPolynomial::from_str("0").unwrap() < NaturalPolynomial::from_str("1").unwrap()
    /// );
    ///
    /// // Constant polynomials compare as the numbers they are.
    /// assert!(
    ///     NaturalPolynomial::from_str("123").unwrap()
    ///         > NaturalPolynomial::from_str("122").unwrap()
    /// );
    /// ```
