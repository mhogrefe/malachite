...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_natural_polynomial_mod_is_reduced);
    register_demo!(runner, demo_natural_polynomial_mod_power_of_2_is_reduced);

    register_bench!(runner, benchmark_natural_polynomial_mod_is_reduced);
    register_bench!(
        runner,
        benchmark_natural_polynomial_mod_power_of_2_is_reduced
    );
}
...
fn demo_natural_polynomial_mod_is_reduced(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in natural_polynomial_gen().get(gm, config).take(limit) {
        let m = p.to_height() + Natural::ONE;
        println!("({p}).mod_is_reduced(&{m}) = {}", p.mod_is_reduced(&m));
    }
}

fn demo_natural_polynomial_mod_power_of_2_is_reduced(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
...
fn benchmark_natural_polynomial_mod_is_reduced(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "NaturalPolynomial.mod_is_reduced(&Natural)",
...
fn benchmark_natural_polynomial_mod_power_of_2_is_reduced(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "NaturalPolynomial.mod_power_of_2_is_reduced(u64)",
        BenchmarkType::Single,
        natural_polynomial_gen().get(gm, config),
        gm.name(),
        limit,
