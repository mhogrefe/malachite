...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_integer_polynomial_partial_eq_integer);
    register_demo!(runner, demo_integer_partial_eq_integer_polynomial);

    register_bench!(
        runner,
        benchmark_integer_polynomial_partial_eq_integer_algorithms
    );
}

fn demo_integer_polynomial_partial_eq_integer(gm: GenMode, config: &GenConfig, limit: usize) {
    for (p, i) in integer_polynomial_integer_pair_gen()
        .get(gm, config)
        .take(limit)
    {
        if p == i {
...
fn demo_integer_partial_eq_integer_polynomial(gm: GenMode, config: &GenConfig, limit: usize) {
    for (p, i) in integer_polynomial_integer_pair_gen()
        .get(gm, config)
        .take(limit)
    {
...
fn benchmark_integer_polynomial_partial_eq_integer_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        ...
        &mut [
            ...
            ("converting the Integer to a polynomial", &mut |(p, i)| {
                no_out!(&p == &IntegerPolynomial::from(i));
            }),
        ],
    );
}
