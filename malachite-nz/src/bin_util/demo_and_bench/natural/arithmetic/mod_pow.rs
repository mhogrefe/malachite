...
use malachite_nz::test_util::bench::bucketers::{
    triple_1_3_prod_natural_bits_bucketer, triple_3_natural_bit_bucketer,
    triple_3_triple_1_3_prod_natural_bits_bucketer,
};
...
pub(crate) fn register(runner: &mut Runner) {
    ...
    register_demo!(runner, demo_natural_mod_pow_precomputed_assign);
    register_demo!(runner, demo_natural_mod_pow_precomputed_assign_ref);
    register_demo!(runner, demo_natural_mod_pow_precomputed);
    register_demo!(runner, demo_natural_mod_pow_precomputed_val_val_ref);
    register_demo!(runner, demo_natural_mod_pow_precomputed_val_ref_val);
    register_demo!(runner, demo_natural_mod_pow_precomputed_val_ref_ref);
    register_demo!(runner, demo_natural_mod_pow_precomputed_ref_val_val);
    register_demo!(runner, demo_natural_mod_pow_precomputed_ref_val_ref);
    register_demo!(runner, demo_natural_mod_pow_precomputed_ref_ref_val);
    register_demo!(runner, demo_natural_mod_pow_precomputed_ref_ref_ref);
    ...
    register_bench!(runner, benchmark_natural_mod_pow_precomputed_algorithms);
}
...
fn demo_natural_mod_pow_precomputed_assign(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let exp_old = exp.clone();
        let m_old = m.clone();
...
fn demo_natural_mod_pow_precomputed_assign_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let exp_old = exp.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
...
fn demo_natural_mod_pow_precomputed(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let exp_old = exp.clone();
        let m_old = m.clone();
...
fn demo_natural_mod_pow_precomputed_val_val_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let exp_old = exp.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
...
fn demo_natural_mod_pow_precomputed_val_ref_val(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let m_old = m.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
...
fn demo_natural_mod_pow_precomputed_val_ref_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let x_old = x.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
        println!(
...
fn demo_natural_mod_pow_precomputed_ref_val_val(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let exp_old = exp.clone();
        let m_old = m.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
...
fn demo_natural_mod_pow_precomputed_ref_val_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let exp_old = exp.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
        println!(
...
fn demo_natural_mod_pow_precomputed_ref_ref_val(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let m_old = m.clone();
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
        println!(
...
fn demo_natural_mod_pow_precomputed_ref_ref_ref(gm: GenMode, config: &GenConfig, limit: usize) {
    for (x, exp, m) in natural_triple_gen_var_5().get(gm, config).take(limit) {
        let data = ModPowPrecomputed::<Natural>::precompute_mod_pow_data(&m);
        println!(
            "(&{}).mod_pow_precomputed(&{}, &{}, &data) = {}",
...
fn benchmark_natural_mod_pow_precomputed_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "Natural.mod_pow_precomputed(Natural, Natural, &ModMulData)",
        BenchmarkType::Algorithms,
        natural_triple_gen_var_5().get(gm, config),
        gm.name(),
        limit,
