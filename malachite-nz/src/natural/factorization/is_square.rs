...
use malachite_base::num::basic::{integers::PrimitiveInt, traits::Zero};
use malachite_base::num::factorization::traits::IsSquare;

const MOD34_BITS: Limb = (Limb::WIDTH / 4) * 3;
const MOD34_MASK: Limb = (1 << MOD34_BITS) - 1;

// This is PERFSQR_MOD_BITS from mpn/perfsqr.h, GMP 6.3.0. Either 49 on 64 bit limb or
// 25 on 32 bit limb.
...
const B1: Limb = Limb::WIDTH / 4;
const B2: Limb = B1 * 2;
const B3: Limb = B1 * 3;

const M1: Limb = (1u64 << B1) - 1;
const M2: Limb = (1u64 << B2) - 1;
const M3: Limb = (1u64 << B3) - 1;
...
//
// Calculate a remainder from `limbs` divided by 2^(Limb::WIDTH*3/4)-1.
// The remainder is not fully reduced, it's any limb value congruent to
// `limbs` modulo that divisor.
//
// Check gen-psqr.c. mpn_mod_34lsub1 preferred over mpn_mod_1 (plus a PERFSQR_PP
// modulus) with 32 and 64 bit limb.
...
// Single limb. Check precomputed bitmasks to see if remainder is a
// quadratic residue
...
// Double limb. Check precomputed bitmasks to see if remainder is a
// quadratic residue
fn perfsqr_mod_2(r: Limb, d: Limb, inv: Limb, mhi: Limb, mlo: Limb) -> bool {
    assert!(d <= 2 * Limb::WIDTH);
    let mut idx = perfsqr_mod_idx(r, d, inv);

    let m = if idx < Limb::WIDTH { mlo } else { mhi };
    idx %= Limb::WIDTH;
...
// This is sq_res0x100 from mpn/perfsqr.h when generated for 64 bit limb, GMP 6.3.0.
...
// This is sq_res0x100 from mpn/perfsqr.h when generated for 32 bit limb, GMP 6.3.0.
// Non-zero bit indicates a quadratic residue mod 0x100.
// This test identifies 82.81% as non-squares (212/256).
#[cfg(feature = "32_bit_limbs")]
...
fn is_square(limbs: &[Limb]) -> bool {
    ...
    // in O(1) time.
    //
    // This just checks the particular bit in the bitmask SQR_MOD256_U64 encoding
    // where the input can be a perfect square mod 256.
    if ((SQR_MOD256[(idx / Limb::WIDTH) as usize] >> (idx % Limb::WIDTH)) & 1) == 0 {
        return false;
    }

    // The second test uses mpn_mod_34lsub1 to detect non-squares according to
    // their residues modulo small primes (or powers of primes).
    // See mpn/perfsqr.h, GMP 6.3.0.
...
impl IsSquare for &Natural {
    /// Determine whether a [`Natural`] is a perfect square. Rules out > 99%
    /// of non-squares in $O(1)$ time using quadratic residue tests, then falls
    /// back to computing the square root. The [`Natural`] is taken by reference.
    ///
    ...
    fn is_square(&self) -> bool {
        match self {
            // use the FLINT n_is_square impl for primitive integers
            // TODO: is the FLINT n_is_square better than the GMP algorithm in
            // this file for word size integers?
            Natural(Small(small)) => small.is_square(),
            Natural(Large(limbs)) => is_square(limbs),
        }
    }
}
...
impl IsSquare for Natural {
    /// Determine whether a [`Natural`] is a perfect square. Rules out > 99%
    /// of non-squares in $O(1)$ time using quadratic residue tests, then falls
    /// back to computing the square root. The [`Natural`] is taken by value.
    ///
    /// $f(x) = (\exists b \in \Z : b^2 = x)$.
    ///
    /// # Worst-case complexity
