...
#[derive(Clone, Debug)]
pub enum NaturalPowerOfTwoDigitIterator<'a, T: PrimitiveUnsigned> {
...
impl<'a, T: PrimitiveUnsigned> Iterator for NaturalPowerOfTwoDigitIterator<'a, T> {
    type Item = T;
    ...
    fn next(&mut self) -> Option<T> {
        match *self {
            NaturalPowerOfTwoDigitIterator::Small(ref mut xs) => xs.next(),
            NaturalPowerOfTwoDigitIterator::FitsInLimb(ref mut xs) => xs.0.next(),
            NaturalPowerOfTwoDigitIterator::SizeOfLimb(ref mut xs) => xs.0.next(),
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match *self {
            ...
            NaturalPowerOfTwoDigitIterator::FitsInLimb(ref xs) => xs.0.size_hint(),
            NaturalPowerOfTwoDigitIterator::SizeOfLimb(ref xs) => xs.0.size_hint(),
...
impl<'a, T: PrimitiveUnsigned> DoubleEndedIterator for NaturalPowerOfTwoDigitIterator<'a, T> {
    ...
    fn next_back(&mut self) -> Option<T> {
        match *self {
            NaturalPowerOfTwoDigitIterator::Small(ref mut xs) => xs.next_back(),
            NaturalPowerOfTwoDigitIterator::FitsInLimb(ref mut xs) => xs.0.next_back(),
            NaturalPowerOfTwoDigitIterator::SizeOfLimb(ref mut xs) => xs.0.next_back(),
...
impl<'a, T: PrimitiveUnsigned> ExactSizeIterator for NaturalPowerOfTwoDigitIterator<'a, T> {}

impl<'a, T: PrimitiveUnsigned> PowerOfTwoDigitIterator<T>
    for NaturalPowerOfTwoDigitIterator<'a, T>
{
    ...
    fn get(&self, index: u64) -> T {
        match *self {
            NaturalPowerOfTwoDigitIterator::Small(ref xs) => xs.get(index),
            NaturalPowerOfTwoDigitIterator::FitsInLimb(ref xs) => xs.0.get(index),
            NaturalPowerOfTwoDigitIterator::SizeOfLimb(ref xs) => xs.0.get(index),
...
macro_rules! iterables {
    (
        ...
        impl<'a> PowerOfTwoDigitIterable<$t> for &'a Natural {
            type PowerOfTwoDigitIterator = NaturalPowerOfTwoDigitIterator<'a, $t>;
            ...
            /// );
            /// ```
            fn power_of_two_digits(self, log_base: u64) -> NaturalPowerOfTwoDigitIterator<'a, $t> {
                    ...
                    Natural(Large(ref limbs)) => {
                        if let Some(log_log_base) = log_base.checked_log_two() {
                            match log_log_base.cmp(&Limb::LOG_WIDTH) {
                                Ordering::Equal => NaturalPowerOfTwoDigitIterator::SizeOfLimb(
                                    $size_of_limb_fn(limbs),
                                ),
                                Ordering::Less => NaturalPowerOfTwoDigitIterator::FitsInLimb(
                                    $fits_in_limb_fn(limbs, log_base),
                                ),
                                Ordering::Greater => {
                                    NaturalPowerOfTwoDigitIterator::MultipleOfLimb(
                                        $multiple_of_limb_fn(limbs, log_base),
                                    )
                                }
                            }
                        } else {
                            NaturalPowerOfTwoDigitIterator::Irregular($irregular_fn(
                                limbs, log_base,
                            ))
                        }
                    }
