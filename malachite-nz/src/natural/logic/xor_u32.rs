...
/// limbs of the bitwise xor of the `Natural` and a `u32`. `limbs` cannot be empty.
///
/// Time: worst case O(n)
///
...
/// # Example
/// ```
/// use malachite_nz::natural::logic::xor_u32::limbs_xor_limb;
///
/// assert_eq!(limbs_xor_limb(&[123, 456], 789), &[878, 456]);
/// ```
pub fn limbs_xor_limb(limbs: &[u32], limb: u32) -> Vec<u32> {
...
/// limbs of the bitwise xor of the `Natural` and a `u32` to an output slice. The output slice must
...
/// # Example
/// ```
/// use malachite_nz::natural::logic::xor_u32::limbs_xor_limb_to_out;
///
/// let mut out_limbs = vec![0, 0, 0];
/// limbs_xor_limb_to_out(&mut out_limbs, &[123, 456], 789);
/// assert_eq!(out_limbs, &[878, 456, 0]);
/// ```
pub fn limbs_xor_limb_to_out(out_limbs: &mut [u32], in_limbs: &[u32], limb: u32) {
    out_limbs[..in_limbs.len()].copy_from_slice(in_limbs);
    limbs_xor_limb_in_place(out_limbs, limb);
}

/// Interpreting a slice of `u32`s as the limbs (in ascending order) of a `Natural`, writes the
/// limbs of the bitwise xor of the `Natural` and a `u32` to the input slice. `limbs` cannot be
/// empty.
///
/// Time: worst case O(1)
///
...
/// # Example
/// ```
/// use malachite_nz::natural::logic::xor_u32::limbs_xor_limb_in_place;
///
/// let mut limbs = vec![123, 456];
/// limbs_xor_limb_in_place(&mut limbs, 789);
/// assert_eq!(limbs, &[878, 456]);
/// ```
pub fn limbs_xor_limb_in_place(limbs: &mut [u32], limb: u32) {
    limbs[0] ^= limb;
}

/// Takes the bitwise xor of a `Natural` and a `u32`, taking the `Natural` by value.
///
/// Time: worst case O(1)
///
/// Additional memory: worst case O(1)
...
impl BitXor<u32> for Natural {
    ...
    fn bitxor(mut self, other: u32) -> Natural {
        self ^= other;
        self
    }
}

/// Takes the bitwise xor of a `Natural` and an `u32`, taking the `Natural` by reference.
///
/// Time: worst case O(n)
///
/// Additional memory: worst case O(n)
...
/// Takes the bitwise xor of a `u32` and a `Natural`, taking the `Natural` by value.
///
/// Time: worst case O(1)
///
/// Additional memory: worst case O(1)
