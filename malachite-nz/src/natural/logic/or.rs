...
fn limbs_or_same_length_no_check(xs: &[u32], ys: &[u32]) -> Vec<u32> {
...
/// ```
pub fn limbs_or_same_length(xs: &[u32], ys: &[u32]) -> Vec<u32> {
    assert_eq!(xs.len(), ys.len());
    limbs_or_same_length_no_check(xs, ys)
}
...
/// ```
pub fn limbs_or(xs: &[u32], ys: &[u32]) -> Vec<u32> {
    let xs_len = xs.len();
    let ys_len = ys.len();
    if xs_len >= ys_len {
        let mut result = limbs_or_same_length_no_check(&xs[0..ys_len], ys);
        result.extend_from_slice(&xs[ys_len..]);
        result
    } else {
        let mut result = limbs_or_same_length_no_check(xs, &ys[0..xs_len]);
        result.extend_from_slice(&ys[xs_len..]);
        result
    }
}

fn limbs_or_same_length_to_out_no_check(out_limbs: &mut [u32], xs: &[u32], ys: &[u32]) {
    for i in 0..xs.len() {
...
/// ```
pub fn limbs_or_same_length_to_out(out_limbs: &mut [u32], xs: &[u32], ys: &[u32]) {
    let len = xs.len();
    assert_eq!(len, ys.len());
    assert!(out_limbs.len() >= len);
    limbs_or_same_length_to_out_no_check(out_limbs, xs, ys);
}
...
/// assert_eq!(limbs, &[102, 101, 102, 10]);
/// ```
pub fn limbs_or_to_out(out_limbs: &mut [u32], xs: &[u32], ys: &[u32]) {
    let xs_len = xs.len();
    let ys_len = ys.len();
    if xs_len >= ys_len {
        assert!(out_limbs.len() >= xs_len);
        limbs_or_same_length_to_out_no_check(out_limbs, &xs[0..ys_len], ys);
        out_limbs[ys_len..xs_len].copy_from_slice(&xs[ys_len..]);
    } else {
        assert!(out_limbs.len() >= ys_len);
        limbs_or_same_length_to_out_no_check(out_limbs, xs, &ys[0..xs_len]);
        out_limbs[xs_len..ys_len].copy_from_slice(&ys[xs_len..]);
    }
}

fn limbs_or_same_length_in_place_left_no_check(xs: &mut [u32], ys: &[u32]) {
    for i in 0..xs.len() {
        xs[i] |= ys[i];
    }
}
...
/// ```
pub fn limbs_or_same_length_in_place_left(xs: &mut [u32], ys: &[u32]) {
    assert_eq!(xs.len(), ys.len());
    limbs_or_same_length_in_place_left_no_check(xs, ys);
}
...
/// assert_eq!(xs, &[102, 101, 102]);
/// ```
pub fn limbs_or_in_place_left(xs: &mut Vec<u32>, ys: &[u32]) {
    let xs_len = xs.len();
    let ys_len = ys.len();
    if xs_len >= ys_len {
        limbs_or_same_length_in_place_left_no_check(&mut xs[0..ys_len], ys);
    } else {
        limbs_or_same_length_in_place_left_no_check(xs, &ys[0..xs_len]);
...
/// ```
pub fn limbs_or_in_place_either(xs: &mut Vec<u32>, ys: &mut Vec<u32>) -> bool {
    let xs_len = xs.len();
    let ys_len = ys.len();
    if xs_len >= ys_len {
        limbs_or_same_length_in_place_left_no_check(&mut xs[0..ys_len], ys);
        ...
        limbs_or_same_length_in_place_left_no_check(&mut ys[0..xs_len], xs);
        true
    }
}
