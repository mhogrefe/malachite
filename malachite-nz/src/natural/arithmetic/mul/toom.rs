...
pub fn _limbs_mul_greater_to_out_toom_32(
    ...
    if s + t > n {
        ...
        if hi < 0 {
            ...
            assert!(!limbs_sub_limb_in_place(out_hi, Limb::exact_from(-hi)));
        } else {
            assert!(!limbs_slice_add_limb_in_place(out_hi, Limb::exact_from(hi)));
        }
    } else {
        assert_eq!(hi, 0);
    }
    ...
    let m = n + 1;
    {
        split_into_chunks_mut!(out, m, [bs1, bsm2, bs2, as2, as1], _unused);
        {
            ...
            *bsm2_last = 0;
            ...
            } else if slice_test_zero(&bsm1[t..])
...
pub fn _limbs_mul_greater_to_out_toom_6h_input_sizes_valid(xs_len: usize, ys_len: usize) -> bool {
    ...
    if xs_len * TOOM_6H_LIMIT_DENOMINATOR < TOOM_6H_LIMIT_NUMERATOR * ys_len {
        ...
        if half {
            ...
            if s < 1 {
                s += isize::exact_from(n);
                half = false;
            } else if t < 1 {
                t += isize::exact_from(n);
                half = false;
            }
        }
    }
...
pub fn _limbs_mul_greater_to_out_toom_6h(
    ...
    assert!(0 < s && s <= isize::exact_from(n));
    assert!(0 < t && t <= isize::exact_from(n));
    assert!(half || s + t > 3);
    assert!(n > 2);
    let s = usize::exact_from(s);
    let t = usize::exact_from(t);
    let m = n + 1;
    let r = 2 * n + 1;
...
pub fn _limbs_mul_greater_to_out_toom_8h_input_sizes_valid(xs_len: usize, ys_len: usize) -> bool {
        ...
        || xs_len * (TOOM_8H_LIMIT_DENOMINATOR >> 1) < TOOM_8H_LIMIT_NUMERATOR * (ys_len >> 1)
    {
        ...
        half = false;
        n = 1 + ((xs_len - 1) >> 3);
        s = isize::exact_from(xs_len) - 7 * isize::exact_from(n);
        t = isize::exact_from(ys_len) - 7 * isize::exact_from(n);
    } else {
