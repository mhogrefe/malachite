    ...
    let (mut diff, sticky_bit, round_bit, neg) = if x_exp == y_exp {
        ...
        if exp_diff < Limb::WIDTH {
            ...
            if a0 == 0 {
                ...
                (a0, sticky_bit & NOT_LIMB_HIGH_BIT, round_bit, neg)
            }
        } else {
            // We compute x - ulp(x)
            if x > LIMB_HIGH_BIT {
                ...
                if exp_diff == Limb::WIDTH && y > LIMB_HIGH_BIT {
                    ...
                    (
                        LIMB_MAX_MINUS_1,
                        y.wrapping_neg() << 2,
                        Limb::from(y.wrapping_neg() >= HALF_LIMB_HIGH_BIT),
                        neg,
                    )
                } else {
                    // cases (a), (c), (d) and (e)
    ...
    let (mut diff_0, mut diff_1, sticky_bit, round_bit, shift_bit, neg) = if x_exp == y_exp {
        ...
        if exp_diff < Limb::WIDTH {
            ...
            let diff_1 = if a1 < LIMB_HIGH_BIT {
                let diff_1 = (a1 << 1) | (a0 >> WIDTH_MINUS_1);
                a0 = (a0 << 1) | (sticky_bit >> WIDTH_MINUS_1);
                sticky_bit <<= 1;
                x_exp.saturating_sub_assign(1);
                diff_1
            } else {
        ...
        if !goto_sub_d1_no_lose && !goto_sub_d1_lose && (exp_diff == 0 || goto_exact_normalize) {
            ...
            if !goto_sub_d1_lose && (limb > LIMB_HIGH_BIT || goto_sub_d1_no_lose) {
                ...
                loop {
                    ...
                    yl_shifted = ys[l - 1] << WIDTH_MINUS_1;
                    l -= 1;
                    if l == 0 {
                        break;
                    }
            ...
            if exp_diff == prec {
                ...
                if exp_diff == prec + 1 {
                    round_bit_2 = 1;
                    sticky_bit_2 = ys[nm1] & LIMB_MAX_HALF;
                    let mut k = nm1;
                    while sticky_bit_2 == 0 && k > 0 {
                        k -= 1;
                        sticky_bit_2 = ys[k];
            ...
            let mut sx_bit = Limb::power_of_2(WIDTH_MINUS_1 - (x & Limb::WIDTH_MASK));
...
fn sub_float_significands_general<'a>(
    ...
    *out_head -= carry;
    ...
    if !goto_end_of_sub && *last_out >> WIDTH_MINUS_1 == 0 {
        // case 1 - varepsilon
        *last_out = LIMB_HIGH_BIT;
        add_exp = true;
    }
