...
// These functions are adapted from binvert_limb, mpn_divexact_1, mpn_bdiv_dbm1c, and
...
/// Finds the inverse of a `u32` mod 2<sup>32</sup>; given x, returns y such that
/// x * y === 1 mod 2<sup>32</sup>. This inverse only exists for odd `u32`s, so `limb` must be odd.
///
/// Time: worst case O(1)
///
...
/// # Examples
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_invert_limb;
///
...
/// slice must be nonempty. The `Natural` must be exactly divisible by the `u32`. If it isn't, the
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_limb;
///
...
/// must be nonempty. The `Natural` must be exactly divisible by the `u32`. If it isn't, the
/// behavior of this function is undefined.
///
/// Time: worst case O(n)
///
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_limb_to_out;
///
...
/// the `u32`. If it isn't, the behavior of this function is undefined.
///
/// Time: worst case O(n)
///
/// Additional memory: worst case O(1)
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_limb_in_place;
///
...
const MAX_OVER_3: u32 = u32::MAX / 3;
const LIMB_INVERSE_3: u32 = (MAX_OVER_3 << 1) + 1;
const CEIL_MAX_OVER_3: u32 = MAX_OVER_3 + 1;
const CEIL_2_MAX_OVER_3: u32 = ((u32::MAX >> 1) / 3 + 1) | (1 << (u32::WIDTH - 1));
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_3;
///
...
/// ```
pub fn limbs_div_exact_3(limbs: &[u32]) -> Vec<u32> {
    let mut quotient = vec![0; limbs.len()];
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_3_to_out;
///
/// let mut out_limbs = vec![10, 10, 10, 10];
...
/// assert_eq!(out_limbs, &[0x5555_5555, 0x5555_5555, 10, 10]);
/// ```
pub fn limbs_div_exact_3_to_out(out_limbs: &mut [u32], in_limbs: &[u32]) {
    const MAX_OVER_3_U64: u64 = MAX_OVER_3 as u64;
    let len = in_limbs.len();
    assert!(len > 0);
    assert!(out_limbs.len() >= len);
...
pub fn _limbs_div_exact_3_to_out_alt(out_limbs: &mut [u32], in_limbs: &[u32]) {
    let len = in_limbs.len();
    assert!(len > 0);
    assert!(out_limbs.len() >= len);
    let last_index = len - 1;
...
/// # Example
/// ```
/// use malachite_nz::natural::arithmetic::div_exact_u32::limbs_div_exact_3_in_place;
///
...
/// assert_eq!(limbs, &[0x5555_5555, 0x5555_5555]);
/// ```
pub fn limbs_div_exact_3_in_place(limbs: &mut [u32]) {
    const MAX_OVER_3_U64: u64 = MAX_OVER_3 as u64;
    let len = limbs.len();
    assert!(len > 0);
    let last_index = len - 1;
...
// Benchmarks show that this algorithm is always worse than the default.
pub fn _limbs_div_exact_3_in_place_alt(limbs: &mut [u32]) {
    let len = limbs.len();
    assert!(len > 0);
    let last_index = len - 1;
    let mut big_carry = 0;
...
impl Natural {
    pub fn _div_exact_no_special_case_3(&self, other: u32) -> Natural {
        if other == 0 {
            panic!("division by zero");
        } else if other == 1 {
            self.clone()
    ...
    pub fn _div_exact_assign_no_special_case_3(&mut self, other: u32) {
        if other == 0 {
            panic!("division by zero");
        } else if other != 1 {
            match *self {
