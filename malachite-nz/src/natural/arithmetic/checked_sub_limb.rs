...
/// Subtracts a `Limb` from a `Natural`, taking the `Natural` by reference. If the `Limb` is greater
...
/// Subtracts a `Natural` from a `Limb`, taking the `Natural` by value. If the `Natural` is greater
/// than the `Limb`, returns `None`.
///
/// Time: worst case O(1)
///
/// Additional memory: worst case O(1)
