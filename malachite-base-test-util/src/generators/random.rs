...
pub fn random_primitive_float_gen_var_13<T: PrimitiveFloat + RoundingFrom<U>, U: PrimitiveInt>(
    ...
    Box::new(
        random_primitive_ints::<U>(EXAMPLE_SEED).map(|n| T::rounding_from(n, RoundingMode::Down)),
    )
}
        ...
        random_union2s(
            ...
            &|seed| {
                ...
                .filter_map(|a| {
                    let f = U::rounding_from(a, RoundingMode::Down);
                    let a = T::checked_from(f)?;
                    let b = T::checked_from(f.next_lower())?;
                    let diff = a - b;
                    if diff.even() {
                        // This happens almost always
                        Some(a - (diff >> 1))
...
pub fn random_unsigned_vec_pair_gen_var_2<T: PrimitiveUnsigned>(
    ...
    Box::new(UnsignedVecSqrtGenerator {
        ...
        .flat_map(|(x, y)| {
            ...
            let mut out_len: usize = in_len.shr_round(1, RoundingMode::Ceiling);
...
pub fn random_unsigned_vec_triple_gen_var_1<T: PrimitiveUnsigned>(
    config: &GenConfig,
) -> It<(Vec<T>, Vec<T>, Vec<T>)> {
