...
fn f(x: Rational, y: Rational, n: u64) -> i64 {
    ...
    // The standard library's spelling of the guard is the same guard: flagged. (The body differs
    // from the previous one so that `collapse_adjacent_ifs` does not also fire.)
    if n.is_power_of_two() {
        return (n.floor_log_base_2() + 1) as i64;
    }
