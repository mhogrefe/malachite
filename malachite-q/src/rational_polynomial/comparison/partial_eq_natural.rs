...
impl PartialEq<Natural> for RationalPolynomial {
    /// Determines whether a [`RationalPolynomial`] is equal to a [`Natural`].
    ///
    /// The polynomial is equal to the [`Natural`] when it is the constant polynomial with that
    /// value, so the zero polynomial is equal to 0 and nothing else, and no polynomial with a
    /// non-integer or negative coefficient, or of positive degree, is equal to any [`Natural`].
    ///
...
impl PartialEq<RationalPolynomial> for Natural {
    /// Determines whether a [`Natural`] is equal to a [`RationalPolynomial`].
    ///
