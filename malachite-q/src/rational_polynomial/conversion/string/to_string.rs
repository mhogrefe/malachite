...
// Writes one coefficient. Unlike an integer, a `Rational` is not spelled the same way in every
// language: math mode stacks a fraction rather than writing a slash.
fn write_coefficient<W: Write>(c: &Rational, language: Language, w: &mut W) -> Result {
    match language {
        Language::Plain => write!(w, "{c}"),
        Language::Latex => write!(w, "{}", c.to_latex()),
        Language::Typst => write!(w, "{}", c.to_typst()),
    }
}

impl RationalPolynomial {
    // Writes a `RationalPolynomial` whose variable is named by `var`, in one of three languages.
    //
    ...
    pub(crate) fn write_with_var<W: Write, S: VarScheme + ?Sized>(
        ...
        let coefficients = self.to_coefficients_asc();
        ...
        for (exponent, coefficient) in coefficients.iter().enumerate().rev() {
            ...
            if exponent == 0 {
                write_coefficient(coefficient, language, w)?;
                continue;
            }
...
impl Display for RationalPolynomial {
    /// Converts an [`RationalPolynomial`] to a [`String`].
    ///
    /// The variable is called `x`. [`to_string_with`](RationalPolynomial::to_string_with) is the
    ...
    /// assert_eq!(
    ///     RationalPolynomial::from_str("2*x^3").unwrap().to_string(),
    ///     "2*x^3"
    /// );
    ///
    ...
    /// );
    /// assert_eq!(
    ///     RationalPolynomial::from_str("x^1").unwrap().to_string(),
    ///     "x"
    /// );
    /// ```
    #[inline]
