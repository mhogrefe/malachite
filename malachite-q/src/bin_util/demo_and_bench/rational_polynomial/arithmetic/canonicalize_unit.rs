...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_rational_polynomial_canonicalize_unit);
    register_demo!(runner, demo_rational_polynomial_canonicalize_unit_assign);
    register_bench!(
        runner,
        benchmark_rational_polynomial_canonicalize_unit_evaluation_strategy
    );
}

fn demo_rational_polynomial_canonicalize_unit(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in rational_polynomial_gen().get(gm, config).take(limit) {
        println!("({p}).canonicalize_unit() = {}", (&p).canonicalize_unit());
    }
}

fn demo_rational_polynomial_canonicalize_unit_assign(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
...
fn benchmark_rational_polynomial_canonicalize_unit_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "RationalPolynomial.canonicalize_unit()",
        ...
        &mut [
            ("RationalPolynomial.canonicalize_unit()", &mut |p| {
            ...
            (
                "RationalPolynomial.canonicalize_unit_assign()",
                &mut |mut p| {
                    p.canonicalize_unit_assign();
                },
            ),
