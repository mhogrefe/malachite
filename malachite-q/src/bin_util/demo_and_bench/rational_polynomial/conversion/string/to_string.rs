...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_rational_polynomial_to_string);
    register_demo!(runner, demo_rational_polynomial_to_string_with);
    register_bench!(runner, benchmark_rational_polynomial_to_string);
    register_bench!(
        runner,
        benchmark_rational_polynomial_to_string_by_language_algorithms
    );
}

fn demo_rational_polynomial_to_string(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in rational_polynomial_gen().get(gm, config).take(limit) {
        println!("{p}");
    }
}

fn demo_rational_polynomial_to_string_with(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in rational_polynomial_gen().get(gm, config).take(limit) {
        println!(
            "{} is {} and {}",
            p,
...
fn benchmark_rational_polynomial_to_string(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
...
fn benchmark_rational_polynomial_to_string_by_language_algorithms(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
