...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_gaussian_rational_is_gaussian_integer);
    register_bench!(runner, benchmark_gaussian_rational_is_gaussian_integer);
}

fn demo_gaussian_rational_is_gaussian_integer(gm: GenMode, config: &GenConfig, limit: usize) {
    for n in gaussian_rational_gen().get(gm, config).take(limit) {
...
fn benchmark_gaussian_rational_is_gaussian_integer(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "GaussianRational.is_gaussian_integer()",
