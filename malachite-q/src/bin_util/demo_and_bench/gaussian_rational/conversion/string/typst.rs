...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_gaussian_rational_to_typst);
    register_bench!(runner, benchmark_gaussian_rational_to_typst_string);
}

fn demo_gaussian_rational_to_typst(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in gaussian_rational_gen().get(gm, config).take(limit) {
        println!("{}.to_typst() = {}", x, x.to_typst());
    }
}

fn benchmark_gaussian_rational_to_typst_string(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "GaussianRational.to_typst_string()",
