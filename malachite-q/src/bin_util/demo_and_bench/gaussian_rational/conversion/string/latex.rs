...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_gaussian_rational_to_latex);
    register_bench!(runner, benchmark_gaussian_rational_to_latex_string);
}

fn demo_gaussian_rational_to_latex(gm: GenMode, config: &GenConfig, limit: usize) {
    for x in gaussian_rational_gen().get(gm, config).take(limit) {
        println!("{}.to_latex() = {}", x, x.to_latex());
    }
}

fn benchmark_gaussian_rational_to_latex_string(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "GaussianRational.to_latex_string()",
