...
impl Conjugate for Rational {
    type Output = Self;

    /// Computes the complex conjugate of a [`Rational`], taking it by value. A real number is its
...
impl Conjugate for &Rational {
    ...
    /// Computes the complex conjugate of a [`Rational`], taking it by reference. A real number is
...
impl ConjugateAssign for Rational {
    /// Replaces a [`Rational`] with its complex conjugate. A real number is its own conjugate, so
    /// this does nothing.
    ///
    /// $$
    /// x \gets \overline{x} = x.
