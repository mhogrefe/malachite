    ...
    Natural: From<T> + PartialOrd<T>,
{
    ...
    // In this generic context a plain `partial_cmp(&1u32)` would resolve to the `Natural:
    // PartialOrd<T>` bound instead of the `u32` impl, so the `Natural` constant is compared
    // directly.
