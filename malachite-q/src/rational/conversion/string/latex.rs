...
fn fmt_latex_unsigned(q: &Rational, f: &mut Formatter) -> Result {
...
impl ToLatex for Rational {
    /// Writes a [`Rational`] as a LaTeX math-mode fragment.
    ///
    ...
    /// use std::str::FromStr;
    ///
    /// assert_eq!(Rational::from(0).to_latex_string(), "0");
    /// assert_eq!(Rational::from(123).to_latex_string(), "123");
    /// assert_eq!(Rational::from(-123).to_latex_string(), "-123");
    /// assert_eq!(
    ///     Rational::from_str("22/7").unwrap().to_latex_string(),
    ///     r"\frac{22}{7}"
    /// );
    /// assert_eq!(
    ///     Rational::from_str("-2/3").unwrap().to_latex_string(),
    ///     r"-\frac{2}{3}"
    /// );
    /// ```
    ///
    ...
    /// |------------------------------|----------------|----------------|
    /// | `Rational::from(123)`        | `123`          | $123$          |
    /// | `Rational::from_str("22/7")` | `\frac{22}{7}` | $\frac{22}{7}$ |
    /// | `Rational::from_str("-2/3")` | `-\frac{2}{3}` | $-\frac{2}{3}$ |
    #[inline]
    fn fmt_latex(&self, f: &mut Formatter) -> Result {
        if !self.sign {
            f.write_char('-')?;
        }
        fmt_latex_unsigned(self, f)
    }
}
