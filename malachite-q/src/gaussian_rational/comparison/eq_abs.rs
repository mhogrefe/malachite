...
impl EqAbs for GaussianRational {
    /// Determines whether the absolute values of two [`GaussianRational`]s are equal.
    ///
    ...
    /// use std::str::FromStr;
    ///
    /// let x = GaussianRational::from_str("1/2+i").unwrap();
    /// let y = GaussianRational::from_str("-1+i/2").unwrap();
    /// assert!(x.eq_abs(&y));
    ///
    /// let x = GaussianRational::from_str("1+i").unwrap();
    /// let y = GaussianRational::from_str("3i/2").unwrap();
    /// // |1+i|^2 = 2 and |3i/2|^2 = 9/4
