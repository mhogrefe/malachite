...
macro_rules! impl_unsigned {
    ($t: ident) => {
        impl PartialOrdAbs<$t> for GaussianRational {
            /// Compares the absolute values of a [`GaussianRational`] and an unsigned primitive
            /// integer.
            ///
            ...
            fn partial_cmp_abs(&self, other: &$t) -> Option<Ordering> {
                if self.imaginary == 0u32 {
                    ...
                    Some(
                        self.abs_squared()
                            .cmp(&Rational::from(*other).abs_squared()),
                    )
                }
            }
        }
...
macro_rules! impl_signed {
    ($t: ident) => {
        impl PartialOrdAbs<$t> for GaussianRational {
            /// Compares the absolute values of a [`GaussianRational`] and a signed primitive
            /// integer.
            ///
            /// # Worst-case complexity
            /// $T(n) = O(n \log n \log\log n)$
