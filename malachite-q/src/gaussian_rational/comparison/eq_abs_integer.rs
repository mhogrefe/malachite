...
impl EqAbs<Integer> for GaussianRational {
    /// Determines whether the absolute values of a [`GaussianRational`] and an [`Integer`] are
    /// equal.
    ///
...
impl EqAbs<GaussianRational> for Integer {
    /// Determines whether the absolute values of an [`Integer`] and a [`GaussianRational`] are
    /// equal.
    ///
    /// # Worst-case complexity
    /// $T(n) = O(n \log n \log\log n)$
