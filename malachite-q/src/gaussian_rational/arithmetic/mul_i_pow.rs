...
impl MulIPow for GaussianRational {
    type Output = Self;

    /// Multiplies a [`GaussianRational`] by $i^k$, taking the [`GaussianRational`] by value.
    ///
...
impl MulIPow for &GaussianRational {
    ...
    /// Multiplies a [`GaussianRational`] by $i^k$, taking the [`GaussianRational`] by reference.
    ///
    /// Only $k$ modulo 4 matters: $i^0 = 1$, $i^1 = i$, $i^2 = -1$, and $i^3 = -i$, so the result
    /// is the number itself, a counterclockwise quarter turn, a half turn, or a clockwise quarter
    /// turn. Since $i^{-k} = i^{3k}$, a negative power is a matter of tripling the exponent.
    ...
    /// ```
    #[inline]
    fn mul_i_pow(self, k: u64) -> GaussianRational {
        self.clone().mul_i_pow(k)
    }
}

impl MulIPowAssign for GaussianRational {
    /// Multiplies a [`GaussianRational`] by $i^k$ in place.
    ///
    /// Only $k$ modulo 4 matters: $i^0 = 1$, $i^1 = i$, $i^2 = -1$, and $i^3 = -i$, so the result
    /// is the number itself, a counterclockwise quarter turn, a half turn, or a clockwise quarter
    /// turn. Since $i^{-k} = i^{3k}$, a negative power is a matter of tripling the exponent.
