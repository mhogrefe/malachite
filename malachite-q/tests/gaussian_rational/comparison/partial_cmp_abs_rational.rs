...
fn test_partial_cmp_abs_rational() {
    ...
    test("0", "0", Some(Equal));
    test("0", "1/2", Some(Less));
    test("i", "1", Some(Equal));
    test("i/2", "-1/2", Some(Equal));
    test("3/5+4i/5", "1", Some(Equal));
    test("3/5+4i/5", "-4/5", Some(Greater));
    test("3/5+4i/5", "6/5", Some(Less));
    test("3/2+2i", "5/2", Some(Equal));
    test("1/2+i/2", "1", Some(Less));
    test("1/2+i/2", "1/2", Some(Greater));
    test("-22/7", "22/7", Some(Equal));
}
...
fn partial_cmp_abs_rational_properties() {
    gaussian_rational_rational_pair_gen().test_properties(|(x, y)| {
        ...
        assert_eq!((-&x).partial_cmp_abs(&y), cmp);
        assert_eq!(x.partial_cmp_abs(&-&y), cmp);
        assert_eq!(Some(x.cmp_abs(&GaussianRational::from(y.clone()))), cmp);
    });
