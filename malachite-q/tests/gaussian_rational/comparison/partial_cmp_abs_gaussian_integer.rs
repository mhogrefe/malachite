...
fn test_partial_cmp_abs_gaussian_integer() {
    ...
    test("0", "0", Some(Equal));
    test("i", "1", Some(Equal));
    test("2-i", "1+2i", Some(Equal));
    test("2-i", "2+2i", Some(Less));
    test("2-i", "2", Some(Greater));
    test("3/5+4i/5", "1", Some(Equal));
    test("3/5+4i/5", "i", Some(Equal));
    test("3/5+4i/5", "1+i", Some(Less));
    test("1/2+i/2", "1", Some(Less));
    test("3+4i", "4-3i", Some(Equal));
    test("3+4i", "5+i", Some(Less));
    test("1/2+2i", "1+2i", Some(Less));
}
...
fn partial_cmp_abs_gaussian_integer_properties() {
    ...
    gaussian_integer_pair_gen().test_properties(|(x, y)| {
        assert_eq!(
            GaussianRational::from(&x).partial_cmp_abs(&y),
            Some(x.cmp_abs(&y))
        );
        assert_eq!(
            x.partial_cmp_abs(&GaussianRational::from(&y)),
            Some(x.cmp_abs(&y))
        );
    });
}
