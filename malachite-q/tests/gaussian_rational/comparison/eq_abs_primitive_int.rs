...
fn eq_abs_primitive_int_properties_helper_unsigned<T: EqAbs<GaussianRational> + PrimitiveUnsigned>()
where
    GaussianRational: EqAbs<T> + From<T>,
    Rational: From<T>,
{
    gaussian_rational_unsigned_pair_gen::<T>().test_properties(|(x, u)| {
        let eq = x.eq_abs(&u);
        assert_eq!(u.eq_abs(&x), eq);
        assert_eq!((&x).abs_squared() == Rational::from(u).abs_squared(), eq);
        assert_eq!((&x).conjugate().eq_abs(&u), eq);
        assert_eq!((-&x).eq_abs(&u), eq);
        assert_eq!(
            <GaussianRational as EqAbs>::eq_abs(&x, &GaussianRational::from(u)),
            eq
        );
    });
}

fn eq_abs_primitive_int_properties_helper_signed<T: EqAbs<GaussianRational> + PrimitiveSigned>()
where
    GaussianRational: EqAbs<T> + From<T>,
    Rational: From<T>,
{
    gaussian_rational_signed_pair_gen::<T>().test_properties(|(x, i)| {
        ...
        assert_eq!((&x).abs_squared() == Rational::from(i).abs_squared(), eq);
        assert_eq!((&x).conjugate().eq_abs(&i), eq);
        assert_eq!((-&x).eq_abs(&i), eq);
        assert_eq!(
            <GaussianRational as EqAbs>::eq_abs(&x, &GaussianRational::from(i)),
            eq
        );
    });
}
