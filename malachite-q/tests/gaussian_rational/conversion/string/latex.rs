...
#[test]
fn test_gaussian_rational_to_latex() {
    let test = |x: GaussianRational, out: &str| assert_eq!(x.to_latex_string(), out);
...
#[test]
fn gaussian_rational_to_latex_properties() {
    gaussian_rational_gen().test_properties(|x| {
        ...
        assert_eq!(ComparableGaussianRational(x.clone()).to_latex_string(), s);
        assert_eq!(ComparableGaussianRationalRef(&x).to_latex_string(), s);
    });
}
