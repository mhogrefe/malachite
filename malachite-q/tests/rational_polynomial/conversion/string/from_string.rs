...
fn test_from_str_fail() {
    let test = |s| {
        assert!(
            RationalPolynomial::from_str(s).is_err(),
            "{s:?} should not parse"
        );
    };
    test("");
    ...
    test("x^0");
    // A `+` separates terms, so it is never written at the front, and two signs in a row leave a
    // term with nothing in it.
    test("x--1");
    test("x+-1");
    test("-");
    test("x-");
...
fn test_from_string_with() {
    ...
    assert_eq!(
        RationalPolynomial::from_string_with(GreekVars.var(0), "α^2+3*α+2").unwrap(),
        p
    );
    assert_eq!(
        RationalPolynomial::from_string_with(XyzVars.var(1), "y^2+3*y+2").unwrap(),
        p
    );
    ...
    assert!(RationalPolynomial::from_string_with(GreekVars.var(0), "x^2").is_none());
    assert!(RationalPolynomial::from_string_with(XyzVars.var(1), "x^2").is_none());
}
...
fn from_str_properties() {
    rational_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(RationalPolynomial::from_str(&p.to_string()).unwrap(), p);
        let greek = p.to_string_with(GreekVars.var(0));
        assert_eq!(
            RationalPolynomial::from_string_with(GreekVars.var(0), &greek).unwrap(),
            p
        );
        // `FromStr` is `from_string_with` with the first `XyzVars` variable.
        assert_eq!(
            RationalPolynomial::from_string_with(XyzVars.var(0), &p.to_string()).unwrap(),
            p
        );
    });
    ...
    string_gen().test_properties(|s| {
        ...
        if let Ok(p) = RationalPolynomial::from_str(&s) {
            assert!(p.is_valid());
            assert_eq!(RationalPolynomial::from_str(&p.to_string()).unwrap(), p);
        }
    });
