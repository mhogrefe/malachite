...
fn test_to_string() {
    let test = |s| {
        assert_eq!(RationalPolynomial::from_str(s).unwrap().to_string(), s);
    };
    test("0");
    test("1");
    test("5");
...
fn test_to_string_normalizes() {
    ...
    let test = |s, out| {
        assert_eq!(RationalPolynomial::from_str(s).unwrap().to_string(), out);
    };
...
fn to_string_properties() {
    rational_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(RationalPolynomial::from_str(&s).unwrap(), p);
        assert!(!s.is_empty());
        ...
        assert_eq!(
            s.contains('-'),
            p.to_coefficients_asc().iter().any(|c| *c < 0u32)
        );
        assert!(!s.ends_with('-') && !s.ends_with('+'));
        assert_eq!(s == "0", p == RationalPolynomial::ZERO);
        ...
        assert_eq!(
            RationalPolynomial::from_string_with(vars.var(0), &named).unwrap(),
            p
        );
