...
fn test_coefficients_asc() {
    let test = |s, out| {
        ...
        assert_eq!(p.to_coefficients_asc().to_debug_string(), out);
...
fn test_degree() {
    let test = |s, out| {
        assert_eq!(RationalPolynomial::from_str(s).unwrap().degree(), out);
    };
    test("0", None);
    test("1", Some(0));
    test("5", Some(0));
...
fn test_truncate() {
    ...
    test("0", 3, "0");
    test("0", u64::MAX, "0");
    test("1/2*x^3+1/3*x^2+1/4*x+1/5", 0, "0");
    test("1/2*x^3+1/3*x^2+1/4*x+1/5", 1, "1/5");
    test("1/2*x^3+1/3*x^2+1/4*x+1/5", 2, "1/4*x+1/5");
    test("1/2*x^3+1/3*x^2+1/4*x+1/5", 4, "1/2*x^3+1/3*x^2+1/4*x+1/5");
    test(
    ...
    test("1/2*x^2+x+1", 2, "x+1");
    test("1/2*x^2+x+1", 1, "1");
    test("1/6*x^2+1/2*x+1/3", 2, "1/2*x+1/3");
    test("1/2*x^3+1/3*x+1/5", 3, "1/3*x+1/5");
    test("1/2*x^5+x", 5, "x");
    test("1/2*x^5+x", 1, "0");
}
...
fn coefficients_properties() {
    rational_polynomial_gen().test_properties(|p| {
        let cs = p.to_coefficients_asc().to_vec();
        // The coefficients are what the polynomial is: they build it back.
        let q = RationalPolynomial::from_coefficients_asc(cs.clone());
        assert!(q.is_valid());
        assert_eq!(q, p);
    ...
    rational_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, i)| {
        ...
        assert_eq!(
            ...
            p.to_coefficients_asc().len() > usize::exact_from(i)
        );
