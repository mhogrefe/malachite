...
use malachite_base::num::conversion::traits::{ConvertibleFrom, ExactFrom, IsInteger, IsReal};
...
fn test_try_from_gaussian_integer() {
    ...
    test("i", "Err(RationalFromGaussianIntegerError)");
    test("-i", "Err(RationalFromGaussianIntegerError)");
    test("2-3i", "Err(RationalFromGaussianIntegerError)");
}
...
#[test]
#[should_panic]
fn rational_exact_from_gaussian_integer_fail() {
    Rational::exact_from(GaussianInteger::from_str("2-3i").unwrap());
}

#[test]
#[should_panic]
fn rational_exact_from_gaussian_integer_ref_fail() {
    Rational::exact_from(&GaussianInteger::from_str("i").unwrap());
}
...
fn try_from_gaussian_integer_properties() {
    ...
    gaussian_integer_gen_var_1().test_properties(|x| {
        ...
        assert_eq!(Rational::exact_from(&x), Rational::from(&x.real));
    });
}
