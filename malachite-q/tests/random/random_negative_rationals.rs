...
fn random_negative_rationals_helper(
    ...
    let xs = random_negative_rationals(EXAMPLE_SEED, mean_bits_numerator, mean_bits_denominator);
    ...
    let actual_sample_moment_stats = moment_stats(xs.take(1000000).map(f64::from));
    assert_eq!(
        (
