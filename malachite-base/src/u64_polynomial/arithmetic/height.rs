...
impl Height for U64Polynomial {
    type Output = u64;

    /// Returns the height of a [`U64Polynomial`]: the largest of its coefficients.
    ///
    ...
    ///
    /// assert_eq!(U64Polynomial::from_str("x^2+3*x+2").unwrap().to_height(), 3);
    /// assert_eq!(U64Polynomial::from_str("x^100").unwrap().to_height(), 1);
    /// assert_eq!(U64Polynomial::from_str("0").unwrap().to_height(), 0);
    /// ```
    ///
    ...
    #[inline]
    fn to_height(&self) -> u64 {
        self.coefficients_asc().iter().copied().max().unwrap_or(0)
    }

    /// Returns the height of a [`U64Polynomial`], taking it by value.
    ///
    ...
    ///
    /// assert_eq!(
    ///     U64Polynomial::from_str("x^2+3*x+2").unwrap().into_height(),
    ///     3
    /// );
    /// assert_eq!(U64Polynomial::from_str("0").unwrap().into_height(), 0);
    /// ```
    #[inline]
    fn into_height(self) -> u64 {
        self.to_height()
    }

    /// Returns the number of significant bits of the height of a [`U64Polynomial`].
    ///
