...
use crate::u64_polynomial::conversion::string::to_string::Language;
...
impl U64Polynomial {
    /// Converts a [`U64Polynomial`] to a LaTeX math-mode fragment, naming its variable with any
    /// [`VarScheme`].
    ///
...
impl ToLatex for U64Polynomial {
    /// Writes a [`U64Polynomial`] as a LaTeX math-mode fragment.
    ///
    /// The variable is called `x`. [`to_latex_string_with`](U64Polynomial::to_latex_string_with) is
    ...
    ///     "x^2+3x+2"
    /// );
    /// assert_eq!(U64Polynomial::from_str("0").unwrap().to_latex_string(), "0");
    /// assert_eq!(U64Polynomial::from_str("5").unwrap().to_latex_string(), "5");
    /// assert_eq!(U64Polynomial::from_str("x").unwrap().to_latex_string(), "x");
    /// assert_eq!(
    ///     U64Polynomial::from_str("2*x^3").unwrap().to_latex_string(),
    ///     "2x^3"
    /// );
    ///
    /// // An exponent of more than one digit is braced.
    /// assert_eq!(
    ///     U64Polynomial::from_str("x^12+x^2")
    ///         .unwrap()
    ///         .to_latex_string(),
    ///     "x^{12}+x^2"
    /// );
