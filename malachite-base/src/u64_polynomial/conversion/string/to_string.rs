...
impl U64Polynomial {
    // Writes a `U64Polynomial` whose variable is named by `var`, in one of three languages.
    //
    ...
    /// Converts a [`U64Polynomial`] to a [`String`], naming its variable with any [`VarScheme`].
    ///
...
impl Display for U64Polynomial {
    /// Converts a [`U64Polynomial`] to a [`String`].
    ///
    /// The variable is called `x`. [`to_string_with`](U64Polynomial::to_string_with) is the way to
    ...
    /// assert_eq!(
    ///     U64Polynomial::from_str("2*x^3").unwrap().to_string(),
    ///     "2*x^3"
    /// );
    ///
    ...
    ///     "x^2+3*x+2"
    /// );
    /// assert_eq!(U64Polynomial::from_str("x^1").unwrap().to_string(), "x");
    /// ```
    #[inline]
