...
impl U64Polynomial {
    /// Converts a [`U64Polynomial`] to a Typst math-mode fragment, naming its variable with any
    /// [`VarScheme`].
    ///
...
impl ToTypst for U64Polynomial {
    /// Writes a [`U64Polynomial`] as a Typst math-mode fragment.
    ///
    /// The variable is called `x`. [`to_typst_string_with`](U64Polynomial::to_typst_string_with) is
    ...
    ///     "x^2+3x+2"
    /// );
    /// assert_eq!(U64Polynomial::from_str("0").unwrap().to_typst_string(), "0");
    /// assert_eq!(U64Polynomial::from_str("5").unwrap().to_typst_string(), "5");
    /// assert_eq!(U64Polynomial::from_str("x").unwrap().to_typst_string(), "x");
    /// assert_eq!(
    ///     U64Polynomial::from_str("2*x^3").unwrap().to_typst_string(),
    ///     "2x^3"
    /// );
    ///
