...
/// Generates random `HashSet`s of a fixed length, where the `Vec`s have no repeated elements, and
/// the elements are in ascending order.
///
/// This `struct` is created by the `random_hash_sets_fixed_length` function. See its documentation
/// for more.
#[derive(Clone, Debug)]
...
/// Generates random `BTreeSet`s of a fixed length, where the `Vec`s have no repeated elements, and
/// the elements are in ascending order.
///
/// This `struct` is created by the `random_b_tree_sets_fixed_length` function. See its
...
///
/// The lengths of the `BTreeSet`s are sampled from a uniform distribution on $[a, b)$. $a$ must be
/// less than or equal to $b$.
///
/// The input iterator must generate at least $b$ distinct elements.
///
