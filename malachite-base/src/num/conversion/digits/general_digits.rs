...
pub fn _unsigned_to_digits_asc_naive<
    T: Copy + DivAssignMod<T, ModOutput = T> + Eq + ExactFrom<u64> + Zero,
    D: WrappingFrom<T>,
>(
    x: &T,
    base: u64,
    ...
    while remainder != T::ZERO {
        digits.push(D::wrapping_from(remainder.div_assign_mod(base)));
    }
    digits
}

fn _unsigned_to_digits_asc<
    T: ConvertibleFrom<u64>
        + Copy
        ...
        + PowerOfTwoDigits<D>
        + Zero,
    D: ConvertibleFrom<u64> + PrimitiveUnsigned + WrappingFrom<T>,
>(
    x: &T,
    base: u64,
) -> Vec<D> {
    assert!(T::convertible_from(base));
    assert!(D::convertible_from(base));
    if *x == T::ZERO {
        Vec::new()
...
>(
    ...
    let mut digits = _unsigned_to_digits_asc(x, base);
    digits.reverse();
    digits
}
...
macro_rules! impl_digits {
    ($t:ident) => {
        macro_rules! impl_digits_inner {
            ($u:ident) => {
                impl Digits<$u, u64> for $t {
                    /// Returns a `Vec` containing the digits of `self` in ascending order (least-
                    /// to most-significant).
                    ///
                    /// The type of each digit is `$u`, and `base` must be convertible to both $t$
                    /// and $u$. If `self` is 0, the `Vec` is empty; otherwise, it ends with a
                    /// nonzero digit.
                    ///
                    ...
                    ///
                    /// # Panics
                    /// Panics if `base` is too large to fit in $t$ or $u$, or if `base` is less
                    /// than 2.
                    ///
                    /// # Examples
                    ...
                    /// module.
                    #[inline]
                    fn to_digits_asc(&self, base: u64) -> Vec<$u> {
                    ...
                    ///
                    /// The type of each digit is `$u`, and `base` must be convertible to both $t$
                    /// and $u$. If `self` is 0, the `Vec` is empty; otherwise, it begins with a
                    /// nonzero digit.
                    ///
                    /// $f(x, b) = (d_i)_ {i=0}^{k-1}$, where $0 \leq d_i < b$ for all $i$, $k=0$ or
                    /// $d_{k-1} \neq 0$, and
                    ...
                    /// module.
                    #[inline]
                    fn to_digits_desc(&self, base: u64) -> Vec<$u> {
                        _unsigned_to_digits_desc(self, base)
                    }

                    #[inline]
                    fn from_digits_asc<I: Iterator<Item = $u>>(_base: u64, _digits: I) -> $t {
                        unimplemented!()
                    }

                    fn from_digits_desc<I: Iterator<Item = $u>>(_base: u64, _digits: I) -> $t {
                        unimplemented!()
                    }
                }
            };
