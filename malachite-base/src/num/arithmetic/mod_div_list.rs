...
// must be reduced mod `m`. Unlike a quotient from `mod_div`, the result is canonical: it does not
// depend on the extended GCD's choice of cofactor.
//
...
macro_rules! impl_mod_div_list {
    ($u:ident, $s:ident) => {
        impl ModDivList<$u> for $u {
            ...
            /// $0 \leq i < \text{length}$, where $\text{start}$ is the smallest quotient,
            /// $\text{stride} = m/g$, and $\text{length} = g$. Unlike the quotient returned by
            /// [`ModDiv`](super::traits::ModDiv), the result is canonical.
            ///
            /// $f(x, y, m) = \operatorname{Some}((s, t, \ell))$, where $qy \equiv x \mod m$ if
            /// and only if $q = s + ti$ for some $0 \leq i < \ell$, if such $q$ exist.
            ///
