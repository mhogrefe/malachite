...
use num::arithmetic::traits::{
    ...
    FloorRoot, FloorRootAssign, FloorSqrt, Parity, Pow, PowerOf2, RootAssignRem, RootRem, SqrtRem,
    XMulYIsZZ,
};
...
// This section is generated by max_base.rs.
...
pub_test! {floor_root_binary<T: PrimitiveUnsigned>(x: T, exp: u64) -> T {
    if exp == 0 {
...
pub_test! {ceiling_root_binary<T: PrimitiveUnsigned>(x: T, exp: u64) -> T {
...
pub_test! {checked_root_binary<T: PrimitiveUnsigned>(x: T, exp: u64) -> Option<T> {
    let floor_root = floor_root_binary(x, exp);
    if floor_root.pow(exp) == x {
        Some(floor_root)
    } else {
...
impl FloorRoot<u64> for u8 {
    ...
    ///
    /// # Notes
    /// This implementation uses lookup tables.
    #[inline]
...
impl FloorRoot<u64> for u16 {
    ...
    ///
    /// # Notes
    /// This implementation calls the implementation for `u32`s.
    #[inline]
...
impl FloorRoot<u64> for u32 {
    ...
    ///
    /// # Notes
    /// For cube roots, this implementation uses a piecewise Chebyshev approximation. For other
    /// roots, it uses Newton's method. In both implementations, the result of these approximations
...
impl FloorRoot<u64> for usize {
    ...
    ///
    /// # Notes
    /// This implementation calls the `u32` or `u64` implementations.
    #[inline]
...
impl RootRem for usize {
    type RootOutput = usize;
    type RemOutput = usize;

    /// Returns the floor of the $n$th root of a `usize`, and the remainder (the difference between
    /// the `usize` and the $n$th power of the floor).
    ///
    /// $f(x, n) = (\lfloor\sqrt\[n\]{x}\rfloor, x - \lfloor\sqrt\[n\]{x}\rfloor^2)$.
    ///
    /// # Worst-case complexity
