...
/// by the value on the RHS.
///
/// If it isn't, this function may crash or assign a meaningless value to the first value.
...
/// Replaces `self` with `self.pow(exp)` mod `m`. Assumes the inputs are already reduced mod `m`.
...
/// precompute some piece of data using `precompute_mod_pow_data` and reuse it in the squaring
/// calls.
...
/// Saturating absolute value. Computes `self.abs()`, saturating at the numeric bounds instead of
/// overflowing.
...
/// Saturating absolute value. Replaces `self` with its absolute value, saturating at the numeric
...
/// Saturating negation. Replaces `self` with its negative, saturating at the numeric bounds
/// instead of overflowing.
