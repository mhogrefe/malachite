...
// The correctly rounded (to nearest) values of $n!$ as primitive floats, for every $n$ at which the
// result is finite. Beyond each table the factorial overflows to infinity. The entries were
// generated by rounding the exact `Natural` factorials, and are verified against them by a test in
// malachite-float.
...
macro_rules! impl_factorial_primitive_float {
    ($t:ident, $table:ident) => {
        impl Factorial for $t {
            ...
            /// value, positive infinity is returned; unlike the primitive-integer implementations,
            /// this function does not panic on overflow, since infinity is the correctly rounded
            /// result.
            ///
            /// $$
            /// f(n) = n!+\varepsilon.
            ...
            /// - If $n!$ is finite but not representable, $|\varepsilon| < 2^{\lfloor\log_2
            ///   n!\rfloor-M}$, where $M$ is the mantissa width.
