...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_unsigned_polynomial_canonicalize_unit);
    register_demo!(runner, demo_unsigned_polynomial_canonicalize_unit_assign);
    register_bench!(
        runner,
        benchmark_unsigned_polynomial_canonicalize_unit_evaluation_strategy
    );
}

fn demo_unsigned_polynomial_canonicalize_unit(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in unsigned_polynomial_gen().get(gm, config).take(limit) {
        println!("({p}).canonicalize_unit() = {}", (&p).canonicalize_unit());
    }
}

fn demo_unsigned_polynomial_canonicalize_unit_assign(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
...
fn benchmark_unsigned_polynomial_canonicalize_unit_evaluation_strategy(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "UnsignedPolynomial<u64>.canonicalize_unit()",
        ...
        &mut [
            ("UnsignedPolynomial<u64>.canonicalize_unit()", &mut |p| {
            ...
            (
                "UnsignedPolynomial<u64>.canonicalize_unit_assign()",
                &mut |mut p| {
                    p.canonicalize_unit_assign();
                },
            ),
