...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_unsigned_polynomial_mutate_coefficient);
    register_bench!(runner, benchmark_unsigned_polynomial_mutate_coefficient);
}

fn demo_unsigned_polynomial_mutate_coefficient(gm: GenMode, config: &GenConfig, limit: usize) {
    for (mut p, i) in unsigned_polynomial_unsigned_pair_gen_var_1()
        .get(gm, config)
        .take(limit)
    {
        let old = p.to_string();
...
fn benchmark_unsigned_polynomial_mutate_coefficient(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "UnsignedPolynomial<u64>.mutate_coefficient(u64, FnOnce)",
