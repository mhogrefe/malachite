...
pub(crate) fn register(runner: &mut Runner) {
    register_unsigned_demos!(runner, demo_mod_div_list);
    register_unsigned_benches!(runner, benchmark_mod_div_list);
}

fn demo_mod_div_list<T: ModDivList<T, T, Output = T> + PrimitiveUnsigned>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
) {
    for (x, y, m) in unsigned_triple_gen_var_12::<T>()
        ...
        .take(limit)
    {
        println!("{x}.mod_div_list({y}, {m}) = {:?}", x.mod_div_list(y, m));
    }
}

fn benchmark_mod_div_list<T: ModDivList<T, T, Output = T> + PrimitiveUnsigned>(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        &format!("{}.mod_div_list({}, {})", T::NAME, T::NAME, T::NAME),
