...
pub(crate) fn register(runner: &mut Runner) {
    register_demo!(runner, demo_u64_polynomial_mod_is_reduced);
    register_demo!(runner, demo_u64_polynomial_mod_power_of_2_is_reduced);

    register_bench!(runner, benchmark_u64_polynomial_mod_is_reduced);
    register_bench!(runner, benchmark_u64_polynomial_mod_power_of_2_is_reduced);
}
...
fn demo_u64_polynomial_mod_is_reduced(gm: GenMode, config: &GenConfig, limit: usize) {
...
fn demo_u64_polynomial_mod_power_of_2_is_reduced(gm: GenMode, config: &GenConfig, limit: usize) {
    for p in u64_polynomial_gen().get(gm, config).take(limit) {
        for pow in [0, 1, 8, 64] {
            println!(
...
fn benchmark_u64_polynomial_mod_is_reduced(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "U64Polynomial.mod_is_reduced(&u64)",
...
fn benchmark_u64_polynomial_mod_power_of_2_is_reduced(
    gm: GenMode,
    config: &GenConfig,
    limit: usize,
    file_name: &str,
    ...
    run_benchmark(
        "U64Polynomial.mod_power_of_2_is_reduced(u64)",
        BenchmarkType::Single,
        u64_polynomial_gen().get(gm, config),
        gm.name(),
        limit,
