...
///
/// `is_constant(xs)` is equivalent to `xs.unique().count() == 1` for finite nonempty iterators,
/// but is more efficient, doesn't require [`Clone`] or [`Hash`] implementations, and doesn't hang
...
/// Let $n_p$ be `p_numerator`, $d_p$ be `p_denominator`, and let $p=n_p/d_p$.
///
/// Every time a value is to be generated, the iterator uniformly samples the special values
/// [`Vec`] with probability $p$, or else returns a value from the inner iterator.
///
/// If $p > 0$, the output length is infinite. Otherwise, it is the same as the length of `xs`.
///
...
pub mod comparison;
/// Contains [`IteratorCache`](iterator_cache::IteratorCache), which remembers values produced by
/// an iterator.
