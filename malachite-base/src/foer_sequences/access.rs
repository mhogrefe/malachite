...
impl<T: Eq> Index<usize> for FoerSequence<T> {
    type Output = T;

    /// Gets a reference to an element of a [`FoerSequence`] at an index.
    ///
    ...
    ///
    /// assert_eq!(FoerSequence::from_slices(&[1, 2], &[3, 4])[1], 2);
    /// assert_eq!(FoerSequence::from_slices(&[1, 2], &[3, 4])[10], 3);
    /// ```
    #[inline]
...
impl<T: Eq> FoerSequence<T> {
    ...
    /// use malachite_base::foer_sequences::FoerSequence;
    ///
    /// assert_eq!(FoerSequence::from_slices(&[1, 2], &[3, 4]).get(1), Some(&2));
    /// assert_eq!(
    ///     FoerSequence::from_slices(&[1, 2], &[3, 4]).get(10),
    ///     Some(&3)
    /// );
    /// ```
...
impl<T: Clone + Eq> FoerSequence<T> {
    /// Mutates an element of a [`FoerSequence`] at an index using a provided closure, and then
    ...
    /// use malachite_base::foer_sequences::FoerSequence;
    ///
    /// let mut xs = FoerSequence::from_slices(&[1, 2], &[3, 4]);
    /// assert_eq!(
    ///     xs.mutate(1, |x| {
    ///         *x = 100;
    ///         25
    ...
    ///     25
    /// );
    /// assert_eq!(xs, FoerSequence::from_slices(&[1, 100], &[3, 4]));
    ///
    /// let mut xs = FoerSequence::from_slices(&[1, 2], &[3, 4]);
    /// assert_eq!(
    ///     xs.mutate(6, |x| {
    ...
    /// assert_eq!(
    ///     xs,
    ///     FoerSequence::from_slices(&[1, 2, 3, 4, 3, 4, 100], &[4, 3])
    /// );
    /// ```
    pub fn mutate<F: FnOnce(&mut T) -> U, U>(&mut self, i: usize, f: F) -> U {
        ...
        foer_sequence_reduce(&mut self.non_repeating, &mut self.repeating);
        result
    }
}
