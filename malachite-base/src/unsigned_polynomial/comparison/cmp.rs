...
impl<T: PrimitiveUnsigned> PartialOrd for UnsignedPolynomial<T> {
    /// Compares two [`UnsignedPolynomial`]s.
    ///
...
impl<T: PrimitiveUnsigned> Ord for UnsignedPolynomial<T> {
    /// Compares two [`UnsignedPolynomial`]s by how they behave for large arguments.
    ///
    ...
    /// constant polynomials it is the order on the `T`s, so the embedding of a number as a
