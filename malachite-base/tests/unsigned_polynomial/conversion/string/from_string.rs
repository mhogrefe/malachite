...
fn test_from_str_fail() {
    let test = |s| {
        assert!(
            UnsignedPolynomial::<u64>::from_str(s).is_err(),
            "{s:?} should not parse"
        );
    };
    test("");
    ...
    test("x^0");
    // Negatives, which no `UnsignedPolynomial` has.
    test("-x");
    test("-1");
    test("x-1");
    // Malformed syntax.
...
fn test_from_string_with() {
    ...
    assert_eq!(
        UnsignedPolynomial::<u64>::from_string_with(GreekVars.var(0), "α^2+3*α+2").unwrap(),
        p
    );
    assert_eq!(
        UnsignedPolynomial::<u64>::from_string_with(XyzVars.var(1), "y^2+3*y+2").unwrap(),
        p
    );
    ...
    assert!(UnsignedPolynomial::<u64>::from_string_with(GreekVars.var(0), "x^2").is_none());
    assert!(UnsignedPolynomial::<u64>::from_string_with(XyzVars.var(1), "x^2").is_none());
}
...
fn from_str_properties() {
    unsigned_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(
            UnsignedPolynomial::<u64>::from_str(&p.to_string()).unwrap(),
            p
        );
        let greek = p.to_string_with(GreekVars.var(0));
        assert_eq!(
            UnsignedPolynomial::<u64>::from_string_with(GreekVars.var(0), &greek).unwrap(),
            p
        );
        // `FromStr` is `from_string_with` with the first `XyzVars` variable.
        assert_eq!(
            UnsignedPolynomial::<u64>::from_string_with(XyzVars.var(0), &p.to_string()).unwrap(),
            p
        );
    });
    ...
    string_gen().test_properties(|s| {
        ...
        if let Ok(p) = UnsignedPolynomial::<u64>::from_str(&s) {
            assert!(p.is_valid());
            assert_eq!(
                UnsignedPolynomial::<u64>::from_str(&p.to_string()).unwrap(),
                p
