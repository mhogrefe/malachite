...
fn test_to_string() {
    let test = |s| {
        assert_eq!(
            UnsignedPolynomial::<u64>::from_str(s).unwrap().to_string(),
            s
        );
    };
    test("0");
...
fn to_string_properties() {
    unsigned_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(UnsignedPolynomial::<u64>::from_str(&s).unwrap(), p);
        assert!(!s.is_empty());
        // Nothing in the output can be mistaken for a variable's name.
        assert!(!s.contains(' '));
        assert!(!s.contains('-'));
        assert_eq!(s == "0", p == UnsignedPolynomial::<u64>::ZERO);
        ...
        assert_eq!(
            UnsignedPolynomial::<u64>::from_string_with(vars.var(0), &named).unwrap(),
            p
        );
