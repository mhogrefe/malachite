...
fn random_hash_maps_size_inclusive_range_helper<
...
>(
    ...
    let xs = random_hash_maps_size_inclusive_range(EXAMPLE_SEED, a, b, keys_gen, values_gen);
    let values = xs.take(20).collect_vec();
    assert_eq!(values.as_slice(), expected_values);
}

#[test]
fn test_random_hash_maps_size_inclusive_range() {
    random_hash_maps_size_inclusive_range_helper(
        2,
        3,
...
#[test]
#[should_panic]
fn random_hash_maps_size_inclusive_range_fail() {
    random_hash_maps_size_inclusive_range(
        EXAMPLE_SEED,
        2,
        1,
