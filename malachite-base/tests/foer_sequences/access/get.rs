...
#[should_panic]
fn index_fail_1() {
    FoerSequence::<u8>::from_vec(vec![])[0];
}
...
#[should_panic]
fn index_fail_2() {
    FoerSequence::from_vec(vec![1, 2, 3])[3];
}

#[test]
fn get_properties() {
    unsigned_foer_sequence_unsigned_pair_gen_var_1::<u8, usize>().test_properties(|(xs, index)| {
...
#[test]
fn index_properties() {
    unsigned_foer_sequence_unsigned_pair_gen_var_2::<u8>().test_properties(|(xs, index)| {
        let x = xs[index];
