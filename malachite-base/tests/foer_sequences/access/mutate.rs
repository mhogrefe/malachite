...
pub fn test_mutate() {
    fn test(
        ...
        let mut xs = FoerSequence::from_slices(non_repeating, repeating);
        assert_eq!(
            xs.mutate(index, |x| {
                *x = new_value;
                out
        ...
        assert_eq!(
            xs,
            FoerSequence::from_slices(non_repeating_out, repeating_out)
        );
    }
...
#[should_panic]
fn mutate_fail_1() {
    FoerSequence::<u8>::from_vec(vec![]).mutate(0, |_| {});
}

#[test]
#[should_panic]
fn mutate_fail_2() {
    FoerSequence::from_vec(vec![1, 2, 3]).mutate(3, |_| {});
}
