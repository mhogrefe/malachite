...
pub fn test_from_vec_and_from_slice() {
    fn test(non_repeating: &[u8], out: &str) {
        assert_eq!(FoerSequence::from_slice(non_repeating).to_string(), out);
        assert_eq!(
            FoerSequence::from_vec(non_repeating.to_vec()).to_string(),
            out
        );
    }
    test(&[], "[]");
...
fn from_vec_and_from_slice_properties() {
    unsigned_vec_gen::<u8>().test_properties(|xs| {
        let rs = FoerSequence::from_slice(&xs);
        assert!(rs.is_valid());
        assert_eq!(rs.to_string(), xs.to_debug_string());
        assert!(rs.is_finite());
        assert_eq!(FoerSequence::from_vec(xs), rs);
    });
}
