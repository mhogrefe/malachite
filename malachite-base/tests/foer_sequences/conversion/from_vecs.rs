...
use malachite_base::foer_sequences::{FoerSequence, foer_sequence_is_reduced};
...
pub fn test_from_vecs_and_from_slices() {
    fn test(non_repeating: &[u8], repeating: &[u8], out: &str) {
        assert_eq!(
            FoerSequence::from_slices(non_repeating, repeating).to_string(),
            out
        );
        assert_eq!(
            FoerSequence::from_vecs(non_repeating.to_vec(), repeating.to_vec()).to_string(),
            out
        );
    }
    test(&[], &[], "[]");
...
fn from_vec_and_from_slice_properties() {
    unsigned_vec_pair_gen::<u8>().test_properties(|(xs, ys)| {
        let rs = FoerSequence::from_slices(&xs, &ys);
        assert!(rs.is_valid());
        assert_eq!(FoerSequence::from_vecs(xs.clone(), ys.clone()), rs);
        if foer_sequence_is_reduced(&xs, &ys) {
