...
#[test]
fn test_mod_op() {
    // `mod_op` is `%` under the name the mod-family traits use; a `U64Polynomial`'s coefficients
    // are never negative, so there is no case where the two could differ.
    let test = |s, m, out| {
        let p = U64Polynomial::from_str(s).unwrap();
        let q = (&p).mod_op(m);
        assert!(q.is_valid());
...
#[should_panic]
fn mod_op_fail() {
    let _ = U64Polynomial::from_str("x").unwrap().mod_op(0);
}
...
#[should_panic]
fn mod_assign_fail() {
    let mut p = U64Polynomial::from_str("x").unwrap();
    p.mod_assign(0);
}
...
fn test_rem_lowers_the_degree() {
    ...
    let test = |s, m, out| {
        assert_eq!((U64Polynomial::from_str(s).unwrap() % m).to_string(), out);
    };
    test("4*x^2+3", 4, "3");
...
#[should_panic]
fn rem_fail() {
    let _ = U64Polynomial::from_str("x").unwrap() % 0;
}
...
fn rem_properties() {
    u64_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, m)| {
    ...
    u64_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(&p % 1, U64Polynomial::ZERO);
