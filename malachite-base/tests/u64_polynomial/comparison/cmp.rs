...
fn test_cmp_degree_dominates() {
    ...
    // largest coefficient a `U64Polynomial` can hold.
    let test = |s, t| {
        let p = U64Polynomial::from_str(s).unwrap();
        let q = U64Polynomial::from_str(t).unwrap();
...
fn cmp_properties() {
    ...
    u64_polynomial_gen().test_properties(|p| {
        ...
        assert!(p >= U64Polynomial::ZERO);
    });

    u64_polynomial_triple_gen().test_properties(|(p, q, r)| {
...
fn test_cmp_constants_agree_with_u64s() {
    ...
    let test = |x: u64, y: u64| {
        assert_eq!(
            U64Polynomial::from(x).cmp(&U64Polynomial::from(y)),
            x.cmp(&y)
        );
    };
    test(0, 0);
