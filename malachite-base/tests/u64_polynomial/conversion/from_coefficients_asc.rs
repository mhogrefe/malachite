...
fn test_from_coefficients_asc() {
    let test = |xs: &[u32], out| {
        let p = U64Polynomial::from_coefficients_asc(u64s(xs));
        assert!(p.is_valid());
...
fn from_coefficients_asc_properties() {
    unsigned_vec_gen::<u64>().test_properties(|xs| {
        let p = U64Polynomial::from_coefficients_asc(xs.clone());
        assert!(p.is_valid());
        ...
        padded.push(0);
        assert_eq!(U64Polynomial::from_coefficients_asc(padded), p);
        assert_eq!(
            U64Polynomial::from_coefficients_asc(p.coefficients_asc().to_vec()),
            p
        );
    });
}
