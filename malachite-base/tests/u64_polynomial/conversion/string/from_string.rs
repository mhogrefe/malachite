...
fn test_from_str_fail() {
    let test = |s| {
        assert!(
            U64Polynomial::from_str(s).is_err(),
            "{s:?} should not parse"
        );
    };
    test("");
    ...
    test("x^0");
    // Negatives, which no `U64Polynomial` has.
    test("-x");
    test("-1");
    test("x-1");
    // Malformed syntax.
...
fn test_from_string_with() {
    ...
    assert_eq!(
        U64Polynomial::from_string_with(GreekVars.var(0), "α^2+3*α+2").unwrap(),
        p
    );
    assert_eq!(
        U64Polynomial::from_string_with(XyzVars.var(1), "y^2+3*y+2").unwrap(),
        p
    );
    ...
    assert!(U64Polynomial::from_string_with(GreekVars.var(0), "x^2").is_none());
    assert!(U64Polynomial::from_string_with(XyzVars.var(1), "x^2").is_none());
}
...
fn from_str_properties() {
    u64_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(U64Polynomial::from_str(&p.to_string()).unwrap(), p);
        let greek = p.to_string_with(GreekVars.var(0));
        assert_eq!(
            U64Polynomial::from_string_with(GreekVars.var(0), &greek).unwrap(),
            p
        );
        // `FromStr` is `from_string_with` with the first `XyzVars` variable.
        assert_eq!(
            U64Polynomial::from_string_with(XyzVars.var(0), &p.to_string()).unwrap(),
            p
        );
    });
    ...
    string_gen().test_properties(|s| {
        ...
        if let Ok(p) = U64Polynomial::from_str(&s) {
            assert!(p.is_valid());
            assert_eq!(U64Polynomial::from_str(&p.to_string()).unwrap(), p);
        }
    });
