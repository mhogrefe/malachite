...
fn test_to_string() {
    let test = |s| {
        assert_eq!(U64Polynomial::from_str(s).unwrap().to_string(), s);
    };
    test("0");
    test("1");
    test("5");
    ...
    test("x^2+3*x+2");
    test("x^10+x");
    test("18446744073709551615*x^2+1");
}
...
fn test_to_string_normalizes() {
    ...
    let test = |s, out| {
        assert_eq!(U64Polynomial::from_str(s).unwrap().to_string(), out);
    };
...
fn to_string_properties() {
    u64_polynomial_gen().test_properties(|p| {
        ...
        assert_eq!(U64Polynomial::from_str(&s).unwrap(), p);
        assert!(!s.is_empty());
        // Nothing in the output can be mistaken for a variable's name.
        assert!(!s.contains(' '));
        assert!(!s.contains('-'));
        assert_eq!(s == "0", p == U64Polynomial::ZERO);
        ...
        assert_eq!(
            U64Polynomial::from_string_with(vars.var(0), &named).unwrap(),
            p
        );
