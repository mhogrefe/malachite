...
fn test_serde() {
    let test = |s, out| {
        ...
        assert_eq!(serde_json::from_str::<U64Polynomial>(out).unwrap(), p);
    };
...
#[test]
fn test_serde_fail() {
    // A trailing zero coefficient is not something a `U64Polynomial` can hold: the list is rejected
    // rather than quietly trimmed, since accepting it would build a polynomial that an equal one
    // would not match.
    let test = |s: &str| {
        assert!(
            serde_json::from_str::<U64Polynomial>(s).is_err(),
...
fn serde_properties() {
    u64_polynomial_gen().test_properties(|p| {
        ...
        let q = serde_json::from_str::<U64Polynomial>(&s).unwrap();
        assert!(q.is_valid());
        assert_eq!(q, p);
