...
fn test_degree() {
    let test = |s, out| {
        assert_eq!(U64Polynomial::from_str(s).unwrap().degree(), out);
    };
    test("0", None);
    test("1", Some(0));
    test("5", Some(0));
...
fn test_coefficient() {
    let test = |s, i, out| {
        assert_eq!(U64Polynomial::from_str(s).unwrap().coefficient(i), out);
    };
    test("0", 0, 0);
    test("0", 100, 0);
    test("x^2+3*x+2", 0, 2);
...
fn test_leading_coefficient() {
    let test = |s, out| {
        assert_eq!(
            U64Polynomial::from_str(s).unwrap().leading_coefficient(),
            out
        );
    };
    test("0", 0);
...
fn coefficients_properties() {
    u64_polynomial_gen().test_properties(|p| {
        ...
        let q = U64Polynomial::from_coefficients_asc(cs.clone());
        assert!(q.is_valid());
        assert_eq!(q, p);
    ...
    u64_polynomial_unsigned_pair_gen_var_1().test_properties(|(p, i)| {
