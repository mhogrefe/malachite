...
fn mod_power_of_two_square_helper<T: PrimitiveUnsigned>() {
    let test = |x: T, pow: u64, out| {
        assert_eq!(x.mod_power_of_two_square(pow), out);

        let mut mut_x = x;
        mut_x.mod_power_of_two_square_assign(pow);
        assert_eq!(mut_x, out);
    };
...
fn test_mod_power_of_two_square() {
    apply_fn_to_unsigneds!(mod_power_of_two_square_helper);
}
