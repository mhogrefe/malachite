...
macro_rules! test_common_constants {
    ($t: ident) => {
        assert_eq!(NiceFloat($t::ZERO), NiceFloat(0.0));
        assert_eq!(NiceFloat($t::ONE), NiceFloat(1.0));
        assert_eq!(NiceFloat($t::TWO), NiceFloat(2.0));
        assert_eq!(NiceFloat($t::NEGATIVE_ONE), NiceFloat(-1.0));
        assert_eq!(NiceFloat($t::NEGATIVE_ZERO), NiceFloat(-0.0));
    };
}
